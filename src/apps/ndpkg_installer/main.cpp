...
std::wstring ToWide(const fs::path &p) { return p.wstring(); }
...
uint64_t GetFileVersionQuad(const fs::path &filePath) {
  ...
  if (!GetFileVersionInfoW(fileWide.c_str(), handle, size, versionData.data()))
    return 0;

  VS_FIXEDFILEINFO *fileInfo = nullptr;
  UINT fileInfoLen = 0;
  if (!VerQueryValueW(versionData.data(), L"\\",
                      reinterpret_cast<LPVOID *>(&fileInfo), &fileInfoLen) ||
      !fileInfo) {
    return 0;
  }
...
bool CopyPayloadToTempZip(const fs::path &ndpkgPath, const fs::path &zipOutPath,
  ...
  if (memcmp(footer.magic, kNdpkgMagic, strlen(kNdpkgMagic)) != 0 ||
      footer.version != 1) {
    errorOut = L"Invalid package: missing or invalid NDPKG footer.";
    return false;
  }

  const std::streamoff payloadSize =
      fileSize - static_cast<std::streamoff>(sizeof(NdpkgFooter));
  in.clear();
  ...
  while (remaining > 0) {
    const auto chunk = static_cast<std::streamsize>(std::min<std::streamoff>(
...
std::vector<std::string> ParseAddons(const json &j) {
  ...
  for (const auto &item : j["addons"]) {
    if (item.is_string())
      addons.push_back(item.get<std::string>());
  }
  return addons;
}
...
bool LoadPackageMetadata(const fs::path &extractDir, PackageInfo &outInfo,
                         std::wstring &errorOut) {
  ...
  if (!j.contains("name") || !j["name"].is_string() || !j.contains("version") ||
      !j["version"].is_string() || !j.contains("author") ||
      !j["author"].is_string()) {
...
bool CopyDirectoryRecursive(
    const fs::path &src, const fs::path &dst, std::wstring &errorOut,
...
bool ResolveInstallTargets(fs::path &widgetsOut, fs::path &addonsOut) {
  ...
  if (docsWidgets.empty() || docsAddons.empty())
    return false;
...
std::vector<fs::path> GetPackageAddonFiles(const AppState *state) {
  ...
  if (!fs::exists(addonsDir) || !fs::is_directory(addonsDir))
    return out;
  ...
  for (const auto &listedAddon : state->info.addons) {
    ...
    if (itName != byName.end()) {
      ...
      if (itStem != byStem.end())
        resolved = itStem->second;
    }
    if (!resolved.empty()) {
...
void BuildAddonRows(AppState *state) {
  ...
  const bool hasTargets =
      ResolveInstallTargets(widgetsTarget, installedAddonsTarget);
  ...
  for (const auto &addonPath : packageAddons) {
    ...
    if (hasTargets) {
      const fs::path installedFile =
          installedAddonsTarget / addonPath.filename();
...
void PopulateAddonsListView(AppState *state) {
  ...
  for (size_t i = 0; i < state->addonRows.size(); ++i) {
    ...
    if (itemIndex >= 0) {
      ...
      ListView_SetItemText(state->hAddons, itemIndex, 1,
                           const_cast<LPWSTR>(statusText));
      ListView_SetCheckState(state->hAddons, itemIndex,
...
void SetInstallingUiState(AppState *state, bool installing) {
  ...
  if (state->hInstall)
    ShowWindow(state->hInstall, installing ? SW_HIDE : SW_SHOW);
  if (state->hCancel)
    ShowWindow(state->hCancel, installing ? SW_HIDE : SW_SHOW);
  if (state->hProgress)
    ShowWindow(state->hProgress, installing ? SW_SHOW : SW_HIDE);
...
bool RequestManageQuit() {
  ...
  DWORD_PTR result = 0;
  SendMessageTimeoutW(manageWindow, closeMessage, 0, 0, SMTO_ABORTIFHUNG, 1500,
                      &result);
  ...
  // Fallback for elevation mismatch (non-admin installer vs admin
  // Manage/Novadesk): invoke manage_novadesk with elevation so it can request
  // close at same integrity level.
  wchar_t exePath[MAX_PATH]{};
  GetModuleFileNameW(nullptr, exePath, MAX_PATH);
  fs::path exeDir = fs::path(exePath).parent_path();
...
bool StartManageWindow() {
  ...
  BOOL started =
      CreateProcessW(nullptr, mutableCommandLine.data(), nullptr, nullptr,
                     FALSE, 0, nullptr, exeDir.wstring().c_str(), &si, &pi);
  if (!started)
    return false;
...
void UpdateUiWithPackage(AppState *state) {
  ...
  state->hPreviewBitmap =
      LoadScaledPreviewBitmap(state->info.previewImagePath, state->hPreview);
  if (state->hPreviewBitmap) {
    SendMessageW(state->hPreview, STM_SETIMAGE, IMAGE_BITMAP,
                 reinterpret_cast<LPARAM>(state->hPreviewBitmap));
  } else {
    SendMessageW(state->hPreview, STM_SETIMAGE, IMAGE_BITMAP, 0);
  }
...
bool LoadPackageFromPath(AppState *state, const fs::path &packagePath,
                         std::wstring &errorOut) {
  ...
  state->workingDir =
      fs::path(tempDir) / (L"ndpkg_installer_" + std::to_wstring(pid) + L"_" +
                           std::to_wstring(tick));
  state->extractDir = state->workingDir / "extract";
  const fs::path payloadZip = state->workingDir / "payload.zip";

  if (!EnsureDirectory(state->workingDir, errorOut))
    return false;
  if (!EnsureDirectory(state->extractDir, errorOut))
    return false;
  if (!CopyPayloadToTempZip(packagePath, payloadZip, errorOut))
    return false;
  if (!ExtractZip(payloadZip, state->extractDir, errorOut))
    return false;

  PackageInfo pkg{};
  if (!LoadPackageMetadata(state->extractDir, pkg, errorOut))
    return false;
...
bool PromptForPackagePath(HWND owner, fs::path &outPath) {
  ...
  ofn.hwndOwner = owner;
  ofn.lpstrFilter =
      L"Novadesk Package (*.ndpkg)\0*.ndpkg\0All Files (*.*)\0*.*\0\0";
...
void HandleInstall(AppState *state) {
  ...
  if (!RequestManageQuit()) {
    ...
    MessageBoxW(state->hwnd, L"Failed to close running Manage Novadesk.",
  ...
  if (!ResolveInstallTargets(widgetsTarget, addonsTarget)) {
    SetInstallingUiState(state, false);
    MessageBoxW(state->hwnd, L"Failed to resolve Novadesk install paths.",
                L"Install Error", MB_ICONERROR | MB_OK);
    return;
  }
  ...
  for (const auto &entry :
       fs::recursive_directory_iterator(widgetsSource, countEc)) {
    if (countEc)
      break;
    if (entry.is_regular_file())
      ++widgetFileCount;
  ...
  if (!CopyDirectoryRecursive(widgetsSource, widgetsTarget, error, [&]() {
        ...
        const int widgetProgress =
            35 +
            static_cast<int>((widgetFileCount == 0
                                  ? 1.0
                                  : (static_cast<double>(copiedWidgetFiles) /
                                     static_cast<double>(widgetFileCount))) *
                             45.0);
  ...
  for (size_t i = 0; i < state->addonRows.size(); ++i) {
    if (!state->hAddons ||
        ListView_GetCheckState(state->hAddons, static_cast<int>(i))) {
  ...
  for (const auto &addonPath : addonFiles) {
    std::error_code ec;
    fs::copy_file(addonPath, addonsTarget / addonPath.filename(),
                  fs::copy_options::overwrite_existing, ec);
    if (ec) {
      SetInstallingUiState(state, false);
      std::wstring fileErr =
          L"Failed to copy addon: " + ToWide(addonPath.filename());
      MessageBoxW(state->hwnd, fileErr.c_str(), L"Install Error",
                  MB_ICONERROR | MB_OK);
      return;
    }
    ++copiedAddons;
    const int addonProgress =
        80 +
        static_cast<int>((addonCount == 0 ? 1.0
                                          : (static_cast<double>(copiedAddons) /
                                             static_cast<double>(addonCount))) *
                         15.0);
  ...
  if (!StartManageWindow()) {
    ...
    MessageBoxW(
        state->hwnd,
        L"Installation completed, but failed to restart Manage Novadesk.",
        L"Install Error", MB_ICONERROR | MB_OK);
    return;
  }
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
  ...
  case WM_CREATE: {
    ...
    state->hTitle = CreateWindowW(
        L"STATIC", L"Select a package to view details", WS_CHILD | WS_VISIBLE,
        16, 10, 656, 34, hwnd, reinterpret_cast<HMENU>(IDC_WIDGET_TITLE),
        nullptr, nullptr);
    state->hTitleFont =
        CreateFontW(-26, 0, 0, 0, FW_BOLD, FALSE, FALSE, FALSE, DEFAULT_CHARSET,
                    OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, CLEARTYPE_QUALITY,
                    DEFAULT_PITCH | FF_DONTCARE, L"Segoe UI");
    state->hUiFont = CreateFontW(-16, 0, 0, 0, FW_NORMAL, FALSE, FALSE, FALSE,
    ...
    HWND hGroupVisuals = CreateWindowW(
        L"BUTTON", L"Widget Visuals", WS_CHILD | WS_VISIBLE | BS_GROUPBOX, 324,
        58, 356, 346, hwnd, nullptr, nullptr, nullptr);
    state->hPreview = CreateWindowExW(
        WS_EX_CLIENTEDGE, L"STATIC", L"",
        WS_CHILD | WS_VISIBLE | SS_BITMAP | SS_CENTERIMAGE, 340, 86, 324, 266,
        hwnd, reinterpret_cast<HMENU>(IDC_PREVIEW), nullptr, nullptr);

    state->hInstall = CreateWindowW(
        L"BUTTON", L"Install", WS_CHILD | WS_VISIBLE | BS_DEFPUSHBUTTON, 468,
        364, 95, 30, hwnd, reinterpret_cast<HMENU>(IDC_INSTALL), nullptr,
        nullptr);
    state->hCancel = CreateWindowW(
        L"BUTTON", L"Cancel", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, 571, 364,
        95, 30, hwnd, reinterpret_cast<HMENU>(IDC_CANCEL), nullptr, nullptr);
    state->hProgress = CreateWindowExW(
        0, PROGRESS_CLASSW, L"", WS_CHILD, 468, 370, 198, 20, hwnd,
    ...
    SendMessageW(hGroupVisuals, WM_SETFONT,
                 reinterpret_cast<WPARAM>(state->hUiFont), TRUE);
    SendMessageW(state->hInstall, WM_SETFONT,
                 reinterpret_cast<WPARAM>(state->hUiFont), TRUE);
    SendMessageW(state->hCancel, WM_SETFONT,
                 reinterpret_cast<WPARAM>(state->hUiFont), TRUE);
...
int WINAPI wWinMain(HINSTANCE hInstance, HINSTANCE, PWSTR, int nCmdShow) {
  ...
  AppState state;
  if (Gdiplus::GdiplusStartup(&state.gdiplusToken, &gdiplusInput, nullptr) !=
      Gdiplus::Ok) {
    MessageBoxW(nullptr, L"Failed to initialize GDI+.", L"NDPKG Installer",
                MB_OK | MB_ICONERROR);
    return 1;
  }
  ...
  if (!RegisterClassExW(&wc)) {
    ...
    MessageBoxW(nullptr, L"Failed to register window class.",
                L"NDPKG Installer", MB_OK | MB_ICONERROR);
    return 1;
  }
  ...
  if (TryGetCliPackagePath(cliPackage)) {
    ...
    if (!LoadPackageFromPath(&state, cliPackage, loadError)) {
      Gdiplus::GdiplusShutdown(state.gdiplusToken);
      MessageBoxW(nullptr, loadError.c_str(), L"Failed to Load Package",
                  MB_ICONERROR | MB_OK);
      return 1;
    }
  } else {
  ...
  HWND hwnd =
      CreateWindowExW(0, kWindowClassName, kWindowTitle,
  ...
  if (!hwnd) {
    Gdiplus::GdiplusShutdown(state.gdiplusToken);
    MessageBoxW(nullptr, L"Failed to create main window.", L"NDPKG Installer",
                MB_OK | MB_ICONERROR);
    return 1;
  }
