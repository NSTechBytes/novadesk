...
#define IOCTL_VIDEO_SET_DISPLAY_BRIGHTNESS CTL_CODE(FILE_DEVICE_VIDEO, 0x127, METHOD_BUFFERED, FILE_ANY_ACCESS)
    ...
    namespace
    {
        ...
        public:
            ...
            bool Play()
            {
                Queue(NowPlayingAction::Play);
                return true;
            }
            bool Pause()
            {
                Queue(NowPlayingAction::Pause);
                return true;
            }
            bool PlayPause()
            {
                Queue(NowPlayingAction::PlayPause);
                return true;
            }
            bool Stop()
            {
                Queue(NowPlayingAction::Stop);
                return true;
            }
            bool Next()
            {
                Queue(NowPlayingAction::Next);
                return true;
            }
            bool Previous()
            {
                Queue(NowPlayingAction::Previous);
                return true;
            }
            bool SetPosition(int value, bool isPercent)
            {
                Queue(NowPlayingAction::SetPosition, value, isPercent);
                return true;
            }
            bool SetShuffle(bool enabled)
            {
                Queue(NowPlayingAction::SetShuffle, 0, enabled);
                return true;
            }
            bool ToggleShuffle()
            {
                Queue(NowPlayingAction::ToggleShuffle);
                return true;
            }
            bool SetRepeat(int mode)
            {
                Queue(NowPlayingAction::SetRepeat, mode, false);
                return true;
            }
                ...
                while (!pending.empty())
                {
                    ...
                    try
                    {
                        ...
                        case NowPlayingAction::SetPosition:
                        {
                            ...
                            int duration = 0;
                            if (timeline != nullptr)
                                duration = (std::max)(0, ToSeconds(timeline.EndTime()) - ToSeconds(timeline.StartTime()));
                ...
                while (true)
                {
                    std::queue<NowPlayingActionItem> pending;
                    {
                        ...
                        m_actionSignal.wait_until(lock, nextRefresh, [&]
                                                  { return m_stopWorker || !m_actions.empty(); });
                        if (m_stopWorker)
                            break;
