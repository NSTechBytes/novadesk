...
bool GetCpuStats(CpuStats &outStats) {
  ...
  double usage = (static_cast<double>(totalDelta - idleDelta) * 100.0) /
...
bool GetDiskStats(const std::wstring &path, DiskStats &outStats) {
  ...
  if (!GetDiskFreeSpaceExW(target.c_str(), &freeBytesAvailable, &totalBytes,
                           &totalFreeBytes)) {
    return false;
  }
...
bool OpenRecycleBin() {
  HINSTANCE result = ShellExecuteW(
      ...
      L"/N,::{645FF040-5081-101B-9F08-00AA002F954E}", nullptr, SW_SHOWNORMAL);
...
BOOL CALLBACK EnumDisplayMonitorsForMetrics(HMONITOR hMonitor, HDC,
  ...
  if (GetMonitorInfoW(hMonitor, &mi)) {
    ...
    while (EnumDisplayDevicesW(mi.szDevice, monIdx++, &ddm, 0)) {
      if ((ddm.StateFlags & DISPLAY_DEVICE_ACTIVE) &&
          (ddm.StateFlags & DISPLAY_DEVICE_ATTACHED)) {
...
DisplayMetrics GetDisplayMetrics() {
  ...
  ctx.out = &out;
  EnumDisplayMonitors(nullptr, nullptr, EnumDisplayMonitorsForMetrics,
...
bool Execute(const std::wstring &target, const std::wstring &parameters,
             const std::wstring &workingDir, int show) {
  HINSTANCE result =
      ShellExecuteW(nullptr, L"open", target.c_str(),
                    parameters.empty() ? nullptr : parameters.c_str(),
                    workingDir.empty() ? nullptr : workingDir.c_str(), show);
  return reinterpret_cast<INT_PTR>(result) > 32;
}
...
bool ParseValueSpan(const std::string &text, size_t &i, size_t end,
                    Span &span) {
  SkipWhitespaceAndComments(text, i, end);
  if (i >= end) {
    return false;
...
bool CollectObjectMembers(const std::string &text, const Span &objSpan,
    ...
    if (it != members.end()) {
      ...
      if (item.value().is_object()) {
        ...
        if (i < span.end && text[i] == '{') {
          Span nestedObj;
          if (FindRootObjectSpan(text.substr(span.start, span.end - span.start),
                                 nestedObj)) {
            Span adjusted;
...
bool JsonReadTextFile(const std::wstring &path, std::string &outText) {
  ...
  outText.assign(std::istreambuf_iterator<char>(f),
...
bool JsonMergePatchFile(const std::wstring &path,
                        const std::string &patchText) {
  try {
    const nlohmann::json patch =
        nlohmann::json::parse(patchText, nullptr, true, true);
    if (!patch.is_object()) {
      return false;
    }

    std::string current;
    if (novadesk::shared::system::JsonReadTextFile(path, current) &&
      ...
      ::Logging::Log(
          ::LogLevel::Warn,
          L"JSON merge failed; rewriting without preserving comments: %s",
          path.c_str());
    }

    return novadesk::shared::system::JsonWriteTextFile(path, patch.dump(4));
  } catch (const nlohmann::json::parse_error &e) {
    ::Logging::Log(::LogLevel::Error, L"JSON parse error in merge patch: %S",
                   e.what());
    return false;
  } catch (...) {
    return false;
...
std::string ShowMessageBox(const MessageBoxOptions &opts) {
  ...
  std::transform(lowerButtons.begin(), lowerButtons.end(), lowerButtons.begin(),
                 ::towlower);
  ...
  if (lowerButtons == L"ok-cancel" || lowerButtons == L"okcancel") {
    ...
    flags |= MB_YESNO;
  } else if (lowerButtons == L"yes-no-cancel" ||
             lowerButtons == L"yesnocancel") {
    flags |= MB_YESNOCANCEL;
  } else if (lowerButtons == L"retry-cancel" ||
             lowerButtons == L"retrycancel") {
    flags |= MB_RETRYCANCEL;
  } else if (lowerButtons == L"abort-retry-ignore" ||
             lowerButtons == L"abortretryignore") {
  ...
  int result =
      MessageBoxW(opts.parent, opts.message.c_str(), opts.title.c_str(), flags);
  ...
  if (SUCCEEDED(hr) && pFileOpen) {
    ...
    if (!opts.defaultPath.empty()) {
      ...
      if (SUCCEEDED(SHCreateItemFromParsingName(
              opts.defaultPath.c_str(), nullptr, IID_PPV_ARGS(&pFolder))) &&
          pFolder) {
    ...
    if (!opts.filters.empty() && !opts.openDirectory) {
      ...
      pFileOpen->SetFileTypes(static_cast<UINT>(filterSpecs.size()),
                              filterSpecs.data());
    }
...
SaveFileDialogResult ShowSaveFileDialog(const SaveFileDialogOptions &opts) {
  ...
  if (SUCCEEDED(hr) && pFileSave) {
    ...
    if (!opts.filters.empty()) {
      ...
      pFileSave->SetFileTypes(static_cast<UINT>(filterSpecs.size()),
                              filterSpecs.data());
    }
...
bool GetNetworkStats(NetworkStats &outStats) {
  ...
  if (g_lastNetworkSample != std::chrono::steady_clock::time_point::min()) {
    const double dt =
        std::chrono::duration<double>(now - g_lastNetworkSample).count();
    if (dt > 0.0) {
...
bool GetPowerStatus(PowerStatus &outStatus) {
  ...
  if (cpuCount > 0) {
    ...
    if (CallNtPowerInformation(ProcessorInformation, nullptr, 0, ppi.data(),
                               sizeof(PROCESSOR_POWER_INFORMATION_LOCAL) *
                                   cpuCount) == 0) {
...
bool SplitRegistryPath(const std::wstring &fullPath, HKEY &outRoot,
                       std::wstring &outSubKey) {
...
bool RegistryReadData(const std::wstring &fullPath,
  ...
  DWORD type = 0;
  DWORD size = 0;
  if (RegQueryValueExW(key, valueName.c_str(), nullptr, &type, nullptr,
                       &size) != ERROR_SUCCESS) {
    RegCloseKey(key);
    return false;
  }
...
bool RegistryWriteString(const std::wstring &fullPath,
  ...
  const bool ok = (RegSetValueExW(key, valueName.c_str(), 0, REG_SZ, raw,
                                  rawSize) == ERROR_SUCCESS);
  RegCloseKey(key);
  return ok;
}
...
bool RegistryWriteNumber(const std::wstring &fullPath,
  ...
  const bool ok = (RegSetValueExW(key, valueName.c_str(), 0, REG_DWORD,
...
bool SetWallpaper(const std::wstring &imagePath, const std::wstring &style) {
  ...
  HKEY key = nullptr;
  const LONG openRes = RegOpenKeyExW(
      HKEY_CURRENT_USER, L"Control Panel\\Desktop", 0, KEY_SET_VALUE, &key);
...
bool WebFetch(const std::wstring &url, std::string &outData) {
  ...
  const auto appendResponseData = [&outData, &url](const char *data,
                                                   size_t size) -> bool {
    if (outData.size() >= kWebFetchMaxResponseBytes ||
        size > kWebFetchMaxResponseBytes - outData.size()) {
      Logging::Log(LogLevel::Error,
                   L"WebFetch: Response from '%s' exceeds the %zu-byte limit",
                   url.c_str(), kWebFetchMaxResponseBytes);
      outData.clear();
      return false;
    }
  ...
  if (!isHttp) {
    ...
    if (url.rfind(L"file://", 0) == 0) {
      path = url.substr(7);
      if (!path.empty() && path[0] == L'/' && path.size() > 2 &&
          path[2] == L':') {
        path = path.substr(1);
      }
    }
  ...
  HINTERNET hInternet = InternetOpenW(
      L"Novadesk WebFetch", INTERNET_OPEN_TYPE_PRECONFIG, nullptr, nullptr, 0);
  if (!hInternet) {
    return false;
  }
  ...
  HINTERNET hUrl =
      InternetOpenUrlW(hInternet, url.c_str(), nullptr, 0, flags, 0);
  if (!hUrl) {
