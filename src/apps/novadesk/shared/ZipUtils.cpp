...
namespace novadesk::shared
{
    ...
    namespace
    {
        bool AddFileToZipHandle(zipFile zf, const fs::path &filePath, const std::string &zipEntryPath, int level)
        {
    ...
    bool CompressToZip(const fs::path &sourcePath,
                       const fs::path &destZipPath,
                       ...
                       std::string &errorOut)
    {
        ...
        {
            for (const auto &entry : fs::recursive_directory_iterator(sourcePath, fs::directory_options::skip_permission_denied, ec))
            {
                if (ec)
                    continue;
    ...
    bool ExtractFromZip(const fs::path &zipPath,
                        const fs::path &destDirPath,
        ...
        do
        {
            ...
            else
            {
                ...
                if (!out.is_open())
                {
                    ...
                    closeUnz();
                    errorOut = "Failed to create target file on disk: " + targetPath.string();
                    return false;
                }
    ...
    bool ReadZipEntryContent(const fs::path &zipPath,
                             ...
                             std::string &errorOut)
    {
        ...
        unz_file_info64 fileInfo = {};
        char rawName[1024] = {};
        if (unzGetCurrentFileInfo64(uf, &fileInfo, rawName, sizeof(rawName), nullptr, 0, nullptr, 0) != UNZ_OK)
        {
            unzClose(uf);
        ...
        if (fileInfo.uncompressed_size > 0)
        {
            ...
            int bytes = unzReadCurrentFile(uf, outContent.data(), static_cast<unsigned int>(outContent.size()));
            if (bytes < 0)
            {
                unzCloseCurrentFile(uf);
                unzClose(uf);
