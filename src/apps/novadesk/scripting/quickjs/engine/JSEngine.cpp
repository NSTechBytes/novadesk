...
std::unordered_map<std::wstring, std::unordered_map<int, JSValue>>
    g_widgetContextMenuCallbacks;
struct TrayCommandCallback {
  int trayId = 0;
  JSValue callback = JS_UNDEFINED;
};
std::unordered_map<int, TrayCommandCallback> g_trayCommandCallbacks;
std::unordered_map<int, std::unordered_map<std::string, std::vector<JSValue>>>
    g_trayEventCallbacks;
struct IpcListener {
  JSValue callback = JS_UNDEFINED;
  std::wstring owner;
};
...
std::unordered_map<std::string, std::vector<IpcListener>>
    g_mainIpcChannelListeners;
std::unordered_map<std::string, std::vector<IpcListener>>
    g_uiIpcChannelListeners;
...
public:
  explicit ScriptExecutionScope(const std::wstring &ownerScriptPath)
      : m_PreviousScriptPath(g_currentScriptPath),
        m_PreviousScriptDir(g_currentScriptDir) {
...
void ClearIpcChannelListenersForScript(
    std::unordered_map<std::string, std::vector<IpcListener>> &map,
    const std::wstring &scriptPath) {
...
bool ExecuteScriptFile(const std::wstring &finalScriptPath) {
  ...
  if (script.empty()) {
    Logging::Log(LogLevel::Error, L"Failed to load script: %s",
                 finalScriptPath.c_str());
    return false;
  }
  ...
  JS_SetPropertyStr(g_context, global, "__dirname",
                    JS_NewString(g_context, dirName.c_str()));
  const std::wstring effectiveMainScriptPath =
      g_mainScriptPath.empty() ? finalScriptPath : g_mainScriptPath;
  const std::wstring effectiveMainScriptDirPath =
      PathUtils::GetParentDir(effectiveMainScriptPath);
  JS_SetPropertyStr(
      g_context, global, "__mainScriptDirPath",
      JS_NewString(g_context,
                   Utils::ToString(effectiveMainScriptDirPath).c_str()));
  JS_SetPropertyStr(
      g_context, global, "__widgetDir",
      JS_NewString(g_context,
                   Utils::ToString(PathUtils::GetWidgetsDir()).c_str()));
  JS_SetPropertyStr(
  ...
  JSValue result = JS_Eval(g_context, moduleSource.c_str(), moduleSource.size(),
                           evalModuleName.c_str(), JS_EVAL_TYPE_MODULE);
...
enum class ConsoleLevel { Log = 0, Info = 1, Warn = 2, Error = 3, Debug = 4 };
...
JSValue JsConsoleWrite(JSContext *ctx, JSValueConst, int argc,
...
JSValue JsSetTimerImpl(JSContext *ctx, JSValueConst, int argc,
                       JSValueConst *argv, int magic) {
...
JSValue JsClearTimerImpl(JSContext *ctx, JSValueConst, int argc,
                         JSValueConst *argv) {
  if (argc < 1)
    return JS_UNDEFINED;
  int64_t id64 = 0;
...
void DispatchToastEventNow(int callbackId, const ToastEventData &data) {
  ...
  JS_SetPropertyStr(g_context, event, "toastId",
                    JS_NewInt64(g_context, data.toastId));
  JS_SetPropertyStr(g_context, event, "type",
                    JS_NewString(g_context, data.type.c_str()));
  if (data.actionIndex >= 0)
    JS_SetPropertyStr(g_context, event, "actionIndex",
                      JS_NewInt32(g_context, data.actionIndex));
  if (!data.input.empty())
    JS_SetPropertyStr(
        g_context, event, "input",
        JS_NewString(g_context, Utils::ToString(data.input).c_str()));
  if (!data.dismissalReason.empty())
    JS_SetPropertyStr(g_context, event, "reason",
                      JS_NewString(g_context, data.dismissalReason.c_str()));
...
JSValue JsPathBasename(JSContext *ctx, JSValueConst, int argc,
                       JSValueConst *argv) {
  if (argc < 1)
...
JSValue JsPathDirname(JSContext *ctx, JSValueConst, int argc,
                      JSValueConst *argv) {
  if (argc < 1)
    return JS_NewString(ctx, ".");
  const char *s = JS_ToCString(ctx, argv[0]);
...
JSValue JsPathExtname(JSContext *ctx, JSValueConst, int argc,
                      JSValueConst *argv) {
  if (argc < 1)
    return JS_NewString(ctx, "");
  const char *s = JS_ToCString(ctx, argv[0]);
...
JSValue JsPathIsAbsolute(JSContext *ctx, JSValueConst, int argc,
                         JSValueConst *argv) {
  if (argc < 1)
...
JSValue JsPathNormalize(JSContext *ctx, JSValueConst, int argc,
                        JSValueConst *argv) {
  if (argc < 1)
    return JS_NewString(ctx, ".");
  const char *s = JS_ToCString(ctx, argv[0]);
...
JSValue JsPathRelative(JSContext *ctx, JSValueConst, int argc,
                       JSValueConst *argv) {
  if (argc < 2)
    return JS_NewString(ctx, "");
  ...
  const std::wstring rel =
      PathUtils::Relative(Utils::ToWString(from), Utils::ToWString(to));
  JS_FreeCString(ctx, from);
  JS_FreeCString(ctx, to);
...
JSValue JsPathParse(JSContext *ctx, JSValueConst, int argc,
                    JSValueConst *argv) {
  if (argc < 1)
  ...
  JS_SetPropertyStr(ctx, obj, "root",
                    JS_NewString(ctx, Utils::ToString(parts.root).c_str()));
  JS_SetPropertyStr(ctx, obj, "dir",
                    JS_NewString(ctx, Utils::ToString(parts.dir).c_str()));
  JS_SetPropertyStr(ctx, obj, "base",
                    JS_NewString(ctx, Utils::ToString(parts.base).c_str()));
  JS_SetPropertyStr(ctx, obj, "ext",
                    JS_NewString(ctx, Utils::ToString(parts.ext).c_str()));
  JS_SetPropertyStr(ctx, obj, "name",
                    JS_NewString(ctx, Utils::ToString(parts.name).c_str()));
  return obj;
}

JSValue JsPathFormat(JSContext *ctx, JSValueConst, int argc,
                     JSValueConst *argv) {
...
void RegisterConsoleBindings(JSContext *ctx) {
  ...
  JSValue printFn = JS_NewCFunction(
      ctx,
      [](JSContext *c, JSValueConst thisVal, int argc,
         JSValueConst *argv) -> JSValue {
        return JsConsoleWrite(c, thisVal, argc, argv,
                              static_cast<int>(ConsoleLevel::Log));
      },
      "print", 1);
  ...
  JS_SetPropertyStr(ctx, consoleObj, "log",
                    JS_NewCFunctionMagic(ctx, JsConsoleWrite, "log", 1,
                                         JS_CFUNC_generic_magic,
                                         static_cast<int>(ConsoleLevel::Log)));
  JS_SetPropertyStr(ctx, consoleObj, "info",
                    JS_NewCFunctionMagic(ctx, JsConsoleWrite, "info", 1,
                                         JS_CFUNC_generic_magic,
                                         static_cast<int>(ConsoleLevel::Info)));
  JS_SetPropertyStr(ctx, consoleObj, "warn",
                    JS_NewCFunctionMagic(ctx, JsConsoleWrite, "warn", 1,
                                         JS_CFUNC_generic_magic,
                                         static_cast<int>(ConsoleLevel::Warn)));
  JS_SetPropertyStr(ctx, consoleObj, "error",
                    JS_NewCFunctionMagic(
                        ctx, JsConsoleWrite, "error", 1, JS_CFUNC_generic_magic,
                        static_cast<int>(ConsoleLevel::Error)));
  JS_SetPropertyStr(ctx, consoleObj, "debug",
                    JS_NewCFunctionMagic(
                        ctx, JsConsoleWrite, "debug", 1, JS_CFUNC_generic_magic,
                        static_cast<int>(ConsoleLevel::Debug)));
  JS_SetPropertyStr(ctx, global, "console", consoleObj);
  JS_SetPropertyStr(ctx, global, "setTimeout",
                    JS_NewCFunctionMagic(ctx, JsSetTimerImpl, "setTimeout", 2,
                                         JS_CFUNC_generic_magic, 0));
  JS_SetPropertyStr(ctx, global, "setInterval",
                    JS_NewCFunctionMagic(ctx, JsSetTimerImpl, "setInterval", 2,
  ...
  JS_SetPropertyStr(ctx, pathObj, "join",
                    JS_NewCFunction(ctx, JsPathJoin, "join", 1));
  JS_SetPropertyStr(ctx, pathObj, "basename",
                    JS_NewCFunction(ctx, JsPathBasename, "basename", 2));
  JS_SetPropertyStr(ctx, pathObj, "dirname",
                    JS_NewCFunction(ctx, JsPathDirname, "dirname", 1));
  JS_SetPropertyStr(ctx, pathObj, "extname",
                    JS_NewCFunction(ctx, JsPathExtname, "extname", 1));
  JS_SetPropertyStr(ctx, pathObj, "isAbsolute",
                    JS_NewCFunction(ctx, JsPathIsAbsolute, "isAbsolute", 1));
  JS_SetPropertyStr(ctx, pathObj, "normalize",
                    JS_NewCFunction(ctx, JsPathNormalize, "normalize", 1));
  JS_SetPropertyStr(ctx, pathObj, "relative",
                    JS_NewCFunction(ctx, JsPathRelative, "relative", 2));
  JS_SetPropertyStr(ctx, pathObj, "parse",
                    JS_NewCFunction(ctx, JsPathParse, "parse", 1));
  JS_SetPropertyStr(ctx, pathObj, "format",
                    JS_NewCFunction(ctx, JsPathFormat, "format", 1));
...
void LogQuickJsValue(JSContext *ctx, JSValueConst value,
...
void HostPromiseRejectionTracker(JSContext *ctx, JSValueConst,
...
int RegisterCallback(std::vector<JSValue> &list, JSContext *ctx,
                     JSValueConst fn) {
...
JSValue BuildIpcMessage(JSContext *ctx, JSValueConst typeVal,
  ...
  JS_SetPropertyStr(ctx, msg, "channel",
                    JS_NewString(ctx, channel ? channel : type.c_str()));
  return msg;
}
...
void DispatchChannelIpc(
    std::unordered_map<std::string, std::vector<IpcListener>> &listeners,
    const std::string &channel, const char *from, const char *to,
    JSValueConst payload, bool payloadFirst = false) {
  auto it = listeners.find(channel);
  if (it == listeners.end())
    return;

  JSValue channelVal = JS_NewString(g_context, channel.c_str());
  JSValue eventObj = BuildIpcMessage(g_context, channelVal, payload, from, to,
                                     channel.c_str());
...
JSValue JsMainIpcOn(JSContext *ctx, JSValueConst, int argc,
                    JSValueConst *argv) {
  ...
  if (!GetChannelArg(ctx, argv[0], channel)) {
    return JS_ThrowTypeError(ctx,
                             "ipcMain.on channel must be non-empty string");
  }
  RegisterChannelListener(g_mainIpcChannelListeners, ctx, channel, argv[1]);
  return JS_UNDEFINED;
}

JSValue JsMainIpcHandle(JSContext *ctx, JSValueConst, int argc,
                        JSValueConst *argv) {
  if (argc < 2 || !JS_IsFunction(ctx, argv[1])) {
  ...
  if (!GetChannelArg(ctx, argv[0], channel)) {
    return JS_ThrowTypeError(ctx,
                             "ipcMain.handle channel must be non-empty string");
  }
...
JSValue JsUiIpcOn(JSContext *ctx, JSValueConst, int argc, JSValueConst *argv) {
  if (argc < 2 || !JS_IsFunction(ctx, argv[1])) {
    return JS_ThrowTypeError(ctx,
                             "ipcRenderer.on requires (channel, listener)");
  }
  std::string channel;
  if (!GetChannelArg(ctx, argv[0], channel)) {
    return JS_ThrowTypeError(ctx,
                             "ipcRenderer.on channel must be non-empty string");
  }
...
JSValue JsUiIpcSend(JSContext *ctx, JSValueConst, int argc,
  ...
  std::string channel;
  if (GetChannelArg(ctx, argv[0], channel)) {
    DispatchChannelIpc(g_mainIpcChannelListeners, channel, "ui", "main",
                       payload);
  }
  return JS_UNDEFINED;
}

JSValue JsUiIpcInvoke(JSContext *ctx, JSValueConst, int argc,
                      JSValueConst *argv) {
  if (argc < 1) {
    return JS_ThrowTypeError(ctx,
                             "ipcRenderer.invoke requires at least a channel");
  }
  std::string channel;
  if (!GetChannelArg(ctx, argv[0], channel)) {
    return JS_ThrowTypeError(
        ctx, "ipcRenderer.invoke channel must be non-empty string");
  }
  auto it = g_mainIpcHandlers.find(channel);
  if (it == g_mainIpcHandlers.end()) {
    return JS_ThrowReferenceError(ctx, "No ipcMain handler for channel: %s",
                                  channel.c_str());
  }
  ...
  JSValue eventObj =
      BuildIpcMessage(ctx, channelVal, payload, "ui", "main", channel.c_str());
...
JSValue CreateUiIpcObjectImpl(JSContext *ctx) {
  ...
  JS_SetPropertyStr(ctx, ipc, "invoke",
                    JS_NewCFunction(ctx, JsUiIpcInvoke, "invoke", 2));
  return ipc;
}
...
bool EnsureRuntime() {
  ...
  JS_SetModuleLoaderFunc(g_runtime,
                         novadesk::scripting::quickjs::ModuleNormalizeName,
                         novadesk::scripting::quickjs::ModuleLoader, nullptr);
  JS_SetHostPromiseRejectionTracker(g_runtime, HostPromiseRejectionTracker,
                                    nullptr);
...
std::wstring ResolveEntryScript(const std::wstring &scriptPath) {
  if (scriptPath.empty()) {
    const std::wstring defaultEntry =
        PathUtils::ResolvePath(L"index.js", PathUtils::GetWidgetsDir());
    std::error_code ec;
  ...
  if (!ec) {
    const std::wstring fromCwd =
        PathUtils::ResolvePath(scriptPath, cwd.wstring());
...
bool LoadAndExecuteScripts(duk_context *ctx,
  ...
  for (const auto &finalScriptPath : resolved) {
    Logging::Log(LogLevel::Info, L"Executing script: %s",
                 finalScriptPath.c_str());
  ...
  if (loadedPaths.empty()) {
    if (isDefaultLoad) {
      Logging::Log(
          LogLevel::Info,
          L"No default Widgets\\index.js found. Starting without scripts.");
      return true;
    }
...
void OnTimer(UINT_PTR id) {
  ...
  if (it->second.repeat) {
    ...
    JSValue ret =
        JS_Call(g_context, entry.callback, JS_UNDEFINED,
                static_cast<int>(entry.args.size()), entry.args.data());
    if (JS_IsException(ret)) {
      LogQuickJsException(g_context);
    } else {
      JS_FreeValue(g_context, ret);
  ...
  ScriptExecutionScope scope(entry.owner);
  JSValue ret = JS_Call(g_context, entry.callback, JS_UNDEFINED,
                        static_cast<int>(entry.args.size()), entry.args.data());
  if (JS_IsException(ret)) {
    LogQuickJsException(g_context);
  } else {
...
void OnMessage(UINT message, WPARAM wParam, LPARAM lParam) {
  ...
  if (message == WM_NOVADESK_DISPATCH) {
    ...
    case DISPATCH_WEBFETCH:
      novadesk::scripting::quickjs::DispatchWebFetchResult(
...
void OnTrayCommand(int commandId) {
  ...
  Logging::Log(
      ...
      commandId, it->second.trayId, ownerScriptPath.c_str());
  ScriptExecutionScope scope(ownerScriptPath);
  JSValue ret =
      JS_Call(g_context, it->second.callback, JS_UNDEFINED, 0, nullptr);
  if (JS_IsException(ret)) {
    LogQuickJsException(g_context);
  } else {
    JS_FreeValue(g_context, ret);
...
void CallEventCallback(int callbackId, Widget *widget,
  ...
  if (data) {
    JS_SetProperty(g_context, arg, g_atom_clientX,
                   JS_NewInt32(g_context, data->clientX));
    JS_SetProperty(g_context, arg, g_atom_clientY,
                   JS_NewInt32(g_context, data->clientY));
    JS_SetProperty(g_context, arg, g_atom_screenX,
                   JS_NewInt32(g_context, data->screenX));
    JS_SetProperty(g_context, arg, g_atom_screenY,
                   JS_NewInt32(g_context, data->screenY));
    JS_SetProperty(g_context, arg, g_atom_offsetX,
                   JS_NewInt32(g_context, data->offsetX));
    JS_SetProperty(g_context, arg, g_atom_offsetY,
                   JS_NewInt32(g_context, data->offsetY));
    JS_SetProperty(g_context, arg, g_atom_offsetXPercent,
                   JS_NewInt32(g_context, data->offsetXPercent));
    JS_SetProperty(g_context, arg, g_atom_offsetYPercent,
                   JS_NewInt32(g_context, data->offsetYPercent));
  } else {
...
void CallEventCallbackWithText(int callbackId, Widget *widget,
  ...
  // e.data and e.value both carry the current text (mirrors the Web InputEvent
  // API).
  const std::string utf8 = Utils::ToString(text);
  JS_SetPropertyStr(g_context, arg, "data",
                    JS_NewString(g_context, utf8.c_str()));
  JS_SetPropertyStr(g_context, arg, "value",
                    JS_NewString(g_context, utf8.c_str()));
...
void CallDropCallback(int callbackId, Widget *widget,
  ...
  if (data) {
    ...
    JS_SetProperty(g_context, arg, g_atom_offsetYPercent,
                   JS_NewInt32(g_context, data->offsetYPercent));
    JS_SetPropertyStr(g_context, arg, "effect",
                      JS_NewString(g_context, data->effect.c_str()));
    ...
    for (uint32_t i = 0; i < static_cast<uint32_t>(data->files.size()); ++i) {
      ...
      JS_SetPropertyUint32(g_context, filesArr, i,
                           JS_NewString(g_context, filePath.c_str()));
    }
    JS_SetPropertyStr(g_context, arg, "files", filesArr);
    JS_SetPropertyStr(
        g_context, arg, "totalFiles",
        JS_NewInt32(g_context, static_cast<int32_t>(data->files.size())));
  }
...
void DispatchToastEventAsync(int callbackId, const ToastEventData &data) {
  ...
  if (g_messageWindow) {
    if (PostMessageW(g_messageWindow, WM_NOVADESK_DISPATCH, DISPATCH_TOAST,
...
bool RegisterWidgetContextMenuCallback(JSContext *ctx,
  ...
  std::lock_guard<std::recursive_mutex> lock(g_engineMutex);
  if (!ctx || ctx != g_context || widgetId.empty() || commandId <= 0 ||
      !JS_IsFunction(ctx, fn)) {
    return false;
  }
...
bool RegisterTrayEventCallback(JSContext *ctx, int trayId,
  ...
  std::lock_guard<std::recursive_mutex> lock(g_engineMutex);
  if (!ctx || ctx != g_context || eventName.empty() ||
      !JS_IsFunction(ctx, fn)) {
    return false;
  }
...
bool ExecuteWidgetScript(Widget *widget) {
  ...
  return novadesk::scripting::quickjs::ExecuteWidgetUiScript(g_context, widget,
                                                             scriptPath);
}
