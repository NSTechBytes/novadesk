...
static void host_RegisterString(novadesk_context c, const char* name, const char* value) {
    ...
    if (!call || call->stack.empty() || !name) return;
...
static void host_RegisterObjectEnd(novadesk_context c, const char* name) {
    auto* call = reinterpret_cast<AddonCallContext*>(c);
    if (!call || call->stack.size() < 2 || !name) return;
...
static void host_RegisterFunction(novadesk_context c, const char* name, int (*func)(novadesk_context), int nargs) {
    auto* call = reinterpret_cast<AddonCallContext*>(c);
    if (!call || call->stack.empty() || !name || !func) return;
...
static void* host_JsGetFunctionPtr(novadesk_context c, int index) {
    auto* call = reinterpret_cast<AddonCallContext*>(c);
    JSValue* v = ResolveByIndex(call, index);
    if (!call || !v || !JS_IsFunction(call->ctx, *v)) return nullptr;
...
static void host_JsCallFunction(novadesk_context c, void* funcPtr, int nargs) {
    ...
    if (!call || !handle || !JS_IsFunction(handle->ctx, handle->fn)) return;
    ...
    for (JSValue& a : argv) JS_FreeValue(call->ctx, a);
    for (int i = 0; i < nargs; ++i) {
        JS_FreeValue(call->ctx, call->stack.back());
        call->stack.pop_back();
    }
...
JSValue JsAddonLoad(JSContext* ctx, JSValueConst, int argc, JSValueConst* argv) {
    ...
    call.ctx = ctx;
    call.addon = &stored;
    call.stack.push_back(JS_NewObject(ctx));

    initFn(reinterpret_cast<novadesk_context>(&call), JSEngine::GetMessageWindow(), &g_hostApi);

    if (call.hasThrow) {
        for (JSValue& v : call.args) JS_FreeValue(ctx, v);
        for (JSValue& v : call.stack) JS_FreeValue(ctx, v);
        if (stored.unloadFn) {
            try { stored.unloadFn(); } catch (...) {}
        }
    ...
    return JS_NewInt32(ctx, stored.id);
}

JSValue JsAddonUnload(JSContext* ctx, JSValueConst, int argc, JSValueConst* argv) {
    if (argc < 1) {
        return JS_ThrowTypeError(ctx, "addon.unload(id) requires id");
    }
    int32_t addonId = 0;
    if (JS_ToInt32(ctx, &addonId, argv[0]) != 0) {
        return JS_ThrowTypeError(ctx, "addon.unload(id) expects number");
    }
    auto pit = g_addonPathById.find(static_cast<int>(addonId));
    ...
    if (it->second.unloadFn) {
        try {
            ...
            Logging::Log(LogLevel::Error, L"Crash in NovadeskAddonUnload for addon id %d", addonId);
        }
    }
    ...
    for (void* p : it->second.functionHandles) {
        ...
        if (h) {
            if (!JS_IsUndefined(h->fn)) JS_FreeValue(h->ctx, h->fn);
            delete h;
        }
    }
