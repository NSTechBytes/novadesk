...
JSValue JsWidgetWindowOn(JSContext *ctx, JSValueConst thisVal, int argc,
                         JSValueConst *argv) {
...
JSValue JsWidgetWindowSetProperties(JSContext *ctx, JSValueConst thisVal,
  ...
  if (parsed.hasBackgroundImage || parsed.hasBackgroundImageSize ||
    ...
    if (parsed.hasBackgroundImageSize) {
      if (parsed.backgroundImageSizeIsExplicit) {
        ...
        size.type = parsed.backgroundImageSize == L"contain"
                        ? BackgroundImageSize::Type::Contain
                    : parsed.backgroundImageSize == L"stretch"
                        ? BackgroundImageSize::Type::Stretch
                        : BackgroundImageSize::Type::Cover;
      }
    }
    ...
    widget->SetBackgroundImage(parsed.hasBackgroundImage
    ...
    if (!iconPath.empty()) {
      if (PathUtils::IsPathRelative(iconPath)) {
        ...
        if (!base.empty())
          ...
          iconPath =
              PathUtils::ResolvePath(iconPath, PathUtils::GetWidgetsDir());
      } else {
...
JSValue JsWidgetWindowGetProperties(JSContext *ctx, JSValueConst thisVal, int,
                                    JSValueConst *) {
  Widget *widget = GetWidget(ctx, thisVal);
  if (!widget)
    return JS_UNDEFINED;
  ...
  JS_SetPropertyStr(ctx, out, "id",
                    JS_NewString(ctx, Utils::ToString(o.id).c_str()));
  ...
  JS_SetPropertyStr(ctx, out, "draggable",
                    JS_NewBool(ctx, o.draggable ? 1 : 0));
  JS_SetPropertyStr(ctx, out, "resizable",
                    JS_NewBool(ctx, o.resizable ? 1 : 0));
  JS_SetPropertyStr(ctx, out, "clickThrough",
                    JS_NewBool(ctx, o.clickThrough ? 1 : 0));
  JS_SetPropertyStr(ctx, out, "keepOnScreen",
                    JS_NewBool(ctx, o.keepOnScreen ? 1 : 0));
  JS_SetPropertyStr(ctx, out, "snapEdges",
                    JS_NewBool(ctx, o.snapEdges ? 1 : 0));
  JS_SetPropertyStr(ctx, out, "showInToolbar",
                    JS_NewBool(ctx, o.showInToolbar ? 1 : 0));
  JS_SetPropertyStr(ctx, out, "toolbarIcon",
                    JS_NewString(ctx, Utils::ToString(o.toolbarIcon).c_str()));
  JS_SetPropertyStr(ctx, out, "toolbarTitle",
                    JS_NewString(ctx, Utils::ToString(o.toolbarTitle).c_str()));
  JS_SetPropertyStr(
      ctx, out, "show",
      JS_NewBool(ctx, IsWindowVisible(widget->GetWindow()) ? 1 : 0));
  JS_SetPropertyStr(ctx, out, "windowOpacity",
                    JS_NewInt32(ctx, static_cast<int>(o.windowOpacity)));
  JS_SetPropertyStr(
      ctx, out, "backgroundColor",
      JS_NewString(ctx, Utils::ToString(o.backgroundColor).c_str()));
  JS_SetPropertyStr(
      ctx, out, "backgroundImage",
      JS_NewString(ctx, Utils::ToString(o.backgroundImage).c_str()));
  JS_SetPropertyStr(
  ...
  if (o.backgroundImageSize.type == BackgroundImageSize::Type::Explicit) {
    ...
    if (o.backgroundImageSize.hasWidth)
      JS_SetPropertyStr(ctx, size, "width",
                        JS_NewFloat64(ctx, o.backgroundImageSize.width));
    if (o.backgroundImageSize.hasHeight)
      JS_SetPropertyStr(ctx, size, "height",
                        JS_NewFloat64(ctx, o.backgroundImageSize.height));
    JS_SetPropertyStr(ctx, out, "backgroundImageSize", size);
  } else {
    const char *size =
        o.backgroundImageSize.type == BackgroundImageSize::Type::Contain
            ? "contain"
        : o.backgroundImageSize.type == BackgroundImageSize::Type::Stretch
            ? "stretch"
            : "cover";
    JS_SetPropertyStr(ctx, out, "backgroundImageSize", JS_NewString(ctx, size));
  }
  if (o.backgroundImagePosition.type ==
      BackgroundImagePosition::Type::Explicit) {
    JSValue position = JS_NewObject(ctx);
    JS_SetPropertyStr(ctx, position, "x",
                      JS_NewFloat64(ctx, o.backgroundImagePosition.x));
    JS_SetPropertyStr(ctx, position, "y",
                      JS_NewFloat64(ctx, o.backgroundImagePosition.y));
    JS_SetPropertyStr(ctx, out, "backgroundImagePosition", position);
  } else {
    JS_SetPropertyStr(
        ctx, out, "backgroundImagePosition",
        JS_NewString(
            ctx, Utils::ToString(o.backgroundImagePosition.keyword).c_str()));
  }
  JS_SetPropertyStr(ctx, out, "zPos",
                    JS_NewInt32(ctx, static_cast<int>(o.zPos)));
  JS_SetPropertyStr(ctx, out, "script",
                    JS_NewString(ctx, Utils::ToString(o.scriptPath).c_str()));
  return out;
}

JSValue JsWidgetWindowClose(JSContext *ctx, JSValueConst thisVal, int,
                            JSValueConst *) {
  Widget *widget = GetWidget(ctx, thisVal);
  if (!widget)
    return JS_UNDEFINED;
...
JSValue JsWidgetWindowDestroy(JSContext *ctx, JSValueConst thisVal, int,
                              JSValueConst *) {
  Widget *widget = GetWidget(ctx, thisVal);
  if (!widget)
    return JS_UNDEFINED;
...
JSValue JsWidgetWindowShow(JSContext *ctx, JSValueConst thisVal, int,
                           JSValueConst *) {
  Widget *widget = GetWidget(ctx, thisVal);
  if (!widget)
    return JS_UNDEFINED;
...
JSValue JsWidgetWindowHide(JSContext *ctx, JSValueConst thisVal, int,
                           JSValueConst *) {
  Widget *widget = GetWidget(ctx, thisVal);
  if (!widget)
    return JS_UNDEFINED;
...
JSValue JsWidgetWindowIsFocused(JSContext *ctx, JSValueConst thisVal, int,
                                JSValueConst *) {
  Widget *widget = GetWidget(ctx, thisVal);
  if (!widget)
...
JSValue JsWidgetWindowIsVisible(JSContext *ctx, JSValueConst thisVal, int,
                                JSValueConst *) {
  Widget *widget = GetWidget(ctx, thisVal);
  if (!widget)
    return JS_NewBool(ctx, 0);
...
JSValue JsWidgetWindowIsDestroyed(JSContext *ctx, JSValueConst thisVal, int,
                                  JSValueConst *) {
...
JSValue JsWidgetWindowSetBounds(JSContext *ctx, JSValueConst thisVal, int argc,
                                JSValueConst *argv) {
  Widget *widget = GetWidget(ctx, thisVal);
  if (!widget)
    return JS_UNDEFINED;
  ...
  if (!JS_IsUndefined(xv) && !JS_IsNull(xv) && JS_ToInt32(ctx, &v, xv) == 0)
    x = static_cast<int>(v);
  JS_FreeValue(ctx, xv);

  JSValue yv = JS_GetPropertyStr(ctx, argv[0], "y");
  if (!JS_IsUndefined(yv) && !JS_IsNull(yv) && JS_ToInt32(ctx, &v, yv) == 0)
    y = static_cast<int>(v);
  JS_FreeValue(ctx, yv);

  JSValue wv = JS_GetPropertyStr(ctx, argv[0], "width");
  if (!JS_IsUndefined(wv) && !JS_IsNull(wv) && JS_ToInt32(ctx, &v, wv) == 0)
    w = static_cast<int>(v);
  JS_FreeValue(ctx, wv);

  JSValue hv = JS_GetPropertyStr(ctx, argv[0], "height");
  if (!JS_IsUndefined(hv) && !JS_IsNull(hv) && JS_ToInt32(ctx, &v, hv) == 0)
    h = static_cast<int>(v);
  JS_FreeValue(ctx, hv);
...
JSValue JsWidgetWindowGetBounds(JSContext *ctx, JSValueConst thisVal, int,
                                JSValueConst *) {
  Widget *widget = GetWidget(ctx, thisVal);
  if (!widget)
    return JS_NULL;
...
JSValue JsWidgetWindowSetSize(JSContext *ctx, JSValueConst thisVal, int argc,
                              JSValueConst *argv) {
  Widget *widget = GetWidget(ctx, thisVal);
  if (!widget)
    return JS_UNDEFINED;
  ...
  widget->SetWindowPosition(CW_USEDEFAULT, CW_USEDEFAULT, static_cast<int>(w),
                            static_cast<int>(h));
  return JS_DupValue(ctx, thisVal);
}

JSValue JsWidgetWindowGetSize(JSContext *ctx, JSValueConst thisVal, int,
                              JSValueConst *) {
  Widget *widget = GetWidget(ctx, thisVal);
  if (!widget)
    return JS_NULL;
...
JSValue JsWidgetWindowGetBackgroundColor(JSContext *ctx, JSValueConst thisVal,
                                         int, JSValueConst *) {
  Widget *widget = GetWidget(ctx, thisVal);
  if (!widget)
    return JS_NULL;
  return JS_NewString(
      ctx, Utils::ToString(widget->GetOptions().backgroundColor).c_str());
}

JSValue JsWidgetWindowSetBackgroundColor(JSContext *ctx, JSValueConst thisVal,
                                         int argc, JSValueConst *argv) {
  Widget *widget = GetWidget(ctx, thisVal);
  if (!widget)
    return JS_UNDEFINED;
...
JSValue JsWidgetWindowSetOpacity(JSContext *ctx, JSValueConst thisVal, int argc,
                                 JSValueConst *argv) {
  Widget *widget = GetWidget(ctx, thisVal);
  if (!widget)
    return JS_UNDEFINED;
...
JSValue JsWidgetWindowSetResizable(JSContext *ctx, JSValueConst thisVal,
                                   int argc, JSValueConst *argv) {
  Widget *widget = GetWidget(ctx, thisVal);
  if (!widget)
    return JS_UNDEFINED;
...
JSValue JsWidgetWindowIsResizable(JSContext *ctx, JSValueConst thisVal, int,
                                  JSValueConst *) {
  Widget *widget = GetWidget(ctx, thisVal);
  if (!widget)
    return JS_NewBool(ctx, 0);
...
JSValue JsWidgetWindowGetMinWidth(JSContext *ctx, JSValueConst thisVal, int,
                                  JSValueConst *) {
  Widget *widget = GetWidget(ctx, thisVal);
  if (!widget)
...
JSValue JsWidgetWindowSetMinHeight(JSContext *ctx, JSValueConst thisVal,
                                   int argc, JSValueConst *argv) {
  Widget *widget = GetWidget(ctx, thisVal);
  if (!widget)
    return JS_UNDEFINED;
...
JSValue JsWidgetWindowGetMinHeight(JSContext *ctx, JSValueConst thisVal, int,
                                   JSValueConst *) {
  Widget *widget = GetWidget(ctx, thisVal);
  if (!widget)
    return JS_NewInt32(ctx, 0);
...
JSValue JsWidgetWindowSetMinSize(JSContext *ctx, JSValueConst thisVal, int argc,
                                 JSValueConst *argv) {
  Widget *widget = GetWidget(ctx, thisVal);
  if (!widget)
    return JS_UNDEFINED;
  ...
  int32_t minH = 0;
  if (JS_ToInt32(ctx, &minW, argv[0]) != 0 ||
      JS_ToInt32(ctx, &minH, argv[1]) != 0) {
    return ThrowTypeError(ctx, "setMinSize",
                          "minWidth/minHeight must be numbers");
  }
  widget->SetMinSize(static_cast<int>(minW), static_cast<int>(minH));
  return JS_DupValue(ctx, thisVal);
}

JSValue JsWidgetWindowGetMinSize(JSContext *ctx, JSValueConst thisVal, int,
                                 JSValueConst *) {
  Widget *widget = GetWidget(ctx, thisVal);
  if (!widget)
    return JS_NULL;
  ...
  JS_SetPropertyStr(ctx, out, "height",
                    JS_NewInt32(ctx, widget->GetMinHeight()));
  return out;
}

JSValue JsWidgetWindowRefresh(JSContext *ctx, JSValueConst thisVal, int,
                              JSValueConst *) {
  Widget *widget = GetWidget(ctx, thisVal);
  if (!widget)
    return JS_UNDEFINED;
...
JSValue JsWidgetWindowSetFocus(JSContext *ctx, JSValueConst thisVal, int,
                               JSValueConst *) {
  Widget *widget = GetWidget(ctx, thisVal);
  if (!widget)
    return JS_UNDEFINED;
...
JSValue JsWidgetWindowUnFocus(JSContext *ctx, JSValueConst thisVal, int,
                              JSValueConst *) {
  Widget *widget = GetWidget(ctx, thisVal);
  if (!widget)
    return JS_UNDEFINED;
...
JSValue JsWidgetWindowMinimize(JSContext *ctx, JSValueConst thisVal, int,
                               JSValueConst *) {
  Widget *widget = GetWidget(ctx, thisVal);
  if (!widget)
    return JS_UNDEFINED;
...
JSValue JsWidgetWindowUnMinimize(JSContext *ctx, JSValueConst thisVal, int,
                                 JSValueConst *) {
  Widget *widget = GetWidget(ctx, thisVal);
  if (!widget)
    return JS_UNDEFINED;
...
JSValue JsWidgetWindowIsMinimized(JSContext *ctx, JSValueConst thisVal, int,
                                  JSValueConst *) {
  Widget *widget = GetWidget(ctx, thisVal);
  if (!widget)
    return JS_NewBool(ctx, 0);
...
JSValue JsWidgetWindowMaximize(JSContext *ctx, JSValueConst thisVal, int,
                               JSValueConst *) {
  Widget *widget = GetWidget(ctx, thisVal);
  if (!widget)
    return JS_UNDEFINED;
...
JSValue JsWidgetWindowRestore(JSContext *ctx, JSValueConst thisVal, int,
                              JSValueConst *) {
  Widget *widget = GetWidget(ctx, thisVal);
  if (!widget)
    return JS_UNDEFINED;
...
JSValue JsWidgetWindowToggleMaximize(JSContext *ctx, JSValueConst thisVal, int,
                                     JSValueConst *) {
  Widget *widget = GetWidget(ctx, thisVal);
  if (!widget)
    return JS_UNDEFINED;
...
JSValue JsWidgetWindowIsMaximized(JSContext *ctx, JSValueConst thisVal, int,
                                  JSValueConst *) {
  Widget *widget = GetWidget(ctx, thisVal);
  if (!widget)
    return JS_NewBool(ctx, 0);
...
JSValue JsWidgetWindowGetHandle(JSContext *ctx, JSValueConst thisVal, int,
                                JSValueConst *) {
  Widget *widget = GetWidget(ctx, thisVal);
  if (!widget)
    return JS_NULL;
  return JS_NewInt64(ctx, static_cast<int64_t>(reinterpret_cast<uintptr_t>(
                              widget->GetWindow())));
}

JSValue JsWidgetWindowGetInternalPointer(JSContext *ctx, JSValueConst thisVal,
                                         int, JSValueConst *) {
  Widget *widget = GetWidget(ctx, thisVal);
  if (!widget)
    return JS_NULL;
  return JS_NewInt64(ctx,
                     static_cast<int64_t>(reinterpret_cast<uintptr_t>(widget)));
}

JSValue JsWidgetWindowGetTitle(JSContext *ctx, JSValueConst thisVal, int,
                               JSValueConst *) {
  Widget *widget = GetWidget(ctx, thisVal);
  if (!widget)
...
bool ParseContextMenuItems(JSContext *ctx, JSValueConst arr,
  ...
  for (uint32_t i = 0; i < len; ++i) {
    ...
    if (!item.isSeparator) {
      ...
      if (JS_IsFunction(ctx, actionV)) {
        ...
        JSEngine::RegisterWidgetContextMenuCallback(ctx, widgetId, item.id,
                                                    actionV);
      }
...
JSValue JsWidgetWindowSetContextMenu(JSContext *ctx, JSValueConst thisVal,
                                     int argc, JSValueConst *argv) {
  Widget *widget = GetWidget(ctx, thisVal);
  if (!widget)
    return JS_UNDEFINED;
...
JSValue JsWidgetWindowClearContextMenu(JSContext *ctx, JSValueConst thisVal,
                                       int, JSValueConst *) {
  Widget *widget = GetWidget(ctx, thisVal);
  if (!widget)
    return JS_UNDEFINED;
...
JSValue JsWidgetWindowDisableContextMenu(JSContext *ctx, JSValueConst thisVal,
                                         int argc, JSValueConst *argv) {
  Widget *widget = GetWidget(ctx, thisVal);
  if (!widget)
    return JS_UNDEFINED;
...
JSValue JsWidgetWindowShowDefaultContextMenuItems(JSContext *ctx,
...
static std::wstring ParseKeywordAndOffset(const std::wstring &expr,
                                          float &outOffset) {
  outOffset = 0.0f;
...
static bool ResolvePositionPreset(const std::wstring &pos,
                                  std::wstring &xKeyword,
                                  std::wstring &yKeyword) {
  std::wstring lower = pos;
  std::transform(lower.begin(), lower.end(), lower.begin(), ::towlower);

  if (lower == L"top-left" || lower == L"topleft") {
    yKeyword = L"top";
    xKeyword = L"left";
    return true;
  }
  if (lower == L"top-center" || lower == L"top") {
    yKeyword = L"top";
    xKeyword = L"center";
    return true;
  }
  if (lower == L"top-right" || lower == L"topright") {
    yKeyword = L"top";
    xKeyword = L"right";
    return true;
  }
  if (lower == L"center-left" || lower == L"left") {
    yKeyword = L"center";
    xKeyword = L"left";
    return true;
  }
  if (lower == L"center" || lower == L"middle") {
    yKeyword = L"center";
    xKeyword = L"center";
    return true;
  }
  if (lower == L"center-right" || lower == L"right") {
    yKeyword = L"center";
    xKeyword = L"right";
    return true;
  }
  if (lower == L"bottom-left" || lower == L"bottomleft") {
    yKeyword = L"bottom";
    xKeyword = L"left";
    return true;
  }
  if (lower == L"bottom-center" || lower == L"bottom") {
    yKeyword = L"bottom";
    xKeyword = L"center";
    return true;
  }
  if (lower == L"bottom-right" || lower == L"bottomright") {
    yKeyword = L"bottom";
    xKeyword = L"right";
    return true;
  }
...
static float ResolveXKeyword(const std::wstring &keyword, const ScreenArea &wa,
                             int winW, float offset) {
  if (keyword == L"left")
    return static_cast<float>(wa.left) + offset;
  if (keyword == L"right")
    return static_cast<float>(wa.right - winW) + offset;
  if (keyword == L"center" || keyword == L"middle")
    return static_cast<float>(wa.left + (wa.width() - winW) / 2) + offset;
  if (keyword == L"offscreen-left")
    return static_cast<float>(wa.left - winW) + offset;
  if (keyword == L"offscreen-right")
    return static_cast<float>(wa.right) + offset;
  return offset; // unknown keyword: use offset as-is
}

static float ResolveYKeyword(const std::wstring &keyword, const ScreenArea &wa,
                             int winH, float offset) {
  if (keyword == L"top")
    return static_cast<float>(wa.top) + offset;
  if (keyword == L"bottom")
    return static_cast<float>(wa.bottom - winH) + offset;
  if (keyword == L"center" || keyword == L"middle")
    return static_cast<float>(wa.top + (wa.height() - winH) / 2) + offset;
  if (keyword == L"offscreen-top")
    return static_cast<float>(wa.top - winH) + offset;
  if (keyword == L"offscreen-bottom")
    return static_cast<float>(wa.bottom) + offset;
  return offset;
}

// Resolve string expressions and position presets into concrete x/y on a
// target.
static void ResolveWindowTargetExpressions(
    Widget::WindowAnimationTarget &target, const ScreenArea &wa, int winW,
    int winH, bool hasXExpr, const std::wstring &xExpr, bool hasYExpr,
    const std::wstring &yExpr, bool hasPosition, const std::wstring &position,
...
static Widget::WindowAnimationTarget BuildWindowAnimationTarget(
    const PropertyParser::WindowAnimationOptions &options, bool from,
    const Widget *widget) {
  ...
  if (widget) {
    ...
    const int winW =
        options.hasWidth
            ? static_cast<int>(from ? options.fromWidth : options.width)
            : widget->GetOptions().width;
    const int winH =
        options.hasHeight
            ? static_cast<int>(from ? options.fromHeight : options.height)
            : widget->GetOptions().height;

    if (from) {
      ResolveWindowTargetExpressions(
          target, wa, winW, winH, options.fromHasXExpr, options.fromXExpr,
          options.fromHasYExpr, options.fromYExpr, options.fromHasPosition,
          options.fromPosition, options.fromOffsetX, options.fromOffsetY);
    } else {
      ResolveWindowTargetExpressions(
          target, wa, winW, winH, options.hasXExpr, options.xExpr,
          options.hasYExpr, options.yExpr, options.hasPosition,
          options.position, options.offsetX, options.offsetY);
    }
  }
...
static std::vector<Widget::WindowAnimationKeyframe>
BuildWindowKeyframes(const PropertyParser::WindowAnimationOptions &options,
                     const Widget *widget) {
  ScreenArea wa{};
  if (widget)
  ...
  for (const auto &kf : options.keyframes) {
    ...
    if (widget) {
      const int winW =
          kf.hasWidth ? static_cast<int>(kf.width) : widget->GetOptions().width;
      const int winH = kf.hasHeight ? static_cast<int>(kf.height)
                                    : widget->GetOptions().height;
      ResolveWindowTargetExpressions(
          item.values, wa, winW, winH, kf.hasXExpr, kf.xExpr, kf.hasYExpr,
          kf.yExpr, kf.hasPosition, kf.position, kf.offsetX, kf.offsetY);
    }
...
JSValue JsWidgetWindowAnimate(JSContext *ctx, JSValueConst thisVal, int argc,
                              JSValueConst *argv) {
  Widget *widget = GetWidget(ctx, thisVal);
  if (!widget)
    return JS_UNDEFINED;
  ...
  if (!options.HasAnyToProps())
    return ThrowTypeError(
        ctx, "animate",
        "to or keyframes must include at least one supported property");
  ...
  if (options.hasKeyframes) {
    const std::vector<Widget::WindowAnimationKeyframe> keyframes =
        BuildWindowKeyframes(options, widget);
    widget->StartWindowKeyframeAnimation(keyframes, options.duration,
                                         options.easing, iterationCount);
    return JS_DupValue(ctx, thisVal);
  }

  const Widget::WindowAnimationTarget to =
      BuildWindowAnimationTarget(options, false, widget);
  const Widget::WindowAnimationTarget from =
      BuildWindowAnimationTarget(options, true, widget);
  widget->StartWindowAnimation(to, from, options.duration, options.easing,
                               iterationCount);
  return JS_DupValue(ctx, thisVal);
}

JSValue JsWidgetWindowStopAnimation(JSContext *ctx, JSValueConst thisVal, int,
                                    JSValueConst *) {
  Widget *widget = GetWidget(ctx, thisVal);
  if (!widget)
    return JS_UNDEFINED;
...
const JSCFunctionListEntry kWidgetWindowEventFuncs[] = {
    ...
    JS_CFUNC_DEF("showDefaultContextMenuItems", 1,
                 JsWidgetWindowShowDefaultContextMenuItems),
};
} // namespace
...
void AttachWidgetWindowEventMethods(JSContext *ctx, JSValue proto) {
  JS_SetPropertyFunctionList(ctx, proto, kWidgetWindowEventFuncs,
                             sizeof(kWidgetWindowEventFuncs) /
                                 sizeof(kWidgetWindowEventFuncs[0]));
}
