...
std::string NormalizeModuleNameImpl(const std::string &baseName,
  ...
  const std::string revisionTag =
      req.revisionTag.empty() ? base.revisionTag : req.revisionTag;
...
char *ModuleNormalizeName(JSContext *ctx, const char *baseName,
  ...
  const std::string normalized =
      NormalizeModuleNameImpl(baseName ? baseName : "", name ? name : "");
...
JSModuleDef *ModuleLoader(JSContext *ctx, const char *moduleName, void *) {
  if (g_moduleSystemDebug) {
    std::cerr << "[novadesk] ModuleLoader: "
              << (moduleName ? moduleName : "<null>") << std::endl;
  }

  const std::string requested =
      moduleName ? std::string(moduleName) : std::string();
  ...
  if (g_uiScriptImportRestricted && IsBuiltinModuleName(requestedParts.path)) {
    JS_ThrowReferenceError(ctx, "Module not allowed in ui script: %s",
  ...
  if (source.empty()) {
    JS_ThrowReferenceError(ctx, "Cannot load module: %s",
                           moduleName ? moduleName : "<null>");
    return nullptr;
  }
