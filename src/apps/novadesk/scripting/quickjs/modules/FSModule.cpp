...
std::wstring ResolveFsPath(JSContext *ctx, JSValueConst v) {
  ...
  return PathUtils::ResolvePath(p, base.empty() ? JSEngine::GetEntryScriptDir()
                                                : base);
}

JSValue JsFsReadFile(JSContext *ctx, JSValueConst, int argc,
                     JSValueConst *argv) {
  if (argc < 1)
...
JSValue JsFsWriteFile(JSContext *ctx, JSValueConst, int argc,
                      JSValueConst *argv) {
  if (argc < 2)
...
JSValue JsFsMkdir(JSContext *ctx, JSValueConst, int argc, JSValueConst *argv) {
  ...
  std::error_code ec;
  bool ok = recursive ? fs::create_directories(fs::path(path), ec)
...
JSValue JsFsReaddir(JSContext *ctx, JSValueConst, int argc,
                    JSValueConst *argv) {
  if (argc < 1)
...
JSValue JsFsCopyFile(JSContext *ctx, JSValueConst, int argc,
                     JSValueConst *argv) {
  if (argc < 2)
  ...
  std::error_code ec;
  const bool ok = fs::copy_file(fs::path(from), fs::path(to),
...
JSValue JsFsStat(JSContext *ctx, JSValueConst, int argc, JSValueConst *argv) {
  ...
  JS_SetPropertyStr(ctx, out, "isFile",
                    JS_NewBool(ctx, fs::is_regular_file(st) ? 1 : 0));
  JS_SetPropertyStr(ctx, out, "isDirectory",
                    JS_NewBool(ctx, fs::is_directory(st) ? 1 : 0));
  JS_SetPropertyStr(ctx, out, "isSymlink",
                    JS_NewBool(ctx, fs::is_symlink(st) ? 1 : 0));
  uintmax_t size = 0;
  ...
  JS_SetPropertyStr(ctx, out, "size",
                    JS_NewFloat64(ctx, static_cast<double>(size)));
  JS_SetPropertyStr(ctx, out, "mode",
                    JS_NewInt32(ctx, static_cast<int32_t>(st.permissions())));
  return out;
}
...
JSValue JsFsZip(JSContext *ctx, JSValueConst, int argc, JSValueConst *argv) {
  if (argc < 2)
    return JS_ThrowTypeError(
        ctx, "fs.zip(sourcePath, destinationZipPath[, options])");
  ...
  std::string error;
  bool ok = novadesk::shared::CompressToZip(fs::path(src), fs::path(dst), opts,
                                            error);
  return JS_NewBool(ctx, ok ? 1 : 0);
}
...
JSValue JsFsUnzip(JSContext *ctx, JSValueConst, int argc, JSValueConst *argv) {
  if (argc < 2)
    return JS_ThrowTypeError(ctx,
                             "fs.unzip(zipPath, destinationDir[, options])");
  ...
  std::string error;
  bool ok = novadesk::shared::ExtractFromZip(fs::path(zipPath),
                                             fs::path(destDir), opts, error);
  return JS_NewBool(ctx, ok ? 1 : 0);
}

JSValue JsFsListZip(JSContext *ctx, JSValueConst, int argc,
                    JSValueConst *argv) {
  if (argc < 1)
  ...
  for (uint32_t i = 0; i < static_cast<uint32_t>(entries.size()); ++i) {
    ...
    JS_SetPropertyStr(ctx, obj, "name",
                      JS_NewString(ctx, entries[i].name.c_str()));
    JS_SetPropertyStr(ctx, obj, "isDirectory",
                      JS_NewBool(ctx, entries[i].isDirectory ? 1 : 0));
    JS_SetPropertyStr(ctx, obj, "size",
                      JS_NewFloat64(ctx, static_cast<double>(entries[i].size)));
    JS_SetPropertyStr(
        ctx, obj, "compressedSize",
        JS_NewFloat64(ctx, static_cast<double>(entries[i].compressedSize)));
...
JSValue JsFsReadZipFile(JSContext *ctx, JSValueConst, int argc,
                        JSValueConst *argv) {
  if (argc < 2)
  ...
  std::string content;
  std::string error;
  bool ok = novadesk::shared::ReadZipEntryContent(fs::path(zipPath), entryName,
                                                  content, error);
  if (!ok)
    return JS_NULL;
...
static int FsModuleInit(JSContext *ctx, JSModuleDef *m) {
  JS_SetModuleExport(ctx, m, "readFile",
                     JS_NewCFunction(ctx, JsFsReadFile, "readFile", 1));
  JS_SetModuleExport(ctx, m, "writeFile",
                     JS_NewCFunction(ctx, JsFsWriteFile, "writeFile", 3));
  JS_SetModuleExport(ctx, m, "exists",
                     JS_NewCFunction(ctx, JsFsExists, "exists", 1));
  JS_SetModuleExport(ctx, m, "mkdir",
                     JS_NewCFunction(ctx, JsFsMkdir, "mkdir", 2));
  JS_SetModuleExport(ctx, m, "readdir",
                     JS_NewCFunction(ctx, JsFsReaddir, "readdir", 1));
  JS_SetModuleExport(ctx, m, "unlink",
                     JS_NewCFunction(ctx, JsFsUnlink, "unlink", 1));
  JS_SetModuleExport(ctx, m, "rename",
                     JS_NewCFunction(ctx, JsFsRename, "rename", 2));
  JS_SetModuleExport(ctx, m, "copyFile",
                     JS_NewCFunction(ctx, JsFsCopyFile, "copyFile", 3));
  ...
  JS_SetModuleExport(ctx, m, "createZip",
                     JS_NewCFunction(ctx, JsFsZip, "createZip", 3));
  JS_SetModuleExport(ctx, m, "unzip",
                     JS_NewCFunction(ctx, JsFsUnzip, "unzip", 3));
  JS_SetModuleExport(ctx, m, "extractZip",
                     JS_NewCFunction(ctx, JsFsUnzip, "extractZip", 3));
  JS_SetModuleExport(ctx, m, "listZip",
                     JS_NewCFunction(ctx, JsFsListZip, "listZip", 1));
  JS_SetModuleExport(ctx, m, "readZipEntries",
                     JS_NewCFunction(ctx, JsFsListZip, "readZipEntries", 1));
  JS_SetModuleExport(ctx, m, "readZipFile",
                     JS_NewCFunction(ctx, JsFsReadZipFile, "readZipFile", 2));
  return 0;
}
