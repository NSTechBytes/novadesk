...
using namespace Js;
void ParseLayoutBoxOptions(JSContext *ctx, JSValueConst obj,
  ...
  if (!bg.empty()) {
    ParseGradientOrColor(bg, options.shape.solidColor, options.shape.solidAlpha,
  ...
  if (!fill.empty()) {
    bool hasFill = false;
    ParseGradientOrColor(fill, options.shape.fillColor, options.shape.fillAlpha,
                         options.shape.fillGradient, hasFill);
  }
  ...
  if (!JS_IsUndefined(opacityV) && !JS_IsNull(opacityV) &&
      JS_ToFloat64(ctx, &opacity, opacityV) == 0) {
    if (opacity <= 1.0)
      opacity *= 255.0;
    if (opacity < 0.0)
      opacity = 0.0;
  ...
  auto parseBorderStyleString =
      [](const std::wstring &str) -> ElementLayoutBox::BorderStyle {
    std::wstring lower = str;
    std::transform(lower.begin(), lower.end(), lower.begin(), ::towlower);
    if (lower == L"none")
      return ElementLayoutBox::BorderStyle::None;
    if (lower == L"hidden")
      return ElementLayoutBox::BorderStyle::Hidden;
    if (lower == L"inset")
      return ElementLayoutBox::BorderStyle::Inset;
    if (lower == L"outset")
      return ElementLayoutBox::BorderStyle::Outset;
    if (lower == L"groove")
      return ElementLayoutBox::BorderStyle::Groove;
    if (lower == L"ridge")
      return ElementLayoutBox::BorderStyle::Ridge;
    if (lower == L"dotted")
      return ElementLayoutBox::BorderStyle::Dotted;
    if (lower == L"dashed")
      return ElementLayoutBox::BorderStyle::Dashed;
    if (lower == L"double")
      return ElementLayoutBox::BorderStyle::Double;
  ...
  if (!JS_IsUndefined(styleV) && !JS_IsNull(styleV)) {
    ...
    if (JS_IsString(styleV)) {
      ...
      if (s) {
        ...
        options.borderTop = options.borderRight = options.borderBottom =
            options.borderLeft = parsed;
  ...
  if (!JS_IsUndefined(shadowV) && !JS_IsNull(shadowV)) {
    auto parseShadowObject = [&](JSValueConst shadowObj,
                                 LayoutBoxShadowOptions &outShadow) -> bool {
    ...
    if (JS_IsString(shadowV)) {
      ...
      options.boxShadowError = L"addLayoutBox: boxShadow string syntax is not "
                               L"supported; use object or object[]";
      ...
      for (uint32_t i = 0; i < len; ++i) {
        ...
        if (!parseShadowObject(item, sh)) {
          options.hasBoxShadowError = true;
          options.boxShadowError =
              L"addLayoutBox: each boxShadow array item must be an object";
          JS_FreeValue(ctx, item);
          break;
        }
      ...
      if (!parseShadowObject(shadowV, sh)) {
        options.hasBoxShadowError = true;
        options.boxShadowError =
            L"addLayoutBox: boxShadow must be an object or object[]";
      } else {
        options.boxShadows.push_back(sh);
      }
    }
  ...
  std::transform(options.direction.begin(), options.direction.end(),
                 options.direction.begin(), ::towlower);
  ...
  if (!parsedFlexDir.empty()) {
    std::transform(parsedFlexDir.begin(), parsedFlexDir.end(),
                   parsedFlexDir.begin(), ::towlower);
    if (parsedFlexDir == L"rowreverse" || parsedFlexDir == L"column" ||
        parsedFlexDir == L"columnreverse" || parsedFlexDir == L"row")
  ...
  if (!parsedAlign.empty()) {
    std::transform(parsedAlign.begin(), parsedAlign.end(), parsedAlign.begin(),
                   ::towlower);
  ...
  if (!parsedJustify.empty()) {
    std::transform(parsedJustify.begin(), parsedJustify.end(),
                   parsedJustify.begin(), ::towlower);
  ...
  if (GetIntProp(ctx, obj, "padding", pad)) {
    options.paddingLeft = options.paddingTop = options.paddingRight =
        options.paddingBottom = pad;
    // Logging::Log(LogLevel::Debug, L"[PADDING] Parsed single padding value:
    // %d", pad);
  }
  ...
  if (JS_IsArray(paddingVal)) {
    ...
    if (values.size() == 1) {
      options.paddingLeft = options.paddingTop = options.paddingRight =
          options.paddingBottom = values[0];
      // Logging::Log(LogLevel::Debug, L"[PADDING] Parsed array padding (1
      // value): %d", values[0]);
    } else if (values.size() == 2) {
      options.paddingLeft = options.paddingRight = values[0];
      options.paddingTop = options.paddingBottom = values[1];
      // Logging::Log(LogLevel::Debug, L"[PADDING] Parsed array padding (2
      // values): H=%d, V=%d", values[0], values[1]);
    } else if (values.size() >= 4) {
      options.paddingLeft = values[0];
      options.paddingTop = values[1];
      options.paddingRight = values[2];
      options.paddingBottom = values[3];
      // Logging::Log(LogLevel::Debug, L"[PADDING] Parsed array padding (4
      // values): L=%d, T=%d, R=%d, B=%d",
  ...
  if (!displayStr.empty()) {
    std::transform(displayStr.begin(), displayStr.end(), displayStr.begin(),
                   ::towlower);
  ...
  if (!listStyleTypeStr.empty()) {
    std::transform(listStyleTypeStr.begin(), listStyleTypeStr.end(),
                   listStyleTypeStr.begin(), ::towlower);
  ...
  if (JS_IsObject(stylePadding)) {
    ...
    if (!alignItems.empty()) {
      std::transform(alignItems.begin(), alignItems.end(), alignItems.begin(),
                     ::towlower);
      options.align = alignItems;
    }
    std::wstring justifyContent =
        GetStringProp(ctx, stylePadding, "justifyContent");
    if (!justifyContent.empty()) {
      std::transform(justifyContent.begin(), justifyContent.end(),
                     justifyContent.begin(), ::towlower);
    ...
    if (!styleDisplay.empty()) {
      std::transform(styleDisplay.begin(), styleDisplay.end(),
                     styleDisplay.begin(), ::towlower);
    ...
    std::wstring styleListStyleType =
        GetStringProp(ctx, stylePadding, "listStyleType");
    if (!styleListStyleType.empty()) {
      std::transform(styleListStyleType.begin(), styleListStyleType.end(),
                     styleListStyleType.begin(), ::towlower);
...
void ApplyLayoutBoxOptions(ElementLayoutBox *element,
                           const LayoutBoxOptions &options) {
  if (!element)
    return;
  ...
  if (options.hasBorderStyle) {
    element->SetBorderStyle(options.borderTop, options.borderRight,
                            options.borderBottom, options.borderLeft);
  }
  ...
  // Logging::Log(LogLevel::Debug, L"[PADDING] ApplyLayoutBoxOptions: Setting
  // padding L=%d, T=%d, R=%d, B=%d on element '%s'",
  //     options.paddingLeft, options.paddingTop, options.paddingRight,
  //     options.paddingBottom, element->GetId().c_str());
  element->SetPadding(options.paddingLeft, options.paddingTop,
                      options.paddingRight, options.paddingBottom);
}
void PreFillLayoutBoxOptions(LayoutBoxOptions &options,
                             ElementLayoutBox *element,
                             const std::wstring *direction, const int *gap,
                             const std::wstring *align,
                             const std::wstring *justify,
                             const int *paddingLeft, const int *paddingTop,
                             const int *paddingRight, const int *paddingBottom,
  ...
  options.flexDirection =
      flexDirection ? *flexDirection : element->GetLayoutDirection();
  ...
  options.paddingRight =
      paddingRight ? *paddingRight : element->GetPaddingRight();
  options.paddingBottom =
      paddingBottom ? *paddingBottom : element->GetPaddingBottom();
