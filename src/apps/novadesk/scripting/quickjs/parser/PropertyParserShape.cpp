...
void ParseShapeOptions(JSContext *ctx, JSValueConst obj, ShapeOptions &options,
  ...
  ParseGradientOrColor(stroke, options.strokeColor, options.strokeAlpha,
                       options.strokeGradient, hasStroke);

  std::wstring fill = GetStringProp(ctx, obj, "fillColor");
  bool hasFill = false;
  ParseGradientOrColor(fill, options.fillColor, options.fillAlpha,
                       options.fillGradient, hasFill);
  ...
  if (GetFloatProp(ctx, obj, "radius", uniformRadius)) {
    // Legacy alias: radius applies to both X/Y when specific values are not
    // provided.
    if (!hasRadiusX)
  ...
  options.hasCombineConsumeAll =
      GetBoolProp(ctx, obj, "consume", options.combineConsumeAll);
...
void ApplyShapeOptions(ShapeElement *element, const ShapeOptions &options) {
  ...
  if (options.strokeGradient.type != GRADIENT_NONE) {
    element->SetStroke(options.strokeWidth, options.strokeColor,
                       options.strokeAlpha);
  ...
  element->SetLinePoints(options.startX, options.startY, options.endX,
                         options.endY);
  element->SetArcParams(options.startArcAngle, options.endArcAngle,
                        options.arcClockwise);
  element->SetPathData(options.pathData);
  element->SetCurveParams(options.startX, options.startY, options.controlX,
                          options.controlY, options.control2X,
                          options.control2Y, options.endX, options.endY,
                          options.curveType);

  element->SetStrokeStyle(options.strokeStartCap, options.strokeEndCap,
                          options.strokeDashCap, options.strokeLineJoin,
                          options.strokeDashOffset, options.strokeDashes);
}
