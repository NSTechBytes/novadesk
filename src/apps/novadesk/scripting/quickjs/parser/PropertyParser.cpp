...
bool GetFloatArrayProp(JSContext* ctx, JSValueConst obj, const char* key, std::vector<float>& out, int minSize) {
    ...
    if (static_cast<int>(tmp.size()) < minSize) return false;
    out = std::move(tmp);
    return true;
}
...
D2D1_COMBINE_MODE ParseCombineMode(const std::wstring& s) {
    if (s == L"intersect") return D2D1_COMBINE_MODE_INTERSECT;
    if (s == L"exclude") return D2D1_COMBINE_MODE_EXCLUDE;
    if (s == L"xor") return D2D1_COMBINE_MODE_XOR;
...
void ParseElementOptions(JSContext* ctx, JSValueConst obj, ElementOptions& options, const std::wstring&) {
    ...
    if (bevelType == L"raised") options.bevelType = 1;
    else if (bevelType == L"sunken") options.bevelType = 2;
    else if (bevelType == L"emboss") options.bevelType = 3;
    else if (bevelType == L"pillow") options.bevelType = 4;
    else if (!bevelType.empty()) options.bevelType = 0;

    GetIntProp(ctx, obj, "bevelWidth", options.bevelWidth);
    BYTE a = options.bevelAlpha;
    std::wstring b1 = GetStringProp(ctx, obj, "bevelColor");
    if (!b1.empty()) ColorUtil::ParseRGBA(b1, options.bevelColor, a), options.bevelAlpha = a;
    a = options.bevelAlpha2;
    std::wstring b2 = GetStringProp(ctx, obj, "bevelColor2");
    if (!b2.empty()) ColorUtil::ParseRGBA(b2, options.bevelColor2, a), options.bevelAlpha2 = a;
...
void ParseImageOptions(JSContext* ctx, JSValueConst obj, ImageOptions& options, const std::wstring&) {
    ...
    if (aspect == L"preserve") options.preserveAspectRatio = IMAGE_ASPECT_PRESERVE;
    else if (aspect == L"crop") options.preserveAspectRatio = IMAGE_ASPECT_CROP;
    else if (aspect == L"stretch") options.preserveAspectRatio = IMAGE_ASPECT_STRETCH;

    GetBoolProp(ctx, obj, "grayscale", options.grayscale);
    GetBoolProp(ctx, obj, "tile", options.tile);
    int alpha = 255;
    if (GetIntProp(ctx, obj, "imageAlpha", alpha)) options.imageAlpha = static_cast<BYTE>(alpha);
    ...
    if (GetFloatArrayProp(ctx, obj, "colorMatrix", colorMatrix, 20)) {
        if (colorMatrix.size() >= 20) {
            for (int i = 0; i < 20; ++i) options.colorMatrix[i] = colorMatrix[i];
...
void ParseTextOptions(JSContext* ctx, JSValueConst obj, TextOptions& options, const std::wstring& baseDir) {
    ...
    if (!fontFace.empty()) options.fontFace = fontFace;
    ...
    if (JS_IsNumber(fw)) {
        int32_t w = 400;
        if (JS_ToInt32(ctx, &w, fw) == 0) options.fontWeight = w;
        ...
        if (s) {
            ...
            if (weight == L"thin") options.fontWeight = 100;
            else if (weight == L"extralight" || weight == L"ultralight") options.fontWeight = 200;
            else if (weight == L"light") options.fontWeight = 300;
            else if (weight == L"normal" || weight == L"regular") options.fontWeight = 400;
            else if (weight == L"medium") options.fontWeight = 500;
            else if (weight == L"semibold" || weight == L"demibold") options.fontWeight = 600;
            else if (weight == L"bold") options.fontWeight = 700;
            else if (weight == L"extrabold" || weight == L"ultrabold") options.fontWeight = 800;
            else if (weight == L"black" || weight == L"heavy") options.fontWeight = 900;
            JS_FreeCString(ctx, s);
        }
    }
    JS_FreeValue(ctx, fw);
    ...
    if (align.empty()) align = GetStringProp(ctx, obj, "align");
    std::transform(align.begin(), align.end(), align.begin(), ::towlower);
    if (align == L"left" || align == L"lefttop") options.textAlign = TEXT_ALIGN_LEFT_TOP;
    else if (align == L"center" || align == L"centertop") options.textAlign = TEXT_ALIGN_CENTER_TOP;
    else if (align == L"right" || align == L"righttop") options.textAlign = TEXT_ALIGN_RIGHT_TOP;
    else if (align == L"leftcenter") options.textAlign = TEXT_ALIGN_LEFT_CENTER;
    else if (align == L"centercenter" || align == L"middlecenter" || align == L"middle") options.textAlign = TEXT_ALIGN_CENTER_CENTER;
    else if (align == L"rightcenter") options.textAlign = TEXT_ALIGN_RIGHT_CENTER;
    else if (align == L"leftbottom") options.textAlign = TEXT_ALIGN_LEFT_BOTTOM;
    else if (align == L"centerbottom") options.textAlign = TEXT_ALIGN_CENTER_BOTTOM;
    else if (align == L"rightbottom") options.textAlign = TEXT_ALIGN_RIGHT_BOTTOM;

    std::wstring clip = GetStringProp(ctx, obj, "clipString");
    if (clip == L"none") options.clip = TEXT_CLIP_NONE;
    else if (clip == L"on" || clip == L"clip") options.clip = TEXT_CLIP_ON;
    else if (clip == L"wrap") options.clip = TEXT_CLIP_WRAP;
    else if (clip == L"ellipsis") options.clip = TEXT_CLIP_ELLIPSIS;
}
...
void ParseBarOptions(JSContext* ctx, JSValueConst obj, BarOptions& options, const std::wstring& baseDir) {
    ...
    if (orientation == L"vertical") options.orientation = BAR_VERTICAL;
    else if (orientation == L"horizontal") options.orientation = BAR_HORIZONTAL;
...
void ParseRoundLineOptions(JSContext* ctx, JSValueConst obj, RoundLineOptions& options, const std::wstring& baseDir) {
    ...
    if (capType == L"round") options.startCap = options.endCap = ROUNDLINE_CAP_ROUND;
    else if (capType == L"flat") options.startCap = options.endCap = ROUNDLINE_CAP_FLAT;

    std::wstring startCap = GetStringProp(ctx, obj, "startCap");
    if (startCap == L"round") options.startCap = ROUNDLINE_CAP_ROUND;
    else if (startCap == L"flat") options.startCap = ROUNDLINE_CAP_FLAT;

    std::wstring endCap = GetStringProp(ctx, obj, "endCap");
    if (endCap == L"round") options.endCap = ROUNDLINE_CAP_ROUND;
    else if (endCap == L"flat") options.endCap = ROUNDLINE_CAP_FLAT;

    std::vector<float> dash;
    if (GetFloatArrayProp(ctx, obj, "dashArray", dash, 1)) options.dashArray = dash;
...
void ParseShapeOptions(JSContext* ctx, JSValueConst obj, ShapeOptions& options, const std::wstring& baseDir) {
    ...
    if (!cap.empty()) options.strokeStartCap = Utils::GetCapStyle(cap);
    cap = GetStringProp(ctx, obj, "strokeEndCap");
    if (!cap.empty()) options.strokeEndCap = Utils::GetCapStyle(cap);
    cap = GetStringProp(ctx, obj, "strokeDashCap");
    if (!cap.empty()) options.strokeDashCap = Utils::GetCapStyle(cap);

    std::wstring join = GetStringProp(ctx, obj, "strokeLineJoin");
    if (!join.empty()) options.strokeLineJoin = Utils::GetLineJoin(join);
        ...
        if (JS_ToUint32(ctx, &len, lenV) == 0) {
            for (uint32_t i = 0; i < len; ++i) {
                ...
                if (JS_IsObject(it)) {
                    ...
                    if (GetBoolProp(ctx, it, "consume", op.consume)) op.hasConsume = true;
                    if (!op.id.empty()) options.combineOps.push_back(op);
                }
                JS_FreeValue(ctx, it);
            }
        }
...
void ApplyImageOptions(ImageElement* element, const ImageOptions& options) {
    ...
    if (options.hasImageTint) element->SetImageTint(options.imageTint, options.imageTintAlpha);
    if (options.hasColorMatrix) element->SetColorMatrix(options.colorMatrix.data());
}
...
void ApplyBarOptions(BarElement* element, const BarOptions& options) {
    ...
    if (options.barGradient.type != GRADIENT_NONE) element->SetBarGradient(options.barGradient);
    else if (options.hasBarColor) element->SetBarColor(options.barColor, options.barAlpha);
}
...
void ApplyRoundLineOptions(RoundLineElement* element, const RoundLineOptions& options) {
    ...
    if (options.lineGradient.type != GRADIENT_NONE) element->SetLineGradient(options.lineGradient);
    else if (options.hasLineColor) element->SetLineColor(options.lineColor, options.lineAlpha);
    if (options.lineGradientBg.type != GRADIENT_NONE) element->SetLineGradientBg(options.lineGradientBg);
    else if (options.hasLineColorBg) element->SetLineColorBg(options.lineColorBg, options.lineAlphaBg);
}
...
void ApplyShapeOptions(ShapeElement* element, const ShapeOptions& options) {
    ...
    if (options.fillGradient.type != GRADIENT_NONE) element->SetFillGradient(options.fillGradient);
    else element->SetFill(options.fillColor, options.fillAlpha);
