...
void ParsePrefixedGeneralImageOptions(JSContext *ctx, JSValueConst obj,
  ...
  std::wstring imageFlip =
      GetStringProp(ctx, obj, (prefix + "ImageFlip").c_str());
  ...
  if (GetFloatArrayProp(ctx, obj, (prefix + "ImageCrop").c_str(), imageCrop,
                        4)) {
  ...
  GetBoolProp(ctx, obj, (prefix + "UseExifOrientation").c_str(),
              out.useExifOrientation);
  ...
  if (!tint.empty() &&
      ColorUtil::ParseRGBA(tint, out.imageTint, out.imageTintAlpha)) {
    out.hasImageTint = true;
  }

  std::vector<float> colorMatrix;
  if (GetFloatArrayProp(ctx, obj, (prefix + "ColorMatrix").c_str(), colorMatrix,
                        20)) {
    for (int i = 0; i < 20; ++i)
    ...
    for (int i = 1; i <= 20; ++i) {
      float val = 0.0f;
      if (GetFloatProp(ctx, obj,
                       (prefix + "ColorMatrix" + std::to_string(i)).c_str(),
                       val)) {
