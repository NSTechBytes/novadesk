...
namespace {
bool ParseColorAlphaProp(JSContext *ctx, JSValueConst obj, const char *key,
...
void ParseInputBoxOptions(JSContext *ctx, JSValueConst obj,
...

  {
    ...
    if (!fontStr.empty()) {
      bool hasColor = false;
      ParseGradientOrColor(fontStr, options.fontColor, options.fontAlpha,
                           options.fontGradient, hasColor);
    }
  }

  {
    ...
    if (!phStr.empty()) {
      bool hasColor = false;
      ParseGradientOrColor(phStr, options.placeholderColor,
                           options.placeholderAlpha,
                           options.placeholderGradient, hasColor);
    }
  }

  {
    ...
    if (!caretStr.empty()) {
      bool hasColor = false;
      ParseGradientOrColor(caretStr, options.caretColor, options.caretAlpha,
                           options.caretGradient, hasColor);
    }
  }

  {
    ...
    if (!selStr.empty()) {
      bool hasColor = false;
      ParseGradientOrColor(selStr, options.selectionColor,
                           options.selectionAlpha, options.selectionGradient,
                           hasColor);
    }
  }

  {
    ...
    if (!fillStr.empty()) {
      ...
      if (lower == L"none" || lower == L"transparent") {
        ...
        bool hasColor = false;
        ParseGradientOrColor(fillStr, options.fillColor, options.fillAlpha,
                             options.fillGradient, hasColor);
  ...
  // JS callbacks
  GetEventCallbackProp(ctx, obj, "onTextChange",
                       options.onTextChangeCallbackId);
...

  {
    ...
    if (!borderStr.empty()) {
      bool hasColor = false;
      ParseGradientOrColor(borderStr, options.borderColor,
                           options.borderColorAlpha, options.borderGradient,
                           hasColor);
    }
  }

  {
    ...
    if (!focusStr.empty()) {
      ...
      if (lower == L"none" || lower == L"transparent") {
        ...
        bool hasColor = false;
        ParseGradientOrColor(focusStr, options.borderFocusColor,
                             options.borderFocusColorAlpha,
                             options.borderFocusGradient, hasColor);
  ...
  GetEventCallbackProp(ctx, obj, "onInvalidInput",
                       options.onInvalidInputCallbackId);
}
...
void ApplyInputBoxOptions(InputBoxElement *element,
  ...
  if (!options.fontPath.empty()) {
    if (PathUtils::IsURL(options.fontPath)) {
      ...
      if (!cachedDir.empty()) {
        ...
        if (fontWidget)
          FontDownloader::RequestAsync(
              options.fontPath, fontWidget->GetInstanceId(), element->GetId());
      }
    } else {
  ...
  if (options.placeholderGradient.type != GRADIENT_NONE)
    ...
    element->SetPlaceholderColor(options.placeholderColor,
                                 options.placeholderAlpha);
  ...
  if (options.borderFocusGradient.type != GRADIENT_NONE)
    ...
    element->SetBorderFocusColor(options.borderFocusColor,
                                 options.borderFocusColorAlpha);
  else
...
void PreFillInputBoxOptions(InputBoxOptions &options,
                            InputBoxElement *element) {
  if (!element)
    return;
