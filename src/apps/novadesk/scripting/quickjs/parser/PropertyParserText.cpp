...
void ParseTextOptions(JSContext *ctx, JSValueConst obj, TextOptions &options,
...

  {
    ...
    if (!JS_IsException(textProp) && !JS_IsUndefined(textProp) &&
        !JS_IsNull(textProp)) {
  ...
  if (!fc.empty()) {
    ParseGradientOrColor(fc, options.fontColor, options.alpha,
                         options.fontGradient, options.hasSolidColor);
  }
  ...
  if (!align.empty()) {
    ...
    else if (align == L"centercenter" || align == L"middlecenter" ||
             align == L"middle" || align == L"center-center" ||
             align == L"middle-center")
...
void ApplyTextOptions(TextElement *element, const TextOptions &options) {
  ...
  element->SetSelectionBackgroundColor(options.selectionBackgroundColor,
                                       options.selectionBackgroundAlpha);
  if (options.hasSelectionTextColor) {
    element->SetSelectionTextColor(options.selectionTextColor,
                                   options.selectionTextAlpha);
  }
}
