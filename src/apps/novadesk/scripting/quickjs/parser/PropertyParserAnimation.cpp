...
static bool HasDisallowedTweenTextProps(JSContext *ctx, JSValueConst obj) {
  static const char *kForbidden[] = {"fontSize", "fontWeight", "letterSpacing",
                                     "fontColor"};
  for (const char *key : kForbidden) {
    JSValue v = JS_GetPropertyStr(ctx, obj, key);
    const bool present =
        !JS_IsException(v) && !JS_IsUndefined(v) && !JS_IsNull(v);
    JS_FreeValue(ctx, v);
    if (present)
      return true;
  }
...
static void ParseAnimationTransformObject(
    JSContext *ctx, JSValueConst obj, bool &hasX, float &x, bool &hasY,
    float &y, bool &hasWidth, float &width, bool &hasHeight, float &height,
    bool &hasRotate, float &rotate, bool &hasXExpr, std::wstring &xExpr,
...
static void ParseAnimationTargetObject(
    JSContext *ctx, JSValueConst obj, bool &hasX, float &x, bool &hasY,
    float &y, bool &hasWidth, float &width, bool &hasHeight, float &height,
    bool &hasRotate, float &rotate, bool &hasFontSize, float &fontSize,
    bool &hasFontWeight, float &fontWeight, bool &hasLetterSpacing,
    float &letterSpacing, bool &hasFontColor, float &fontColorR,
    float &fontColorG, float &fontColorB, float &fontAlpha, bool &hasXExpr,
    std::wstring &xExpr, bool &hasYExpr, std::wstring &yExpr) {
...
static bool ParseKeyframeOffsetValue(JSContext *ctx, JSValueConst val,
                                     float &outOffset) {
...
static void ParseAnimationKeyframeValues(JSContext *ctx, JSValueConst obj,
                                         AnimationKeyframeOptions &kf) {
  ParseAnimationTargetObject(
      ctx, obj, kf.hasX, kf.x, kf.hasY, kf.y, kf.hasWidth, kf.width,
      kf.hasHeight, kf.height, kf.hasRotate, kf.rotate, kf.hasFontSize,
      kf.fontSize, kf.hasFontWeight, kf.fontWeight, kf.hasLetterSpacing,
      kf.letterSpacing, kf.hasFontColor, kf.fontColorR, kf.fontColorG,
      kf.fontColorB, kf.fontAlpha, kf.hasXExpr, kf.xExpr, kf.hasYExpr,
      kf.yExpr);
...
static bool ParseAnimationKeyframeEntry(JSContext *ctx, JSValueConst obj,
                                        float forcedOffset,
                                        bool hasForcedOffset,
                                        AnimationKeyframeOptions &out) {
  if (!JS_IsObject(obj))
    return false;
  ...
  if (hasForcedOffset) {
    ...
    if (JS_IsException(offsetVal) || JS_IsUndefined(offsetVal) ||
        JS_IsNull(offsetVal)) {
...
static void ParseAnimationKeyframesArray(JSContext *ctx, JSValueConst arr,
  ...
  for (uint32_t i = 0; i < len; ++i) {
    ...
    if (!ParseAnimationKeyframeEntry(ctx, item, 0.0f, false, kf)) {
      ...
      options.keyframesError =
          L"each keyframes entry needs offset/at and at least one property";
    } else {
...
static void ParseAnimationKeyframesObject(JSContext *ctx, JSValueConst obj,
                                          AnimationOptions &options) {
  JSPropertyEnum *tab = nullptr;
  uint32_t tabLen = 0;
  if (JS_GetOwnPropertyNames(ctx, &tab, &tabLen, obj,
  ...
  for (uint32_t i = 0; i < tabLen; ++i) {
    ...
    if (!hasOffset) {
      options.keyframesInvalid = true;
      options.keyframesError =
          L"keyframes object keys must be percentages like \"0%\" or \"33%\"";
      continue;
    }
    ...
    if (!ParseAnimationKeyframeEntry(ctx, item, offset, true, kf)) {
      options.keyframesInvalid = true;
      options.keyframesError =
          L"each keyframes entry must include at least one property";
    } else {
      options.keyframes.push_back(kf);
    }
    JS_FreeValue(ctx, item);
...
void ParseAnimationOptions(JSContext *ctx, JSValueConst obj,
                           AnimationOptions &options) {
  ...
  if (!JS_IsException(iterationVal) && !JS_IsUndefined(iterationVal) &&
      !JS_IsNull(iterationVal)) {
    ...
    if (JS_IsString(iterationVal)) {
      ...
      if (iterUtf8) {
        ...
        std::transform(iterStr.begin(), iterStr.end(), iterStr.begin(),
                       ::tolower);
        if (iterStr == "infinite")
  ...
  if (!JS_IsException(keyframesVal) && !JS_IsUndefined(keyframesVal) &&
      !JS_IsNull(keyframesVal)) {
    ...
    if (JS_IsArray(keyframesVal))
      ...
      options.keyframesInvalid = true;
      options.keyframesError =
          L"keyframes must be an array or object with percent keys";
    }
  }
  ...
  if (options.hasKeyframes) {
    ...
    if (!options.keyframesInvalid) {
      if (options.keyframes.size() < 2) {
        ...
        std::sort(options.keyframes.begin(), options.keyframes.end(),
                  [](const AnimationKeyframeOptions &a,
                     const AnimationKeyframeOptions &b) {
        ...
        for (size_t i = 1; i < options.keyframes.size(); ++i) {
          if (options.keyframes[i].offset <= options.keyframes[i - 1].offset) {
            options.keyframesInvalid = true;
            options.keyframesError =
                L"keyframes offsets must be strictly increasing";
            break;
          }
        }
      }
  ...
  if (JS_IsObject(toVal)) {
    if (HasDisallowedTweenTextProps(ctx, toVal)) {
      ...
      options.tweenError = L"from/to support only x, y, width, height, and "
                           L"rotate; use keyframes for text properties";
    } else {
      ParseAnimationTransformObject(
          ctx, toVal, options.hasX, options.x, options.hasY, options.y,
          options.hasWidth, options.width, options.hasHeight, options.height,
          options.hasRotate, options.rotate, options.hasXExpr, options.xExpr,
  ...
  if (JS_IsObject(fromVal)) {
    if (HasDisallowedTweenTextProps(ctx, fromVal)) {
      ...
      ParseAnimationTransformObject(
          ctx, fromVal, options.fromHasX, options.fromX, options.fromHasY,
          options.fromY, options.fromHasWidth, options.fromWidth,
          options.fromHasHeight, options.fromHeight, options.fromHasRotate,
          options.fromRotate, options.fromHasXExpr, options.fromXExpr,
...
static void ParseWindowAnimationTargetObject(
    JSContext *ctx, JSValueConst obj, bool &hasX, float &x, bool &hasY,
    float &y, bool &hasWidth, float &width, bool &hasHeight, float &height,
    bool &hasOpacity, float &opacity, bool &hasBgColor, float &bgR, float &bgG,
    float &bgB, float &bgA, bool &hasXExpr, std::wstring &xExpr, bool &hasYExpr,
    std::wstring &yExpr, bool &hasPosition, std::wstring &position,
  ...
  hasWidth = GetFloatProp(ctx, obj, "width", width);
  if (!hasWidth)
    hasWidth = GetFloatProp(ctx, obj, "w", width);
  hasHeight = GetFloatProp(ctx, obj, "height", height);
  if (!hasHeight)
    hasHeight = GetFloatProp(ctx, obj, "h", height);

  float opVal = 1.0f;
  if (GetFloatProp(ctx, obj, "opacity", opVal) ||
      GetFloatProp(ctx, obj, "alpha", opVal)) {
    hasOpacity = true;
    if (opVal > 1.0f)
      opVal = opVal / 255.0f;
    if (opVal < 0.0f)
  ...
  if (bgStr.empty())
    bgStr = GetStringProp(ctx, obj, "bgColor");
  if (!bgStr.empty()) {
    COLORREF c = 0;
    BYTE a = 255;
...
static void
ParseWindowAnimationKeyframeValues(JSContext *ctx, JSValueConst obj,
                                   WindowAnimationKeyframeOptions &kf) {
  ParseWindowAnimationTargetObject(
      ctx, obj, kf.hasX, kf.x, kf.hasY, kf.y, kf.hasWidth, kf.width,
      kf.hasHeight, kf.height, kf.hasOpacity, kf.opacity, kf.hasBackgroundColor,
      kf.bgColorR, kf.bgColorG, kf.bgColorB, kf.bgAlpha, kf.hasXExpr, kf.xExpr,
      kf.hasYExpr, kf.yExpr, kf.hasPosition, kf.position, kf.offsetX,
      kf.offsetY);
...
static bool
ParseWindowAnimationKeyframeEntry(JSContext *ctx, JSValueConst obj,
                                  float forcedOffset, bool hasForcedOffset,
                                  WindowAnimationKeyframeOptions &out) {
  if (!JS_IsObject(obj))
    return false;
...
static void
ParseWindowAnimationKeyframesArray(JSContext *ctx, JSValueConst arr,
  ...
  for (uint32_t i = 0; i < len; ++i) {
    ...
    if (!ParseWindowAnimationKeyframeEntry(ctx, item, 0.0f, false, kf)) {
      options.keyframesInvalid = true;
      options.keyframesError = L"each keyframes entry needs offset/at and at "
                               L"least one window property";
    } else {
      options.keyframes.push_back(kf);
    }
    JS_FreeValue(ctx, item);
...
static void
ParseWindowAnimationKeyframesObject(JSContext *ctx, JSValueConst obj,
                                    WindowAnimationOptions &options) {
  JSPropertyEnum *tab = nullptr;
  uint32_t tabLen = 0;
  if (JS_GetOwnPropertyNames(ctx, &tab, &tabLen, obj,
  ...
  for (uint32_t i = 0; i < tabLen; ++i) {
    ...
    if (!hasOffset) {
      options.keyframesInvalid = true;
      options.keyframesError =
          L"keyframes object keys must be percentages like \"0%\" or \"50%\"";
      continue;
    }
...
void ParseWindowAnimationOptions(JSContext *ctx, JSValueConst obj,
                                 WindowAnimationOptions &options) {
  GetIntProp(ctx, obj, "duration", options.duration);
  if (options.duration <= 0)
    options.duration = 250;
  ...
  if (options.hasKeyframes) {
    ...
    if (!options.keyframesInvalid) {
      if (options.keyframes.size() < 2) {
        ...
        std::sort(options.keyframes.begin(), options.keyframes.end(),
                  [](const WindowAnimationKeyframeOptions &a,
                     const WindowAnimationKeyframeOptions &b) {
                    return a.offset < b.offset;
                  });
        for (size_t i = 1; i < options.keyframes.size(); ++i) {
          if (options.keyframes[i].offset <= options.keyframes[i - 1].offset) {
  ...
  if (JS_IsObject(toVal)) {
    ParseWindowAnimationTargetObject(
        ctx, toVal, options.hasX, options.x, options.hasY, options.y,
        options.hasWidth, options.width, options.hasHeight, options.height,
        options.hasOpacity, options.opacity, options.hasBackgroundColor,
        options.bgColorR, options.bgColorG, options.bgColorB, options.bgAlpha,
        options.hasXExpr, options.xExpr, options.hasYExpr, options.yExpr,
        options.hasPosition, options.position, options.offsetX,
        options.offsetY);
  }
  JS_FreeValue(ctx, toVal);
  ...
  if (JS_IsObject(fromVal)) {
    ParseWindowAnimationTargetObject(
        ctx, fromVal, options.fromHasX, options.fromX, options.fromHasY,
        options.fromY, options.fromHasWidth, options.fromWidth,
        options.fromHasHeight, options.fromHeight, options.fromHasOpacity,
        options.fromOpacity, options.fromHasBackgroundColor,
        options.fromBgColorR, options.fromBgColorG, options.fromBgColorB,
        options.fromBgAlpha, options.fromHasXExpr, options.fromXExpr,
