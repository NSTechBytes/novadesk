...
using namespace Js;
void ParseAreaGraphOptions(JSContext *ctx, JSValueConst obj,
                           AreaGraphOptions &options,
...
void ParseBarOptions(JSContext *ctx, JSValueConst obj, BarOptions &options,
                     const std::wstring &baseDir) {
  ParseElementOptions(ctx, obj, options, baseDir);
  ...
  ParseGradientOrColor(bc, options.barColor, options.barAlpha,
                       options.barGradient, options.hasBarColor);
}

void ParseLineOptions(JSContext *ctx, JSValueConst obj, LineOptions &options,
                      const std::wstring &baseDir) {
  ParseElementOptions(ctx, obj, options, baseDir);
  ...
  std::wstring horizontalLineColor =
      GetStringProp(ctx, obj, "horizontalLineColor");
  if (!horizontalLineColor.empty()) {
    ...
    } else if (ColorUtil::ParseRGBA(horizontalLineColor,
                                    options.horizontalLineColor,
                                    options.horizontalLineAlpha)) {
      options.horizontalLineGradient = GradientInfo();
    }
  }

  std::wstring graphStart = GetStringProp(ctx, obj, "graphStart");
  std::transform(graphStart.begin(), graphStart.end(), graphStart.begin(),
                 ::towlower);
  ...
  std::transform(graphOrientation.begin(), graphOrientation.end(),
                 graphOrientation.begin(), ::towlower);
  ...
  std::transform(transformStroke.begin(), transformStroke.end(),
                 transformStroke.begin(), ::towlower);
  ...
  for (int i = 0; i < options.lineCount; ++i) {
    std::wstring dataKey =
        (i == 0) ? L"data" : (L"data" + std::to_wstring(i + 1));
    ...
    std::wstring colorKey =
        (i == 0) ? L"lineColor" : (L"lineColor" + std::to_wstring(i + 1));
    ...
    if (!colorValue.empty()) {
      ...
      } else if (ColorUtil::ParseRGBA(colorValue, options.lineColors[(size_t)i],
                                      options.lineAlphas[(size_t)i])) {
        options.lineGradients[(size_t)i] = GradientInfo();
      }
    }

    std::wstring scaleKey =
        (i == 0) ? L"lineScale" : (L"lineScale" + std::to_wstring(i + 1));
...
void ParseHistogramOptions(JSContext *ctx, JSValueConst obj,
  ...
  if (!primaryColor.empty()) {
    ...
    } else if (ColorUtil::ParseRGBA(primaryColor, options.primaryColor,
                                    options.primaryAlpha)) {
  ...
  if (!secondaryColor.empty()) {
    ...
    } else if (ColorUtil::ParseRGBA(secondaryColor, options.secondaryColor,
                                    options.secondaryAlpha)) {
  ...
  if (!bothColor.empty()) {
    ...
    } else if (ColorUtil::ParseRGBA(bothColor, options.bothColor,
                                    options.bothAlpha)) {
...
void ParseRoundLineOptions(JSContext *ctx, JSValueConst obj,
                           RoundLineOptions &options,
                           const std::wstring &baseDir) {
  ParseElementOptions(ctx, obj, options, baseDir);
  ...
  std::wstring lc = GetStringProp(ctx, obj, "lineColor");
  ParseGradientOrColor(lc, options.lineColor, options.lineAlpha,
                       options.lineGradient, options.hasLineColor);

  std::wstring lcb = GetStringProp(ctx, obj, "lineColorBg");
  ParseGradientOrColor(lcb, options.lineColorBg, options.lineAlphaBg,
                       options.lineGradientBg, options.hasLineColorBg);
}
void ApplyAreaGraphOptions(AreaGraphElement *element,
                           const AreaGraphOptions &options) {
  if (!element)
    return;
...
void ApplyLineOptions(LineElement *element, const LineOptions &options) {
  ...
  element->SetHorizontalLineColor(options.horizontalLineColor,
                                  options.horizontalLineAlpha);
...
void ApplyHistogramOptions(HistogramElement *element,
...
void ApplyRoundLineOptions(RoundLineElement *element,
                           const RoundLineOptions &options) {
  if (!element)
    return;
  ApplyElementOptions(element, options);
  element->SetValue(options.value);
...
void PreFillAreaGraphOptions(AreaGraphOptions &options,
                             AreaGraphElement *element) {
  if (!element)
    return;
...
void PreFillHistogramOptions(HistogramOptions &options,
...
void PreFillRoundLineOptions(RoundLineOptions &options,
