...
bool ParseBoolProp(JSContext *ctx, JSValueConst obj, const char *key,
                   bool def) {
...
void ParseColorPickerOptions(JSContext *ctx, JSValueConst obj,
  ...
  ParseColorAlpha(ctx, obj, "popupAccentColor", o.popupAccentColor,
                  o.popupAccentAlpha);
  ...
  if (inBg.empty())
    inBg = Js::GetStringProp(ctx, obj, "popupInputBgColor");
  if (!inBg.empty()) {
    ColorUtil::ParseRGBA(inBg, o.popupInputBackground,
                         o.popupInputBackgroundAlpha);
    o.hasPopupInputBackground = true;
  }

  std::wstring inColor = Js::GetStringProp(ctx, obj, "popupInputColor");
  if (inColor.empty())
    inColor = Js::GetStringProp(ctx, obj, "popupInputTextColor");
  if (!inColor.empty()) {
  ...
  Js::GetEventCallbackProp(ctx, obj, "onEyedropperOpen",
                           o.onEyedropperOpenCallbackId);
  Js::GetEventCallbackProp(ctx, obj, "onEyedropperPick",
                           o.onEyedropperPickCallbackId);
}

// ─────────────────────────────────────────────────────────────────────────────
void ApplyColorPickerOptions(ColorPickerElement *e,
                             const ColorPickerOptions &o) {
