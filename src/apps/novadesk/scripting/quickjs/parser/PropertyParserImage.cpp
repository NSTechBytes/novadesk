...
void ParseGeneralImageOptions(JSContext *ctx, JSValueConst obj,
  ...
  if (!options.fallbackPath.empty()) {
    if (!PathUtils::IsURL(options.fallbackPath)) {
      options.fallbackPath =
          PathUtils::ResolvePath(options.fallbackPath, baseDir);
    }
  }
  ...
  if (!tint.empty() &&
      ColorUtil::ParseRGBA(tint, options.imageTint, options.imageTintAlpha)) {
...
void ParseImageOptions(JSContext *ctx, JSValueConst obj, ImageOptions &options,
  ...
  if (!options.path.empty()) {
    ...
    if (!PathUtils::IsURL(options.path)) {
      ...
      std::error_code ec;
      const bool exists =
          std::filesystem::exists(std::filesystem::path(options.path), ec);
    }
  }
...
void ParseButtonOptions(JSContext *ctx, JSValueConst obj,
                        ButtonOptions &options, const std::wstring &baseDir) {
  ParseElementOptions(ctx, obj, options, baseDir);
  ParseGeneralImageOptions(ctx, obj, options, baseDir);
  ...
  if (!buttonImageName.empty()) {
    ...
    if (!PathUtils::IsURL(buttonImageName)) {
      options.buttonImageName =
          PathUtils::ResolvePath(buttonImageName, baseDir);
    } else {
      options.buttonImageName = buttonImageName;
    }
  }

  GetEventCallbackProp(ctx, obj, "buttonAction",
                       options.onLeftMouseUpCallbackId);
}

void ParseBitmapOptions(JSContext *ctx, JSValueConst obj,
                        BitmapOptions &options, const std::wstring &baseDir) {
  ParseElementOptions(ctx, obj, options, baseDir);
  ParseGeneralImageOptions(ctx, obj, options, baseDir);
  ...
  if (!bitmapImageName.empty()) {
    ...
    if (!PathUtils::IsURL(bitmapImageName)) {
      options.bitmapImageName =
          PathUtils::ResolvePath(bitmapImageName, baseDir);
    } else {
  ...
  {
    float tmp = static_cast<float>(options.minValue);
    if (GetFloatProp(ctx, obj, "minValue", tmp))
      options.minValue = static_cast<double>(tmp);
  }
  {
    float tmp = static_cast<float>(options.maxValue);
    if (GetFloatProp(ctx, obj, "maxValue", tmp))
      options.maxValue = static_cast<double>(tmp);
  }
...
void ParseRotatorOptions(JSContext *ctx, JSValueConst obj,
                         RotatorOptions &options, const std::wstring &baseDir) {
  ParseElementOptions(ctx, obj, options, baseDir);
  ParseGeneralImageOptions(ctx, obj, options, baseDir);
  ...
  if (!rotatorImageName.empty()) {
    ...
    if (!PathUtils::IsURL(rotatorImageName)) {
      options.rotatorImageName =
          PathUtils::ResolvePath(rotatorImageName, baseDir);
    } else {
  ...
  {
    float tmp = static_cast<float>(options.offsetX);
    if (GetFloatProp(ctx, obj, "offsetX", tmp))
      options.offsetX = static_cast<double>(tmp);
  }
  {
    float tmp = static_cast<float>(options.offsetY);
    if (GetFloatProp(ctx, obj, "offsetY", tmp))
      options.offsetY = static_cast<double>(tmp);
  }
  {
    float tmp = static_cast<float>(options.startAngle);
    if (GetFloatProp(ctx, obj, "startAngle", tmp))
      options.startAngle = static_cast<double>(tmp);
  }
  {
    float tmp = static_cast<float>(options.rotationAngle);
    if (GetFloatProp(ctx, obj, "rotationAngle", tmp))
      options.rotationAngle = static_cast<double>(tmp);
  }
...
void ApplyGeneralImageOptions(GeneralImage *image,
  ...
  if (options.hasImageCrop)
    image->SetImageCrop(options.imageCropX, options.imageCropY,
                        options.imageCropW, options.imageCropH,
                        options.imageCropOrigin);
...
void ApplyImageOptions(ImageElement *element, const ImageOptions &options) {
  ...
  if (options.hasScaleMargins)
    element->SetScaleMargins(options.scaleMarginLeft, options.scaleMarginTop,
                             options.scaleMarginRight,
                             options.scaleMarginBottom);
