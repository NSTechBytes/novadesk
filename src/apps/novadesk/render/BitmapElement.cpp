...
BitmapElement::BitmapElement(const std::wstring &id, int x, int y,
...
void BitmapElement::SetValue(double value) { m_Value = value; }
...
bool BitmapElement::ComputeFrameGeometry(float imageWidth, float imageHeight,
                                         bool &verticalFrames,
                                         float &frameWidth,
...
int BitmapElement::GetDigitCountForValue(long long value,
                                         int realFrames) const {
  if (m_Digits > 0)
    return m_Digits;

  const long long baseValue =
      (realFrames == 1) ? 2LL : static_cast<long long>(realFrames);
...
void BitmapElement::Render(ID2D1DeviceContext *context) {
  ...
  if (!ComputeFrameGeometry(imageSize.width, imageSize.height, verticalFrames,
  ...
  if (m_Extend) {
    ...
    const float totalWidth =
        frameWidth * static_cast<float>(digitCount) +
    ...
    for (int i = 0; i < digitCount; ++i) {
      ...
      const D2D1_RECT_F srcRect =
          D2D1::RectF(srcX, srcY, srcX + frameWidth, srcY + frameHeight);
      const float digitX =
          startX +
          static_cast<float>(i) * (frameWidth + static_cast<float>(spacing));
      const D2D1_RECT_F dstRect =
          D2D1::RectF(digitX, static_cast<float>(contentY), digitX + frameWidth,
      ...
      if (finalImage.Get() == static_cast<ID2D1Image *>(bitmap)) {
        ...
        context->SetTransform(
            ...
            effectTransform);
        context->DrawImage(finalImage.Get(), nullptr, &srcRect,
                           m_AntiAlias
    ...
    double normalizedValue =
        (range > 0.0) ? (m_Value - m_MinValue) / range : 0.0;
    ...
    const D2D1_RECT_F srcRect =
        D2D1::RectF(srcX, srcY, srcX + frameWidth, srcY + frameHeight);
    const D2D1_RECT_F dstRect =
        D2D1::RectF(static_cast<float>(contentX), static_cast<float>(contentY),
...
void BitmapElement::OnImageDownloaded(const std::wstring &url,
