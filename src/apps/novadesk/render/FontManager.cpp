...
public:
  DirectoryFontFileEnumerator(IDWriteFactory *factory,
                              const std::wstring &directoryPath)
      : m_RefCount(1), m_pFactory(factory), m_DirectoryPath(directoryPath),
        m_CurrentIndex(-1) {
    ...
    try {
      for (const auto &entry :
        ...
        if (entry.is_regular_file()) {
          ...
          if (ext == L".ttf" || ext == L".otf" || ext == L".ttc") {
            Logging::Log(LogLevel::Debug, L"FontManager: Found font file: %s",
      ...
      Logging::Log(
          LogLevel::Info,
          L"FontManager: Finished scanning '%s'. Found %zu font files.",
          directoryPath.c_str(), m_FilePaths.size());
    } catch (const std::exception &e) {
      Logging::Log(LogLevel::Error,
                   L"FontManager: Error scanning directory '%s': %S",
  ...
  virtual HRESULT STDMETHODCALLTYPE MoveNext(BOOL *hasCurrentFile) override {
    ...
    if (m_CurrentIndex < (int)m_FilePaths.size()) {
      *hasCurrentFile = TRUE;
      ...
      return m_pFactory->CreateFontFileReference(
          m_FilePaths[m_CurrentIndex].c_str(), nullptr, &m_pCurrentFile);
    }
  ...
  GetCurrentFontFile(IDWriteFontFile **fontFile) override {
    if (m_pCurrentFile) {
    ...
    if (!buffer.empty()) {
      ...
      if (SUCCEEDED(hr) && g_pInMemoryLoader) {
        ...
        hr = g_pInMemoryLoader->CreateInMemoryFontFileReference(
            factory, buffer.data(), static_cast<UINT32>(buffer.size()), nullptr,
            &fontFile);
        if (SUCCEEDED(hr)) {
          m_FontFile = fontFile;
          Logging::Log(LogLevel::Info,
                       L"FontManager: Successfully created in-memory font file "
                       L"reference for '%s'",
                       url.c_str());
        } else {
          Logging::Log(LogLevel::Error,
                       L"FontManager: CreateInMemoryFontFileReference failed "
                       L"(0x%08X) for '%s'",
                       hr, url.c_str());
        }
      } else {
        Logging::Log(LogLevel::Error,
                     L"FontManager: IDWriteFactory5 or g_pInMemoryLoader not "
                     L"available for in-memory font '%s'",
                     url.c_str());
      }
    } else {
      Logging::Log(LogLevel::Error,
                   L"FontManager: InMemory data not found for URL '%s'",
                   url.c_str());
    }
  }
...
public:
  ...
  virtual HRESULT STDMETHODCALLTYPE CreateEnumeratorFromKey(
    ...
    std::wstring keyPath((const wchar_t *)collectionKey,
                         collectionKeySize / sizeof(wchar_t));
    // Remove ANY trailing nulls or junk
    while (!keyPath.empty() &&
           (keyPath.back() == L'\0' || keyPath.back() == L' ' ||
            keyPath.back() == L'\r' || keyPath.back() == L'\n')) {
      keyPath.pop_back();
    }

    if (keyPath.rfind(L"http://", 0) == 0 ||
        keyPath.rfind(L"https://", 0) == 0) {
      Logging::Log(LogLevel::Debug,
                   L"FontManager: Enumerating in-memory file for URL: '%s'",
                   keyPath.c_str());
      *fontFileEnumerator = new InMemoryFontFileEnumerator(factory, keyPath);
    } else {
      Logging::Log(LogLevel::Debug,
                   L"FontManager: Enumerating files for directory key: '%s'",
                   keyPath.c_str());
...
bool Initialize() {
  ...
  if (FAILED(hr)) {
    Logging::Log(
        LogLevel::Error,
        L"FontManager: Failed to register font collection loader (0x%08X)", hr);
    return false;
  }
  ...
  if (SUCCEEDED(writeFactory->QueryInterface(IID_PPV_ARGS(&factory5)))) {
    ...
    if (SUCCEEDED(hr)) {
      ...
      if (FAILED(hr)) {
        Logging::Log(
            LogLevel::Error,
            L"FontManager: Failed to register memory font file loader (0x%08X)",
            hr);
      ...
      Logging::Log(
          LogLevel::Error,
          L"FontManager: Failed to create memory font file loader (0x%08X)",
          hr);
    }
  } else {
    Logging::Log(LogLevel::Warn,
                 L"FontManager: IDWriteFactory5 not supported on this Windows "
                 L"version. Online fonts will not function.");
  }
...
GetFontCollection(const std::wstring &directoryPath) {
  ...
  if (directoryPath.rfind(L"http://", 0) != 0 &&
      directoryPath.rfind(L"https://", 0) != 0) {
  ...
  HRESULT hr = writeFactory->CreateCustomFontCollection(
      g_pLoader.Get(), key.c_str(), (UINT32)(key.length() * sizeof(wchar_t)),
      &pCollection);
  ...
  if (SUCCEEDED(hr)) {
    ...
    Logging::Log(
        LogLevel::Error,
        L"FontManager: Failed to create font collection for '%s' (0x%08X)",
        key.c_str(), hr);
    return nullptr;
  }
}
...
void AddMemoryFont(const std::wstring &url, const std::string &data) {
  ...
  Logging::Log(LogLevel::Info,
               L"FontManager: Registered memory font for '%s' (%zu bytes)",
               url.c_str(), g_MemoryFonts[url].size());
}
