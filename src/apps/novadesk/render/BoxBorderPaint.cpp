...
D2D1_ROUNDED_RECT
BoxBorderPaint::BuildBorderGeometryRect(const D2D1_ROUNDED_RECT &elementRect,
...
void BoxBorderPaint::PaintForElement(ID2D1DeviceContext *context,
  ...
  const D2D1_ROUNDED_RECT borderRect =
      BuildBorderGeometryRect(elementRect, params);
  if (params.position == BoxBorder::Position::Inside) {
    D2D1_LAYER_PARAMETERS1 clipParams = D2D1::LayerParameters1(
        elementRect.rect, nullptr, D2D1_ANTIALIAS_MODE_PER_PRIMITIVE,
...
void BoxBorderPaint::Paint(ID2D1DeviceContext *context,
                           const D2D1_ROUNDED_RECT &rect,
                           const BoxBorderPaintParams &params,
                           ID2D1Brush *strokeBrush) {
  if (params.styleTop == BoxBorder::Style::None &&
      params.styleRight == BoxBorder::Style::None &&
      params.styleBottom == BoxBorder::Style::None &&
      params.styleLeft == BoxBorder::Style::None)
    return;
  if (params.styleTop == BoxBorder::Style::Hidden &&
      params.styleRight == BoxBorder::Style::Hidden &&
      params.styleBottom == BoxBorder::Style::Hidden &&
      params.styleLeft == BoxBorder::Style::Hidden)
    return;
  ...
  auto paintRectangularDashedBorder = [&]() {
    ...
    auto buildSegments = [&](float length) {
      ...
      const int dashCount = std::max(
          1, static_cast<int>(std::round((length + w) / targetPeriod)));
    ...
    auto paintSegments = [&]() {
      for (const auto &segment : buildSegments(R - L)) {
        context->FillRectangle(
            D2D1::RectF(L + segment.start, T, L + segment.end, T + w),
            strokeBrush);
        context->FillRectangle(
            D2D1::RectF(L + segment.start, B - w, L + segment.end, B),
            strokeBrush);
      }
      for (const auto &segment : buildSegments(B - T)) {
        context->FillRectangle(
            D2D1::RectF(L, T + segment.start, L + w, T + segment.end),
            strokeBrush);
        context->FillRectangle(
            D2D1::RectF(R - w, T + segment.start, R, T + segment.end),
            strokeBrush);
      }
    };
    ...
    if (!factory ||
        FAILED(factory->CreateRoundedRectangleGeometry(rect, &clipGeometry))) {
      paintSegments();
      return;
    }

    D2D1_LAYER_PARAMETERS1 layerParams = D2D1::LayerParameters1(
        rect.rect, clipGeometry.Get(), D2D1_ANTIALIAS_MODE_PER_PRIMITIVE,
        D2D1::Matrix3x2F::Identity(), 1.0f, nullptr, D2D1_LAYER_OPTIONS1_NONE);
    context->PushLayer(layerParams, nullptr);
    paintSegments();
    context->PopLayer();
  };
  if (sameStyle && params.styleTop == BoxBorder::Style::Dashed &&
      rect.radiusX <= params.strokeWidth &&
      rect.radiusY <= params.strokeWidth) {
    paintRectangularDashedBorder();
    return;
  }
  auto getStrokeStyle = [&](BoxBorder::Style style,
                            ID2D1StrokeStyle1 **outStyle) {
    *outStyle = nullptr;
    ID2D1Factory1 *factory = Direct2D::GetFactory();
    if (!factory)
      return;
  ...
  auto insetRoundedRect = [](D2D1_ROUNDED_RECT rr,
                             float amount) -> D2D1_ROUNDED_RECT {
    rr.rect.left += amount;
    rr.rect.top += amount;
  ...
  auto strokePathForBorder = [&](const D2D1_ROUNDED_RECT &outer,
                                 float strokeWidth) -> D2D1_ROUNDED_RECT {
    // `outer` is the outer edge of the border band; stroke is centered half a
    // width inward.
    return insetRoundedRect(outer, params.strokeWidth * 0.5f);
  };
  auto drawStyleRect = [&](const D2D1_ROUNDED_RECT &r, BoxBorder::Style style,
    ...
    if (style == BoxBorder::Style::Double) {
    ...
    } else if (style == BoxBorder::Style::Groove ||
               style == BoxBorder::Style::Ridge ||
               style == BoxBorder::Style::Inset ||
               style == BoxBorder::Style::Outset) {
      COLORREF baseColor = params.strokeColor;
      BYTE rC = GetRValue(baseColor), gC = GetGValue(baseColor),
           bC = GetBValue(baseColor);
      COLORREF lightC = RGB(std::min(255, rC + 50), std::min(255, gC + 50),
                            std::min(255, bC + 50));
      COLORREF darkC =
          RGB(std::max(0, rC - 50), std::max(0, gC - 50), std::max(0, bC - 50));
      Microsoft::WRL::ComPtr<ID2D1SolidColorBrush> lightBrush, darkBrush;
      context->CreateSolidColorBrush(
          Direct2D::ColorToD2D(lightC, params.strokeAlpha / 255.0f),
          &lightBrush);
      context->CreateSolidColorBrush(
          Direct2D::ColorToD2D(darkC, params.strokeAlpha / 255.0f), &darkBrush);
      ...
      if (params.elementRadiusX == 0.0f && params.elementRadiusY == 0.0f) {
        ...
        float w = width;
        auto drawPolygon = [&](const D2D1_POINT_2F *points, int count,
                               ID2D1Brush *pBrush) {
          if (!pBrush)
            return;
        ...
        if (style == BoxBorder::Style::Inset ||
            style == BoxBorder::Style::Outset) {
          ...
          D2D1_POINT_2F topPts[] = {D2D1::Point2F(L, T), D2D1::Point2F(R, T),
                                    D2D1::Point2F(R - w, T + w),
                                    D2D1::Point2F(L + w, T + w)};
          drawPolygon(topPts, 4, tbBrush);
          D2D1_POINT_2F leftPts[] = {
              D2D1::Point2F(L, T), D2D1::Point2F(L + w, T + w),
              D2D1::Point2F(L + w, B - w), D2D1::Point2F(L, B)};
          drawPolygon(leftPts, 4, lrBrush);
          D2D1_POINT_2F bottomPts[] = {
              D2D1::Point2F(L + w, B - w), D2D1::Point2F(R - w, B - w),
              D2D1::Point2F(R, B), D2D1::Point2F(L, B)};
          drawPolygon(bottomPts, 4, bbBrush);
          D2D1_POINT_2F rightPts[] = {D2D1::Point2F(R - w, T + w),
                                      D2D1::Point2F(R, T), D2D1::Point2F(R, B),
                                      D2D1::Point2F(R - w, B - w)};
          drawPolygon(rightPts, 4, rrBrush);
        } else if (style == BoxBorder::Style::Groove ||
                   style == BoxBorder::Style::Ridge) {
          float h = w * 0.5f;
          ID2D1Brush *outerTopLeftBrush = (style == BoxBorder::Style::Groove)
                                              ? darkBrush.Get()
                                              : lightBrush.Get();
          ID2D1Brush *innerTopLeftBrush = (style == BoxBorder::Style::Groove)
                                              ? lightBrush.Get()
                                              : darkBrush.Get();
          ID2D1Brush *outerBotRightBrush = (style == BoxBorder::Style::Groove)
                                               ? lightBrush.Get()
                                               : darkBrush.Get();
          ID2D1Brush *innerBotRightBrush = (style == BoxBorder::Style::Groove)
                                               ? darkBrush.Get()
                                               : lightBrush.Get();
          D2D1_POINT_2F topOuterPts[] = {
              D2D1::Point2F(L, T), D2D1::Point2F(R, T),
              D2D1::Point2F(R - h, T + h), D2D1::Point2F(L + h, T + h)};
          drawPolygon(topOuterPts, 4, outerTopLeftBrush);
          D2D1_POINT_2F topInnerPts[] = {
              D2D1::Point2F(L + h, T + h), D2D1::Point2F(R - h, T + h),
              D2D1::Point2F(R - w, T + w), D2D1::Point2F(L + w, T + w)};
          drawPolygon(topInnerPts, 4, innerTopLeftBrush);
          D2D1_POINT_2F leftOuterPts[] = {
              D2D1::Point2F(L, T), D2D1::Point2F(L + h, T + h),
              D2D1::Point2F(L + h, B - h), D2D1::Point2F(L, B)};
          drawPolygon(leftOuterPts, 4, outerTopLeftBrush);
          D2D1_POINT_2F leftInnerPts[] = {
              D2D1::Point2F(L + h, T + h), D2D1::Point2F(L + w, T + w),
              D2D1::Point2F(L + w, B - w), D2D1::Point2F(L + h, B - h)};
          drawPolygon(leftInnerPts, 4, innerTopLeftBrush);
          D2D1_POINT_2F bottomOuterPts[] = {
              D2D1::Point2F(L + h, B - h), D2D1::Point2F(R - h, B - h),
              D2D1::Point2F(R, B), D2D1::Point2F(L, B)};
          drawPolygon(bottomOuterPts, 4, outerBotRightBrush);
          D2D1_POINT_2F bottomInnerPts[] = {
              D2D1::Point2F(L + w, B - w), D2D1::Point2F(R - w, B - w),
              D2D1::Point2F(R - h, B - h), D2D1::Point2F(L + h, B - h)};
          drawPolygon(bottomInnerPts, 4, innerBotRightBrush);
          D2D1_POINT_2F rightOuterPts[] = {
              D2D1::Point2F(R - h, T + h), D2D1::Point2F(R, T),
              D2D1::Point2F(R, B), D2D1::Point2F(R - h, B - h)};
          drawPolygon(rightOuterPts, 4, outerBotRightBrush);
          D2D1_POINT_2F rightInnerPts[] = {
              D2D1::Point2F(R - w, T + w), D2D1::Point2F(R - h, T + h),
              D2D1::Point2F(R - h, B - h), D2D1::Point2F(R - w, B - w)};
        ...
        if (d2dFactory) {
          ...
          if (len > 0.001f) {
            ...
            // extent only worked for small boxes; scale with the diagonal
            // length.
          ...
          Microsoft::WRL::ComPtr<ID2D1PathGeometry> tlClip;
          {
            ...
            if (SUCCEEDED(d2dFactory->CreatePathGeometry(&tlClip)) &&
              ...
              sink->AddLine(
                  D2D1::Point2F(pTR_ext.x + tlPerpX, pTR_ext.y + tlPerpY));
              sink->AddLine(
                  D2D1::Point2F(pBL_ext.x + tlPerpX, pBL_ext.y + tlPerpY));
              sink->EndFigure(D2D1_FIGURE_END_CLOSED);
              sink->Close();
            }
          }
          Microsoft::WRL::ComPtr<ID2D1PathGeometry> brClip;
          {
            ...
            if (SUCCEEDED(d2dFactory->CreatePathGeometry(&brClip)) &&
              ...
              sink->AddLine(pTR_ext);
              sink->AddLine(
                  D2D1::Point2F(pTR_ext.x + brPerpX, pTR_ext.y + brPerpY));
              sink->AddLine(
                  D2D1::Point2F(pBL_ext.x + brPerpX, pBL_ext.y + brPerpY));
              sink->EndFigure(D2D1_FIGURE_END_CLOSED);
              sink->Close();
            }
          }
          auto drawSplitRing = [&](const D2D1_ROUNDED_RECT &rr, float sw,
            ...
            if (tlClip && tlBrush) {
              auto lp =
                  D2D1::LayerParameters1(D2D1::InfiniteRect(), tlClip.Get());
            ...
            if (brClip && brBrush) {
              auto lp =
                  D2D1::LayerParameters1(D2D1::InfiniteRect(), brClip.Get());
              context->PushLayer(lp, nullptr);
          ...
          if (style == BoxBorder::Style::Inset ||
            ...
            ID2D1Brush *outerTl = (style == BoxBorder::Style::Groove)
                                      ? darkBrush.Get()
                                      : lightBrush.Get();
            ID2D1Brush *outerBr = (style == BoxBorder::Style::Groove)
                                      ? lightBrush.Get()
                                      : darkBrush.Get();
            ID2D1Brush *innerTl = (style == BoxBorder::Style::Groove)
                                      ? lightBrush.Get()
                                      : darkBrush.Get();
            ID2D1Brush *innerBr = (style == BoxBorder::Style::Groove)
                                      ? darkBrush.Get()
                                      : lightBrush.Get();
      ...
      if (perimeter > 0.001f) {
        ...
        const float segStarts[] = {
            -halfArc, L_top + halfArc, L_top + L_arc + L_right + halfArc,
        ...
        for (int s = 0; s < 4; ++s) {
          ...
          if (segLen <= 0.001f)
            continue;
          const int n =
              std::max(1, static_cast<int>(segLen / ideal_spacing + 0.5f));
          const int iStart = (s == 0) ? 0 : 1;
          const int iEnd = (s == 3) ? n - 1 : n;
          for (int i = iStart; i <= iEnd; ++i) {
            const float t =
                tStart + segLen * static_cast<float>(i) / static_cast<float>(n);
            float x = 0.0f, y = 0.0f;
            getPoint(t, x, y);
            context->FillEllipse(
                D2D1::Ellipse(D2D1::Point2F(x, y), r_dot, r_dot), brush);
          }
        }
      }
    } else {
  ...
  const bool usePerSideBandPaint =
      !sameStyle || (params.styleTop == BoxBorder::Style::Dashed &&
                     rect.radiusX <= params.strokeWidth &&
                     rect.radiusY <= params.strokeWidth);
  if (!usePerSideBandPaint) {
    ...
    const float rx =
        std::min(rect.radiusX, (rect.rect.right - rect.rect.left) * 0.5f);
    const float ry =
        std::min(rect.radiusY, (rect.rect.bottom - rect.rect.top) * 0.5f);
    ...
    const float L = hasRadius ? rect.rect.left
                              : (crispSnap ? (std::floor(rect.rect.left) + 0.5f)
                                           : rect.rect.left);
    const float T = hasRadius ? rect.rect.top
                              : (crispSnap ? (std::floor(rect.rect.top) + 0.5f)
                                           : rect.rect.top);
    const float R = hasRadius
                        ? rect.rect.right
                        : (crispSnap ? (std::floor(rect.rect.right) + 0.5f)
                                     : rect.rect.right);
    const float B = hasRadius
                        ? rect.rect.bottom
                        : (crispSnap ? (std::floor(rect.rect.bottom) + 0.5f)
                                     : rect.rect.bottom);
    if (w <= 0.0f || R <= L || B <= T)
      return;
    ID2D1Factory1 *factory = Direct2D::GetFactory();
    if (!factory)
      return;
    auto makePolyClip =
        [&](const D2D1_POINT_2F *pts) -> Microsoft::WRL::ComPtr<ID2D1Geometry> {
    ...
    const float outerPad =
        (params.position == BoxBorder::Position::Outside)  ? eps
        : (params.position == BoxBorder::Position::Center) ? aaPad
                                                           : 0.0f;
    const float innerPad =
        (params.position == BoxBorder::Position::Inside)   ? 0.0f
        : (params.position == BoxBorder::Position::Center) ? aaPad
                                                           : eps;
    D2D1_POINT_2F topPts[4] = {
        D2D1::Point2F(L - outerPad, T - outerPad),
        D2D1::Point2F(R + outerPad, T - outerPad),
        D2D1::Point2F(R - w + innerPad, T + w + innerPad),
        D2D1::Point2F(L + w - innerPad, T + w + innerPad)};
    D2D1_POINT_2F rightPts[4] = {
        D2D1::Point2F(R + outerPad, T - outerPad),
        D2D1::Point2F(R + outerPad, B + outerPad),
        D2D1::Point2F(R - w - innerPad, B - w + innerPad),
        D2D1::Point2F(R - w - innerPad, T + w - innerPad)};
    D2D1_POINT_2F bottomPts[4] = {
        D2D1::Point2F(L - outerPad, B + outerPad),
        D2D1::Point2F(R + outerPad, B + outerPad),
        D2D1::Point2F(R - w + innerPad, B - w - innerPad),
        D2D1::Point2F(L + w - innerPad, B - w - innerPad)};
    ...
    auto fillPolygon = [&](const D2D1_POINT_2F *pts, int count,
    ...
    auto drawBlinkSolidSideWithBrush =
        [&](int side, float x1, float y1, float x2, float y2,
    ...
    auto drawBlinkSolidSide = [&](int side, float x1, float y1, float x2,
                                  float y2, float adjacentWidth1,
                                  float adjacentWidth2) {
      drawBlinkSolidSideWithBrush(side, x1, y1, x2, y2, adjacentWidth1,
                                  adjacentWidth2, strokeBrush);
    };
    auto drawBlinkDoubleSide = [&](int side, float x1, float y1, float x2,
                                   float y2, float adjacentWidth1,
                                   float adjacentWidth2) {
      ...
      if (adjacentWidth1 == 0.0f && adjacentWidth2 == 0.0f) {
        if (side == 0 || side == 2) {
          context->FillRectangle(D2D1::RectF(x1, y1, x2, y1 + third),
                                 strokeBrush);
          context->FillRectangle(D2D1::RectF(x1, y2 - third, x2, y2),
                                 strokeBrush);
        } else {
          context->FillRectangle(D2D1::RectF(x1, y1, x1 + third, y2),
                                 strokeBrush);
          context->FillRectangle(D2D1::RectF(x2 - third, y1, x2, y2),
                                 strokeBrush);
        }
        return;
      }
      auto bigThird = [](float value) {
        return value > 0.0f ? std::floor((value + 1.0f) / 3.0f)
                            : std::ceil((value - 1.0f) / 3.0f);
      };
      ...
      case 0:
        drawBlinkSolidSide(
            side, x1 + std::max((-adjacentWidth1 * 2.0f + 1.0f) / 3.0f, 0.0f),
            y1, x2 - std::max((-adjacentWidth2 * 2.0f + 1.0f) / 3.0f, 0.0f),
            y1 + third, adjacent1BigThird, adjacent2BigThird);
        drawBlinkSolidSide(
            side, x1 + std::max((adjacentWidth1 * 2.0f + 1.0f) / 3.0f, 0.0f),
            y2 - third,
            x2 - std::max((adjacentWidth2 * 2.0f + 1.0f) / 3.0f, 0.0f), y2,
      ...
      case 3:
        ...
        drawBlinkSolidSide(
            side, x2 - third,
            y1 + std::max((adjacentWidth1 * 2.0f + 1.0f) / 3.0f, 0.0f), x2,
            y2 - std::max((adjacentWidth2 * 2.0f + 1.0f) / 3.0f, 0.0f),
            adjacent1BigThird, adjacent2BigThird);
        break;
      case 2:
    ...
    Microsoft::WRL::ComPtr<ID2D1SolidColorBrush> lightBrush, darkBrush;
    context->CreateSolidColorBrush(
        Direct2D::ColorToD2D(lightC, params.strokeAlpha / 255.0f), &lightBrush);
    context->CreateSolidColorBrush(
        Direct2D::ColorToD2D(darkC, params.strokeAlpha / 255.0f), &darkBrush);
    auto drawBlink3DSide = [&](int side, BoxBorder::Style style, float x1,
      ...
      auto bigHalf = [](float value) {
        return value > 0.0f ? std::floor((value + 1.0f) / 2.0f)
                            : std::ceil((value - 1.0f) / 2.0f);
      };
      const float adjacent1BigHalf = bigHalf(adjacentWidth1);
      const float adjacent2BigHalf = bigHalf(adjacentWidth2);
      ID2D1Brush *insetBrush =
          (side == 0 || side == 3) ? darkBrush.Get() : lightBrush.Get();
      ID2D1Brush *outsetBrush =
          (side == 0 || side == 3) ? lightBrush.Get() : darkBrush.Get();
      if (style == BoxBorder::Style::Inset ||
          style == BoxBorder::Style::Outset) {
        ID2D1Brush *solidBrush =
            (style == BoxBorder::Style::Inset) ? insetBrush : outsetBrush;
        drawBlinkSolidSideWithBrush(side, x1, y1, x2, y2, adjacentWidth1,
                                    adjacentWidth2, solidBrush);
        return;
      }
      ID2D1Brush *firstBrush =
          (style == BoxBorder::Style::Groove) ? insetBrush : outsetBrush;
      ID2D1Brush *secondBrush =
          (style == BoxBorder::Style::Groove) ? outsetBrush : insetBrush;
      switch (side) {
      case 0:
        drawBlinkSolidSideWithBrush(
            0, x1 + std::max(-adjacentWidth1, 0.0f) / 2.0f, y1,
            x2 - std::max(-adjacentWidth2, 0.0f) / 2.0f,
            std::floor((y1 + y2 + 1.0f) / 2.0f), adjacent1BigHalf,
            adjacent2BigHalf, firstBrush);
        drawBlinkSolidSideWithBrush(
            0, x1 + std::max(adjacentWidth1 + 1.0f, 0.0f) / 2.0f,
            std::floor((y1 + y2 + 1.0f) / 2.0f),
            x2 - std::max(adjacentWidth2 + 1.0f, 0.0f) / 2.0f, y2,
            adjacentWidth1 / 2.0f, adjacentWidth2 / 2.0f, secondBrush);
        break;
      case 2:
        drawBlinkSolidSideWithBrush(
            2, x1 + std::max(adjacentWidth1, 0.0f) / 2.0f, y1,
            x2 - std::max(adjacentWidth2, 0.0f) / 2.0f,
            std::floor((y1 + y2 + 1.0f) / 2.0f), adjacent1BigHalf,
            adjacent2BigHalf, secondBrush);
        drawBlinkSolidSideWithBrush(
            2, x1 + std::max(-adjacentWidth1 + 1.0f, 0.0f) / 2.0f,
            std::floor((y1 + y2 + 1.0f) / 2.0f),
            x2 - std::max(-adjacentWidth2 + 1.0f, 0.0f) / 2.0f, y2,
      ...
      case 3:
        drawBlinkSolidSideWithBrush(
            3, x1, y1 + std::max(-adjacentWidth1, 0.0f) / 2.0f,
            std::floor((x1 + x2 + 1.0f) / 2.0f),
            y2 - std::max(-adjacentWidth2, 0.0f) / 2.0f, adjacent1BigHalf,
            adjacent2BigHalf, firstBrush);
        drawBlinkSolidSideWithBrush(
            3, std::floor((x1 + x2 + 1.0f) / 2.0f),
            y1 + std::max(adjacentWidth1 + 1.0f, 0.0f) / 2.0f, x2,
      ...
      default:
        drawBlinkSolidSideWithBrush(
            1, x1, y1 + std::max(adjacentWidth1, 0.0f) / 2.0f,
            std::floor((x1 + x2 + 1.0f) / 2.0f),
            y2 - std::max(adjacentWidth2, 0.0f) / 2.0f, adjacent1BigHalf,
            adjacent2BigHalf, secondBrush);
        drawBlinkSolidSideWithBrush(
            1, std::floor((x1 + x2 + 1.0f) / 2.0f),
            y1 + std::max(-adjacentWidth1 + 1.0f, 0.0f) / 2.0f, x2,
            y2 - std::max(-adjacentWidth2 + 1.0f, 0.0f) / 2.0f,
            adjacentWidth1 / 2.0f, adjacentWidth2 / 2.0f, firstBrush);
        break;
    ...
    auto stylePriority = [](BoxBorder::Style s) -> int {
      if (s == BoxBorder::Style::Dotted || s == BoxBorder::Style::Dashed ||
          s == BoxBorder::Style::Double)
        return 1;
    ...
    std::vector<SidePaint> sides = {
        {params.styleTop, topClip, stylePriority(params.styleTop), topP0, topP1,
         0},
        {params.styleRight, rightClip, stylePriority(params.styleRight),
         rightP0, rightP1, 1},
        {params.styleBottom, bottomClip, stylePriority(params.styleBottom),
         bottomP0, bottomP1, 2},
        {params.styleLeft, leftClip, stylePriority(params.styleLeft), leftP0,
         leftP1, 3}};
    std::stable_sort(sides.begin(), sides.end(),
                     [](const SidePaint &a, const SidePaint &b) {
                       return a.priority < b.priority;
                     });
    auto isVisibleStyleForSide = [](BoxBorder::Style style) {
      return style != BoxBorder::Style::None &&
             style != BoxBorder::Style::Hidden;
    };
    auto is3DStyleForSide = [](BoxBorder::Style style) {
      return style == BoxBorder::Style::Groove ||
             style == BoxBorder::Style::Ridge ||
             style == BoxBorder::Style::Inset ||
             style == BoxBorder::Style::Outset;
    };
    ...
    auto quarterEllipseArcLength = [](float radX, float radY) -> float {
      ...
      const float fullPerimeter =
          3.141592653589793f * (a + b) *
    ...
    auto locatePerimeterT = [&](float t, float &edgeRadX, float &edgeRadY,
                                float &cx, float &cy, bool outer,
                                int &segment) -> float {
      if (t < 0.0f)
        t = 0.0f;
      if (t > perimeter)
    ...
    auto getOuterEdgePoint = [&](float t, float &x, float &y) {
      ...
      int segment = 0;
      float local =
          locatePerimeterT(t, edgeRadX, edgeRadY, cx, cy, true, segment);
      switch (segment) {
      case 0:
        x = L + rx + local;
        y = T;
    ...
    auto getInnerEdgePoint = [&](float t, float &x, float &y) {
      ...
      int segment = 0;
      float local =
          locatePerimeterT(t, edgeRadX, edgeRadY, cx, cy, false, segment);
      switch (segment) {
      case 0:
        x = il + irx + local;
        y = it;
    ...
    // Each side owns half of its start/end corner arcs (CSS 45deg corner
    // split).
    ...
    auto fillRoundedSideBandBetween = [&](int sideIndex, float depth0,
                                          float depth1, ID2D1Brush *brush) {
      if (!brush)
        return;
      ...
      for (int i = 0; i <= steps; ++i) {
        const float localT = -joinPad + spanLen * static_cast<float>(i) /
    ...
    auto createRoundedSideBandClip =
        [&](int sideIndex) -> Microsoft::WRL::ComPtr<ID2D1Geometry> {
      const float segLen = sideSegmentLength(sideIndex);
      if (segLen <= 0.001f)
        return nullptr;
      const float spanLen = segLen + 2.0f * joinPad;
    ...
    auto pushSideBandClip = [&](int sideIndex) {
      Microsoft::WRL::ComPtr<ID2D1Geometry> clip =
          createRoundedSideBandClip(sideIndex);
      if (!clip)
        return;
    ...
    auto drawRoundedSideDotted = [&](int sideIndex) {
      ...
      for (int i = 0; i < numDots; ++i) {
        ...
        context->FillEllipse(
            D2D1::Ellipse(D2D1::Point2F(x, y), dotRadius, dotRadius),
            strokeBrush);
      }
    };
    auto fillRoundedDashStrip = [&](int sideIndex, float localStart,
                                    float localEnd) {
      const float segLen = sideSegmentLength(sideIndex);
      ...
      for (int i = 0; i <= samples; ++i) {
        const float localT = clipStart + stripLen * static_cast<float>(i) /
    ...
    auto drawRoundedSideDashed = [&](int sideIndex) {
      ...
      if (dashCount < 1) {
        if (runLen >= w) {
          ...
          fillRoundedDashStrip(sideIndex, runStart + inset,
                               runStart + inset + dashWidth);
        }
        return;
      }
      const float gapActual =
          (runLen - static_cast<float>(dashCount) * dashLen) /
          static_cast<float>(dashCount + 1);
      for (int i = 0; i < dashCount; ++i) {
        const float dashStart = runStart + gapActual +
    ...
    auto drawRectSideDashed = [&](int sideIndex) {
      ...
      switch (sideIndex) {
      case 0:
        context->DrawLine(D2D1::Point2F(L, T + w * 0.5f),
                          D2D1::Point2F(R, T + w * 0.5f), strokeBrush, w,
                          dashedStroke.Get());
        break;
      case 1:
        context->DrawLine(D2D1::Point2F(R - w * 0.5f, T),
                          D2D1::Point2F(R - w * 0.5f, B), strokeBrush, w,
                          dashedStroke.Get());
        break;
      case 2:
        context->DrawLine(D2D1::Point2F(L, B - w * 0.5f),
                          D2D1::Point2F(R, B - w * 0.5f), strokeBrush, w,
                          dashedStroke.Get());
        break;
      case 3:
      default:
        context->DrawLine(D2D1::Point2F(L + w * 0.5f, T),
                          D2D1::Point2F(L + w * 0.5f, B), strokeBrush, w,
                          dashedStroke.Get());
        break;
      }
    };
    for (const auto &side : sides) {
      if (!side.clip || side.style == BoxBorder::Style::None ||
          side.style == BoxBorder::Style::Hidden)
        continue;
      const bool useSideClip =
          !hasRadius && side.style == BoxBorder::Style::Dotted;
      if (useSideClip) {
        D2D1_LAYER_PARAMETERS1 params = D2D1::LayerParameters1(
            D2D1::InfiniteRect(), side.clip.Get(),
            D2D1_ANTIALIAS_MODE_PER_PRIMITIVE, D2D1::Matrix3x2F::Identity(),
            1.0f, nullptr, D2D1_LAYER_OPTIONS1_NONE);
        context->PushLayer(params, nullptr);
      }
      if (side.style == BoxBorder::Style::Dotted ||
          side.style == BoxBorder::Style::Dashed) {
        if (hasRadius) {
          if (side.style == BoxBorder::Style::Dotted) {
          ...
          if (len > 0.001f) {
            ...
            if (side.style == BoxBorder::Style::Dotted) {
              ...
              int count =
                  std::max(2, static_cast<int>(len / targetSpacing + 0.5f) + 1);
              for (int i = 0; i < count; ++i) {
                const float t =
                    len * static_cast<float>(i) / static_cast<float>(count - 1);
                const D2D1_POINT_2F pt =
                    D2D1::Point2F(side.p0.x + ux * t, side.p0.y + uy * t);
                context->FillEllipse(D2D1::Ellipse(pt, w * 0.5f, w * 0.5f),
                                     strokeBrush);
              }
            } else {
        ...
        if (hasRadius) {
          ...
          const BoxBorder::Style adjacentStyle1 =
              side.index == 0   ? params.styleLeft
              : side.index == 1 ? params.styleTop
              : side.index == 2 ? params.styleLeft
                                : params.styleTop;
          const BoxBorder::Style adjacentStyle2 =
              side.index == 0   ? params.styleRight
              : side.index == 1 ? params.styleBottom
              : side.index == 2 ? params.styleRight
                                : params.styleBottom;
          const float adjacent1 =
              (side.index == 2 && is3DStyleForSide(adjacentStyle1))
                  ? 0.0f
                  : (isVisibleStyleForSide(adjacentStyle1) ? w : 0.0f);
          const float adjacent2 =
              isVisibleStyleForSide(adjacentStyle2) ? w : 0.0f;
          if (side.index == 0)
        ...
        if (hasRadius) {
          ...
          const float adjacent1 =
              isVisibleStyleForSide(side.index == 0   ? params.styleLeft
                                    : side.index == 1 ? params.styleTop
                                    : side.index == 2 ? params.styleLeft
                                                      : params.styleTop)
                  ? w
                  : 0.0f;
          const float adjacent2 =
              isVisibleStyleForSide(side.index == 0   ? params.styleRight
                                    : side.index == 1 ? params.styleBottom
                                    : side.index == 2 ? params.styleRight
                                                      : params.styleBottom)
                  ? w
        ...
        if (hasRadius) {
          ID2D1Brush *insetBrush = (side.index == 0 || side.index == 3)
                                       ? darkBrush.Get()
                                       : lightBrush.Get();
          ID2D1Brush *outsetBrush = (side.index == 0 || side.index == 3)
                                        ? lightBrush.Get()
                                        : darkBrush.Get();
          ...
          const float adjacent2 =
              isVisibleStyleForSide(side.index == 0   ? params.styleRight
                  ...
                  : 0.0f;
          if (side.index == 0)
            drawBlink3DSide(0, side.style, L, T, R, T + w, adjacent1,
                            adjacent2);
          else if (side.index == 1)
            drawBlink3DSide(1, side.style, R - w, T, R, B, adjacent1,
                            adjacent2);
          else if (side.index == 2)
            drawBlink3DSide(2, side.style, L, B - w, R, B, adjacent1,
                            adjacent2);
          else
            drawBlink3DSide(3, side.style, L, T, L + w, B, adjacent1,
                            adjacent2);
        }
      }
      if (useSideClip)
    ...
    auto fillCornerBevel = [&](D2D1_POINT_2F a, D2D1_POINT_2F b,
                               D2D1_POINT_2F c) {
    ...
    if (hasRadius) {
      auto fillRoundedCornerPatch = [&](float junctionT, ID2D1Brush *brush) {
        ...
        float o0x = 0.0f, o0y = 0.0f, o1x = 0.0f, o1y = 0.0f, ojx = 0.0f,
              ojy = 0.0f;
        float i0x = 0.0f, i0y = 0.0f, i1x = 0.0f, i1y = 0.0f, ijx = 0.0f,
              ijy = 0.0f;
        ...
        const D2D1_POINT_2F pts[] = {
            D2D1::Point2F(o0x, o0y), D2D1::Point2F(ojx, ojy),
            D2D1::Point2F(o1x, o1y), D2D1::Point2F(i1x, i1y),
            D2D1::Point2F(ijx, ijy), D2D1::Point2F(i0x, i0y)};
      ...
      const float junctionBL =
          L_top + L_arc + L_right + sideArcSpan + L_bottom + L_arc;
      ...
      if (isVisibleStyle(params.styleTop) && isVisibleStyle(params.styleLeft) &&
          !isComplexJoinStyle(params.styleTop) &&
          !isComplexJoinStyle(params.styleLeft))
        fillCornerBevel(D2D1::Point2F(L, T), D2D1::Point2F(L + w, T),
                        D2D1::Point2F(L, T + w));
      if (isVisibleStyle(params.styleTop) &&
          isVisibleStyle(params.styleRight) &&
          !isComplexJoinStyle(params.styleTop) &&
          !isComplexJoinStyle(params.styleRight))
        fillCornerBevel(D2D1::Point2F(R - w, T), D2D1::Point2F(R, T),
                        D2D1::Point2F(R, T + w));
      if (isVisibleStyle(params.styleBottom) &&
          isVisibleStyle(params.styleLeft) &&
          !isComplexJoinStyle(params.styleBottom) &&
          !isComplexJoinStyle(params.styleLeft))
        fillCornerBevel(D2D1::Point2F(L, B - w), D2D1::Point2F(L + w, B - w),
                        D2D1::Point2F(L, B));
      if (isVisibleStyle(params.styleBottom) &&
          isVisibleStyle(params.styleRight) &&
          !isComplexJoinStyle(params.styleBottom) &&
          !isComplexJoinStyle(params.styleRight))
        fillCornerBevel(D2D1::Point2F(R - w, B - w), D2D1::Point2F(R, B - w),
                        D2D1::Point2F(R, B));
    }
  }
}
