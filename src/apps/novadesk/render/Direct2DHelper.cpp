...
bool ReadExifOrientation(IWICBitmapFrameDecode *frame, USHORT &outOrientation) {
  ...
  if (value.vt == VT_UI2) {
    ...
    found = true;
             ...
             value.caub.cElems >= sizeof(USHORT)) {
    USHORT tmp = 1;
...
bool Initialize() {
  ...
  if (FAILED(hr)) {
    Logging::Log(LogLevel::Error, L"Failed to create DWriteFactory (0x%08X)",
                 hr);
    if (g_comInitialized) {
      CoUninitialize();
      g_comInitialized = false;
  ...
  hr = CoCreateInstance(CLSID_WICImagingFactory, nullptr, CLSCTX_INPROC_SERVER,
                        IID_PPV_ARGS(g_pWICFactory.GetAddressOf()));
  if (FAILED(hr)) {
    Logging::Log(LogLevel::Error,
                 L"Failed to create WICImagingFactory (0x%08X)", hr);
    if (g_comInitialized) {
      CoUninitialize();
      g_comInitialized = false;
    }
...
bool CreateLinearGradientBrush(ID2D1RenderTarget *context,
  ...
  HRESULT hr =
      context->CreateGradientStopCollection(stops, 2, pStops.GetAddressOf());
  if (FAILED(hr))
    return false;

  hr = context->CreateLinearGradientBrush(
      D2D1::LinearGradientBrushProperties(start, end), pStops.Get(), brush);
  return SUCCEEDED(hr);
}

bool CreateGradientBrush(ID2D1RenderTarget *context, const D2D1_RECT_F &rect,
                         const GradientInfo &info, ID2D1Brush **brush) {
  if (!context || info.type == GRADIENT_NONE || info.stops.empty())
    return false;
  ...
  HRESULT hr = context->CreateGradientStopCollection(
      stops.data(), (UINT32)stops.size(), pStops.GetAddressOf());
  if (FAILED(hr))
    return false;
  ...
  if (info.type == GRADIENT_LINEAR) {
    ...
    hr = context->CreateLinearGradientBrush(
        D2D1::LinearGradientBrushProperties(start, end), pStops.Get(),
        &pLinear);
    if (SUCCEEDED(hr)) {
...
bool CreateBrushFromGradientOrColor(ID2D1RenderTarget *context,
  ...
  if (gradient && gradient->type != GRADIENT_NONE && !gradient->stops.empty()) {
    if (CreateGradientBrush(context, rect, *gradient, brush))
      return true;
  }
...
bool LoadWICBitmapFromFile(const std::wstring &path, IWICBitmap **wicBitmap,
  ...
  if (path.length() >= 6 && _wcsnicmp(path.c_str(), L"res://", 6) == 0) {
    ...
    return LoadWICBitmapFromResource(GetModuleHandleW(NULL), nameParam,
                                     resType.c_str(), wicBitmap);
  }
  ...
  if (useExifOrientation) {
    ...
    if (ReadExifOrientation(pFrame.Get(), orientation)) {
      const WICBitmapTransformOptions transform =
          ExifOrientationToWicTransform(orientation);
      if (transform != WICBitmapTransformRotate0) {
        ComPtr<IWICBitmapFlipRotator> pFlipRotator;
        hr =
            g_pWICFactory->CreateBitmapFlipRotator(pFlipRotator.GetAddressOf());
        if (FAILED(hr))
          return false;
  ...
  hr = g_pWICFactory->CreateBitmapFromSource(pConverter.Get(),
...
bool LoadWICBitmapFromURL(const std::wstring &url, IWICBitmap **wicBitmap,
  ...
  if (FAILED(hr)) {
    Logging::Log(LogLevel::Error,
                 L"[novadesk] Failed to initialize stream from memory");
    return false;
  }

  // Decode image from stream
  ComPtr<IWICBitmapDecoder> pDecoder;
  hr = g_pWICFactory->CreateDecoderFromStream(pStream.Get(), nullptr,
                                              WICDecodeMetadataCacheOnLoad,
                                              pDecoder.GetAddressOf());

  if (FAILED(hr)) {
    Logging::Log(LogLevel::Error,
                 L"[novadesk] Failed to create decoder from stream");
    return false;
  }
  ...
  if (FAILED(hr)) {
    Logging::Log(LogLevel::Error,
                 L"[novadesk] Failed to get frame from decoder");
    return false;
  }
  ...
  if (FAILED(hr)) {
    Logging::Log(LogLevel::Error,
                 L"[novadesk] Failed to create format converter");
    return false;
  }
  ...
  if (FAILED(hr)) {
    Logging::Log(LogLevel::Error,
                 L"[novadesk] Failed to initialize format converter");
    return false;
  }
  ...
  if (FAILED(hr)) {
    Logging::Log(LogLevel::Error,
                 L"[novadesk] Failed to create bitmap from source");
    return false;
  }

  Logging::Log(LogLevel::Info,
               L"[novadesk] Successfully loaded image from URL: %s",
               url.c_str());
  return true;
}

bool LoadBitmapFromFile(ID2D1RenderTarget *context, const std::wstring &path,
                        ID2D1Bitmap **bitmap, IWICBitmap **wicBitmap,
                        bool useExifOrientation) {
  if (bitmap)
    *bitmap = nullptr;
  if (wicBitmap)
  ...
  if (!context || !g_pWICFactory)
    return false;

  ComPtr<IWICBitmap> pWICBitmap;
  if (!LoadWICBitmapFromFile(path, pWICBitmap.GetAddressOf(),
                             useExifOrientation))
    return false;

  HRESULT hr =
      context->CreateBitmapFromWicBitmap(pWICBitmap.Get(), nullptr, bitmap);
...
D2D1_POINT_2F FindEdgePoint(float angle, const D2D1_RECT_F &r) {
  ...
  const float axis_angle = [&]() -> float {
    ...
    default:
    case 1:
      return base_radians - M_PI_F * 0.0f;
    case 2:
      return M_PI_F * 1.0f - base_radians;
    case 3:
      return base_radians - M_PI_F * 1.0f;
    case 4:
      return M_PI_F * 2.0f - base_radians;
    }
  }();

  const float half_area =
      std::sqrt(std::pow(width, 2.0f) + std::pow(height, 2.0f)) / 2.0f;
  const float cos_axis = std::cos(std::fabs(axis_angle - rectangle_tangent));

  return D2D1::Point2F(r.left + (width / 2.0f) +
                           (half_area * cos_axis * std::cos(base_radians)),
                       r.top + (height / 2.0f) +
                           (half_area * cos_axis * std::sin(base_radians)));
}
...
bool LoadWICBitmapFromResource(HMODULE hModule, LPCWSTR resourceName,
  ...
  hr = pStream->InitializeFromMemory(static_cast<BYTE *>(pResourceData),
                                     cbResource);
  if (FAILED(hr))
    return false;
