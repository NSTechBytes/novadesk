...
int ArcShape::GetAutoWidth() {
  if (m_RadiusX > 0.0f)
    return (int)ceilf(m_RadiusX * 2.0f);
  if (m_RadiusY > 0.0f)
    return (int)ceilf(m_RadiusY * 2.0f);
  return 0;
}
...
D2D1_POINT_2F ArcShape::CheckPoint(float angle, float rx, float ry, float cx,
                                   float cy) const {
...
bool ArcShape::CreateArcGeometry(ID2D1Factory *factory,
                                 ID2D1PathGeometry **outGeometry, float &outRx,
                                 float &outRy, float &outCx,
                                 float &outCy) const {
  if (!factory)
    return false;
  if (FAILED(factory->CreatePathGeometry(outGeometry)) || !*outGeometry)
    return false;
  ...
  D2D1_POINT_2F startPoint =
      CheckPoint(m_StartAngle, outRx, outRy, outCx, outCy);
  D2D1_POINT_2F endPoint = CheckPoint(m_EndAngle, outRx, outRy, outCx, outCy);

  pSink->BeginFigure(startPoint, m_HasFill ? D2D1_FIGURE_BEGIN_FILLED
                                           : D2D1_FIGURE_BEGIN_HOLLOW);

  D2D1_SWEEP_DIRECTION sweep = m_Clockwise
                                   ? D2D1_SWEEP_DIRECTION_CLOCKWISE
                                   : D2D1_SWEEP_DIRECTION_COUNTER_CLOCKWISE;

  float sweepAngle = m_EndAngle - m_StartAngle;
  if (m_Clockwise && sweepAngle < 0)
    sweepAngle += 360.0f;
  if (!m_Clockwise && sweepAngle > 0)
    sweepAngle -= 360.0f;
  ...
  pSink->AddArc(
      D2D1::ArcSegment(endPoint, D2D1::SizeF(outRx, outRy), 0.0f, sweep,
...
void ArcShape::Render(ID2D1DeviceContext *context) {
  ...
  if (CreateGeometry(pFactory, geometry)) {
    ...
    if (pStrokeBrush) {
      ...
      context->DrawGeometry(geometry.Get(), pStrokeBrush.Get(), m_StrokeWidth,
                            m_StrokeStyle.Get());
    }
  }
