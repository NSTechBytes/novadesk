...
PathShape::PathShape(const std::wstring &id, int x, int y, int width,
                     int height)
    : ShapeElement(id, x, y, width, height) {}

PathShape::~PathShape() {}

void PathShape::SetCombinedGeometry(
    Microsoft::WRL::ComPtr<ID2D1Geometry> geometry, const D2D1_RECT_F &bounds) {
  m_CombinedGeometry = geometry;
  m_HasCombinedGeometry = (geometry != nullptr);
  m_CombinedBounds = GfxRect((int)floorf(bounds.left), (int)floorf(bounds.top),
...
void PathShape::SetCombineData(const std::wstring &baseId,
                               const std::vector<CombineOp> &ops,
                               bool consumeBase) {
...
void PathShape::GetCombineData(std::wstring &baseId,
                               std::vector<CombineOp> &ops,
...
void PathShape::CreatePathGeometry(ID2D1Factory *factory,
  ...
  for (NSVGshape *shape = image->shapes; shape != NULL; shape = shape->next) {
    for (NSVGpath *path = shape->paths; path != NULL; path = path->next) {
      ...
      pSink->BeginFigure(startPt, m_HasFill ? D2D1_FIGURE_BEGIN_FILLED
      ...
      pSink->EndFigure(path->closed ? D2D1_FIGURE_END_CLOSED
