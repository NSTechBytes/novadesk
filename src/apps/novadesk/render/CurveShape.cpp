...
CurveShape::CurveShape(const std::wstring &id, int x, int y, int width,
                       int height)
    : ShapeElement(id, x, y, width, height) {}

CurveShape::~CurveShape() {}

bool CurveShape::CreateCurveGeometry(ID2D1Factory *factory,
  ...
  sink->BeginFigure(D2D1::Point2F(m_StartX, m_StartY),
                    m_HasFill ? D2D1_FIGURE_BEGIN_FILLED
                              : D2D1_FIGURE_BEGIN_HOLLOW);

  if (m_IsCubic) {
    sink->AddBezier(D2D1::BezierSegment(D2D1::Point2F(m_ControlX, m_ControlY),
                                        D2D1::Point2F(m_Control2X, m_Control2Y),
                                        D2D1::Point2F(m_EndX, m_EndY)));
  } else {
    sink->AddQuadraticBezier(D2D1::QuadraticBezierSegment(
        D2D1::Point2F(m_ControlX, m_ControlY), D2D1::Point2F(m_EndX, m_EndY)));
  }
...
bool CurveShape::CreateGeometry(
    ID2D1Factory *factory,
    Microsoft::WRL::ComPtr<ID2D1Geometry> &geometry) const {
  ID2D1PathGeometry *path = nullptr;
  if (!CreateCurveGeometry(factory, &path))
    return false;
