...
ShapeElement::ShapeElement(const std::wstring &id, int x, int y, int width,
...
GfxRect ShapeElement::GetBackgroundBounds() {
  ...
  return GfxRect(bounds.X - pad, bounds.Y - pad, bounds.Width + pad * 2,
                 bounds.Height + pad * 2);
}

void ShapeElement::CreateBrush(ID2D1DeviceContext *context,
                               ID2D1Brush **ppBrush, bool isStroke) {
  bool hasGradient = isStroke ? m_HasStrokeGradient : m_HasFillGradient;

  D2D1_RECT_F rect = D2D1::RectF((float)m_X, (float)m_Y, (float)(m_X + m_Width),
                                 (float)(m_Y + m_Height));
  ...
  Direct2D::CreateBrushFromGradientOrColor(context, rect, gradInfo, c,
                                           a / 255.0f, ppBrush);
}

bool ShapeElement::TryCreateStrokeBrush(
    ID2D1DeviceContext *context, Microsoft::WRL::ComPtr<ID2D1Brush> &outBrush) {
...
bool ShapeElement::TryCreateFillBrush(
    ID2D1DeviceContext *context, Microsoft::WRL::ComPtr<ID2D1Brush> &outBrush) {
  if (!m_HasFill)
    return false;
  ...
  if (m_UpdateStrokeStyle || !m_StrokeStyle) {
    ...
    if (factory) {
      ...
      if (FAILED(hrQI)) {
        Logging::Log(
            LogLevel::Error,
            L"ShapeElement: QueryInterface ID2D1Factory1 failed (0x%08X)",
            hrQI);
      }
    }
    ...
    if (factory1) {
      ...
      props.dashStyle = (m_StrokeDashes.empty() ? D2D1_DASH_STYLE_SOLID
      ...
      HRESULT hrCreate = factory1->CreateStrokeStyle(
          props, m_StrokeDashes.data(), (UINT32)m_StrokeDashes.size(),
          m_StrokeStyle.GetAddressOf());
      if (FAILED(hrCreate)) {
        Logging::Log(LogLevel::Error,
                     L"ShapeElement: CreateStrokeStyle failed (0x%08X)",
                     hrCreate);
        return;
      }
    }
...
void ShapeElement::EnsureStrokeStyle() {
  ...
  HRESULT hr = factory->CreateStrokeStyle(props, m_StrokeDashes.data(),
                                          (UINT32)m_StrokeDashes.size(),
                                          m_StrokeStyle.GetAddressOf());
  if (FAILED(hr)) {
    Logging::Log(LogLevel::Error,
                 L"ShapeElement: CreateStrokeStyle failed (0x%08X)", hr);
    return;
  }
...
D2D1_MATRIX_3X2_F ShapeElement::GetRenderTransformMatrix() const {
  if (m_HasTransformMatrix) {
    return D2D1::Matrix3x2F(m_TransformMatrix[0], m_TransformMatrix[1],
                            m_TransformMatrix[2], m_TransformMatrix[3],
                            m_TransformMatrix[4], m_TransformMatrix[5]);
  }
