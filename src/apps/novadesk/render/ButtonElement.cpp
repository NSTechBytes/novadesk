...
ButtonElement::ButtonElement(const std::wstring &id, int x, int y,
                             const std::wstring &path)
    : Element(ELEMENT_BUTTON, id, x, y, 0, 0) {
  Logging::Log(LogLevel::Info, L"ButtonElement created: id=%s, path=%s",
               id.c_str(), path.c_str());
  UpdateImage(path);
}
...
void ButtonElement::Render(ID2D1DeviceContext *context) {
  ...
  if (m_ButtonImage.IsLoaded()) {
    ...
    if (m_ButtonImage.BuildProcessedImage(context, finalImage) && finalImage) {
      ...
      D2D1_RECT_F srcRect;
      if (horizontal) {
        srcRect = D2D1::RectF(frameWidth * frameIndex, 0,
                              frameWidth * (frameIndex + 1), imageHeight);
      } else {
        srcRect = D2D1::RectF(0, frameHeight * frameIndex, imageWidth,
                              frameHeight * (frameIndex + 1));
      }
      ...
      D2D1_MATRIX_3X2_F finalDrawTransform =
          ...
          D2D1::Matrix3x2F::Translation(
              static_cast<float>(m_X + m_PaddingLeft),
              static_cast<float>(m_Y + m_PaddingTop)) *
          currentTransform;

      context->SetTransform(finalDrawTransform);
      context->DrawImage(finalImage.Get(),
                         D2D1::Point2F(srcRect.left, srcRect.top), srcRect);

      context->SetTransform(currentTransform);

      // Logging::Log(LogLevel::Info, L"Button Render Success: id=%s, frame=%d,
      // dest=[%.1f, %.1f]", m_Id.c_str(), frameIndex, destW, destH);
    } else {
      Logging::Log(LogLevel::Error,
                   L"Button Render Error: BuildProcessedImage failed for %s",
                   m_Id.c_str());
    }
  }
...
bool ButtonElement::HitTest(int x, int y) {
  ...
  float height =
      static_cast<float>(GetHeight() - m_PaddingTop - m_PaddingBottom);

  int autoW = GetAutoWidth();
  int autoH = GetAutoHeight();

  if (width <= 0.0f)
    width = static_cast<float>(autoW);
  if (height <= 0.0f)
    height = static_cast<float>(autoH);
  ...
  float frameWidth =
      horizontal ? (imgWidth / 3.0f) : static_cast<float>(imgWidth);
  float frameHeight =
      horizontal ? static_cast<float>(imgHeight) : (imgHeight / 3.0f);
  ...
  int pixelX = static_cast<int>((relX * frameWidth) +
                                (horizontal ? (frameIndex * frameWidth) : 0));
  int pixelY = static_cast<int>((relY * frameHeight) +
                                (horizontal ? 0 : (frameIndex * frameHeight)));
...
void ButtonElement::OnImageDownloaded(const std::wstring &url,
