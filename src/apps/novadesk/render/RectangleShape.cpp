...
RectangleShape::RectangleShape(const std::wstring &id, int x, int y, int width,
                               int height)
...
bool RectangleShape::CreateGeometry(
  ...
  rect.rect = D2D1::RectF((float)m_X, (float)m_Y, (float)(m_X + m_Width),
                          (float)(m_Y + m_Height));
...
void RectangleShape::Render(ID2D1DeviceContext *context) {
  ...
  if (pStrokeBrush) {
    ...
    context->DrawRoundedRectangle(rect, pStrokeBrush.Get(), m_StrokeWidth,
                                  m_StrokeStyle.Get());
  }
  RenderBevel(context);
