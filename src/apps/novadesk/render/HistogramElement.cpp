...
HistogramElement::HistogramElement(const std::wstring &id, int x, int y, int w,
                                   int h)
...
float HistogramElement::SampleAtFromNewest(const std::vector<float> &series,
...
float HistogramElement::NormalizeValue(float value, float minValue,
                                       float maxValue) {
...
bool HistogramElement::HitTest(int x, int y) {
  ...
  if (m_HasTransformMatrix || m_Rotate != 0.0f) {
    ...
    if (matrix.Invert()) {
      const D2D1_POINT_2F transformed =
          matrix.TransformPoint(D2D1::Point2F(targetX, targetY));
  ...
  int secondarySize = 0;
  if (hasSecondary) {
    const float secondaryValue =
        SampleAtFromNewest(m_SecondaryData, sampleIndex);
  ...
  const int bothSize =
      hasSecondary ? (std::min)(primarySize, secondarySize) : 0;
  const bool bothVisible =
      hasSecondary && bothSize > 0 &&
      (m_BothAlpha > 0 || m_BothGradient.type != GRADIENT_NONE);
  const bool primaryVisible =
      primarySize > 0 &&
      (m_PrimaryAlpha > 0 || m_PrimaryGradient.type != GRADIENT_NONE);
  const bool secondaryVisible =
      secondarySize > 0 &&
      (m_SecondaryAlpha > 0 || m_SecondaryGradient.type != GRADIENT_NONE);

  if (m_GraphHorizontalOrientation) {
    const int xOffset = m_GraphStartLeft ? static_cast<int>(targetX - left)
  ...
  const int yOffset = m_Flip ? static_cast<int>(targetY - top)
...
void HistogramElement::DrawSpan(ID2D1DeviceContext *context,
                                const D2D1_RECT_F &dstRect,
                                const D2D1_RECT_F &gradientRect,
                                const GradientInfo *gradient, COLORREF color,
                                BYTE alpha) {
  if (!context)
    return;
  ...
  Direct2D::CreateBrushFromGradientOrColor(context, gradientRect, gradient,
                                           color, alpha / 255.0f,
                                           brush.GetAddressOf());
  if (brush) {
...
void HistogramElement::Render(ID2D1DeviceContext *context) {
  ...
  for (int i = 0; i < samples; ++i) {
    ...
    if (hasSecondary) {
      ...
      const float secondaryN =
          NormalizeValue(secondaryValue, minValue, maxValue);
      secondarySize =
          m_GraphHorizontalOrientation
              ? static_cast<int>(secondaryN * static_cast<float>(width))
              : static_cast<int>(secondaryN * static_cast<float>(height));
    ...
    if (m_GraphHorizontalOrientation) {
      ...
      if (hasSecondary && bothSize > 0) {
        D2D1_RECT_F bothRect =
            m_GraphStartLeft
                ? D2D1::RectF(left, y, left + static_cast<float>(bothSize),
                              y + 1.0f)
                : D2D1::RectF(right - static_cast<float>(bothSize), y, right,
                              y + 1.0f);
        DrawSpan(context, bothRect, gradientRect, &m_BothGradient, m_BothColor,
                 m_BothAlpha);
      }
      ...
      if (hasSecondary) {
        ...
        const int larger = secondaryLarger ? secondarySize : primarySize;
        if (larger > bothSize) {
          D2D1_RECT_F restRect =
              m_GraphStartLeft
                  ? D2D1::RectF(left + static_cast<float>(bothSize), y,
                                left + static_cast<float>(larger), y + 1.0f)
                  : D2D1::RectF(right - static_cast<float>(larger), y,
                                right - static_cast<float>(bothSize), y + 1.0f);
          if (secondaryLarger) {
            DrawSpan(context, restRect, gradientRect, &m_SecondaryGradient,
                     m_SecondaryColor, m_SecondaryAlpha);
          } else {
            DrawSpan(context, restRect, gradientRect, &m_PrimaryGradient,
                     m_PrimaryColor, m_PrimaryAlpha);
          }
        }
      } else if (primarySize > 0) {
        D2D1_RECT_F primaryRect =
            m_GraphStartLeft
                ? D2D1::RectF(left, y, left + static_cast<float>(primarySize),
                              y + 1.0f)
                : D2D1::RectF(right - static_cast<float>(primarySize), y, right,
                              y + 1.0f);
        DrawSpan(context, primaryRect, gradientRect, &m_PrimaryGradient,
                 m_PrimaryColor, m_PrimaryAlpha);
      }
    } else {
      ...
      if (hasSecondary && bothSize > 0) {
        D2D1_RECT_F bothRect =
            m_Flip ? D2D1::RectF(x, top, x + 1.0f,
                                 top + static_cast<float>(bothSize))
                   : D2D1::RectF(x, bottom - static_cast<float>(bothSize),
                                 x + 1.0f, bottom);
        DrawSpan(context, bothRect, gradientRect, &m_BothGradient, m_BothColor,
                 m_BothAlpha);
      }
      ...
      if (hasSecondary) {
        ...
        if (larger > bothSize) {
          D2D1_RECT_F restRect =
              m_Flip ? D2D1::RectF(x, top + static_cast<float>(bothSize),
                                   x + 1.0f, top + static_cast<float>(larger))
                     : D2D1::RectF(x, bottom - static_cast<float>(larger),
                                   x + 1.0f,
                                   bottom - static_cast<float>(bothSize));
          if (secondaryLarger) {
            DrawSpan(context, restRect, gradientRect, &m_SecondaryGradient,
                     m_SecondaryColor, m_SecondaryAlpha);
          } else {
        ...
        D2D1_RECT_F primaryRect =
            m_Flip ? D2D1::RectF(x, top, x + 1.0f,
                                 top + static_cast<float>(primarySize))
                   : D2D1::RectF(x, bottom - static_cast<float>(primarySize),
                                 x + 1.0f, bottom);
        DrawSpan(context, primaryRect, gradientRect, &m_PrimaryGradient,
                 m_PrimaryColor, m_PrimaryAlpha);
      }
