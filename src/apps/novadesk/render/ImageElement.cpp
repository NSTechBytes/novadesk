...
void ImageElement::SetScaleMargins(float left, float top, float right,
                                   float bottom) {
...
bool ImageElement::ResolveImageCropRect(float imageWidth, float imageHeight,
                                        D2D1_RECT_F &rect) const {
  return m_GeneralImage.ResolveImageCropRect(imageWidth, imageHeight, rect);
}

bool ImageElement::ComputeImageLayout(float imageWidth, float imageHeight,
                                      ImageLayout &layout) {
  ...
  if (layout.contentW <= 0 || layout.contentH <= 0 || imageWidth <= 0.0f ||
      imageHeight <= 0.0f) {
    return false;
  }

  layout.finalRect = D2D1::RectF((float)layout.contentX, (float)layout.contentY,
...
bool ImageElement::MapPointToImagePixel(float targetX, float targetY,
  ...
  if (m_PreserveAspectRatio == IMAGE_ASPECT_PRESERVE) {
    ...
    if (targetX < layout.contentX ||
        targetX >= layout.contentX + layout.contentW ||
        targetY < layout.contentY ||
        targetY >= layout.contentY + layout.contentH) {
      return false;
    }
  }
  ...
  pixelX =
      layout.srcRect.left + ((targetX - layout.finalRect.left) / finalW) * srcW;
  pixelY =
      layout.srcRect.top + ((targetY - layout.finalRect.top) / finalH) * srcH;

  m_GeneralImage.ApplyFlipToPixel(pixelX, pixelY, layout.srcRect);

  return pixelX >= 0.0f && pixelX < imageWidth && pixelY >= 0.0f &&
         pixelY < imageHeight;
}
...
void ImageElement::Render(ID2D1DeviceContext *context) {
  ...
  if (m_Tile) {
    ...
    context->CreateImageBrush(pFinalImage.Get(), brushProps,
                              pBrush.GetAddressOf());
    if (pBrush) {
      pBrush->SetTransform(D2D1::Matrix3x2F::Translation(
          layout.finalRect.left - layout.srcRect.left,
          layout.finalRect.top - layout.srcRect.top));
    ...
    if (useScaleMargins) {
      ...
      auto drawPatch = [&](float dL, float dT, float dR, float dB, float sL,
                           float sT, float sR, float sB) {
      ...
      context->DrawBitmap(bitmap, &layout.finalRect, opacity, interp,
                          &layout.srcRect);
    } else {
      ...
      // We need to apply scaling to the transform if finalRect size != srcRect
      // size.
      float scaleX = (layout.finalRect.right - layout.finalRect.left) /
                     (layout.srcRect.right - layout.srcRect.left);
      float scaleY = (layout.finalRect.bottom - layout.finalRect.top) /
                     (layout.srcRect.bottom - layout.srcRect.top);

      D2D1_MATRIX_3X2_F effectTransform;
      context->GetTransform(&effectTransform);

      // Adjust transform for scale and source rect offset
      context->SetTransform(D2D1::Matrix3x2F::Translation(-layout.srcRect.left,
                                                          -layout.srcRect.top) *
                            D2D1::Matrix3x2F::Scale(scaleX, scaleY) *
                            D2D1::Matrix3x2F::Translation(
                                layout.finalRect.left, layout.finalRect.top) *
                            effectTransform);

      context->DrawImage(pFinalImage.Get(), nullptr, nullptr,
                         m_AntiAlias
                             ? D2D1_INTERPOLATION_MODE_LINEAR
...
bool ImageElement::HitTest(int x, int y) {
  ...
  if (m_HasTransformMatrix || m_Rotate != 0.0f) {
    ...
    if (matrix.Invert()) {
      D2D1_POINT_2F transformed =
          matrix.TransformPoint(D2D1::Point2F((float)x, (float)y));
  ...
  if (!MapPointToImagePixel(targetX, targetY, imgW, imgH, targetPixelX,
                            targetPixelY))
    return false;
...
void ImageElement::OnImageDownloaded(const std::wstring &url,
