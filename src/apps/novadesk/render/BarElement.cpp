...
BarElement::BarElement(const std::wstring &id, int x, int y, int w, int h,
                       float value, BarOrientation orientation)
    : Element(ELEMENT_BAR, id, x, y, w, h), m_Value(value),
...
bool BarElement::HitTest(int x, int y) {
  ...
  auto fillContainsPoint = [&](const D2D1_RECT_F &rect, float radius) -> bool {
    ...
    if (radius > 0.0f) {
      ...
      if (FAILED(factory->CreateRoundedRectangleGeometry(
              rounded, geometry.GetAddressOf())))
        return false;
      if (SUCCEEDED(geometry->FillContainsPoint(p, nullptr, &hit)) && hit)
        return true;
    } else {
      Microsoft::WRL::ComPtr<ID2D1RectangleGeometry> geometry;
      if (FAILED(
              factory->CreateRectangleGeometry(rect, geometry.GetAddressOf())))
        return false;
      if (SUCCEEDED(geometry->FillContainsPoint(p, nullptr, &hit)) && hit)
        return true;
    }
  ...
  // Background hit-test
  if (m_HasSolidColor &&
      (m_SolidAlpha > 0 || m_SolidGradient.type != GRADIENT_NONE)) {
    D2D1_RECT_F bgRect =
        D2D1::RectF((float)m_X, (float)m_Y, (float)(m_X + GetWidth()),
                    (float)(m_Y + GetHeight()));
    if (fillContainsPoint(bgRect, (float)m_CornerRadius))
      return true;
  }
  ...
  if (m_HasBarColor || m_BarGradient.type != GRADIENT_NONE) {
    ...
    if (m_BarAlpha > 0 || m_BarGradient.type != GRADIENT_NONE) {
      ...
      if (m_Orientation == BAR_HORIZONTAL) {
        barRect = D2D1::RectF((float)m_X, (float)m_Y, (float)(m_X + w * val),
                              (float)(m_Y + h));
      } else {
        float barH = h * val;
        barRect = D2D1::RectF((float)m_X, (float)(m_Y + h - barH),
                              (float)(m_X + w), (float)(m_Y + h));
      }

      if (fillContainsPoint(barRect, (float)m_BarCornerRadius))
        return true;
    }
  }
...
void BarElement::Render(ID2D1DeviceContext *context) {
  ...
  if (m_HasBarColor || m_BarGradient.type != GRADIENT_NONE) {
    ...
    if (barRect.right > barRect.left && barRect.bottom > barRect.top) {
      ...
      Direct2D::CreateBrushFromGradientOrColor(context, barRect, &m_BarGradient,
                                               m_BarColor, m_BarAlpha / 255.0f,
                                               barBrush.GetAddressOf());

      if (barBrush) {
        if (m_BarCornerRadius > 0) {
          D2D1_ROUNDED_RECT roundedBar = D2D1::RoundedRect(
              barRect, (float)m_BarCornerRadius, (float)m_BarCornerRadius);
