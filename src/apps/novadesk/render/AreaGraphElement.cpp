...
namespace {
float DistancePointToSegment(const D2D1_POINT_2F &p, const D2D1_POINT_2F &a,
...
bool PointInPolygon(const D2D1_POINT_2F &p,
                    const std::vector<D2D1_POINT_2F> &poly) {
  if (poly.size() < 3)
    return false;
...
AreaGraphElement::AreaGraphElement(const std::wstring &id, int x, int y, int w,
                                   int h)
...
void AreaGraphElement::Render(ID2D1DeviceContext *context) {
  ...
  const D2D1_RECT_F elementRect =
      D2D1::RectF(left, top, right + 1.0f, bottom + 1.0f);

  // 1. Draw Grid
  if (m_GridVisible && m_GridAlpha > 0) {
    Microsoft::WRL::ComPtr<ID2D1Brush> gridBrush;
    Direct2D::CreateBrushFromGradientOrColor(
        context, elementRect, &m_GridGradient, m_GridColor,
        m_GridAlpha / 255.0f, gridBrush.GetAddressOf());
    if (gridBrush) {
      // Vertical lines
      if (m_GridXSpacing > 0) {
        for (float x = left + m_GridXSpacing; x < right; x += m_GridXSpacing) {
          context->DrawLine(D2D1::Point2F(x, top), D2D1::Point2F(x, bottom),
                            gridBrush.Get(), 1.0f);
        }
      }
      // Horizontal lines
  ...
  if (!m_Data.empty()) {
    ...
    for (int i = 0; i < numPoints; ++i) {
      ...
      float px = m_GraphStartLeft ? (left + (float)(numPoints - 1 - i) * dx)
                                  : (right - (float)(numPoints - 1 - i) * dx);
      float py =
          !m_Flip ? (bottom - norm * drawHeight) : (top + norm * drawHeight);
      ...
      if (factory) {
        ...
        if (SUCCEEDED(factory->CreatePathGeometry(geometry.GetAddressOf()))) {
          ...
          if (SUCCEEDED(geometry->Open(sink.GetAddressOf()))) {
            // Area Fill Path
            sink->BeginFigure(D2D1::Point2F(points[0].x, bottom),
            ...
            Direct2D::CreateBrushFromGradientOrColor(
                context, elementRect, &m_FillGradient, m_FillColor,
        ...
        // Top Line Path (separately to ensure clean stroke)
        {
          ...
          if (SUCCEEDED(factory->CreatePathGeometry(lineGeom.GetAddressOf()))) {
            ...
            if (SUCCEEDED(lineGeom->Open(sink.GetAddressOf()))) {
              ...
              Direct2D::CreateBrushFromGradientOrColor(
                  context, elementRect, &m_LineGradient, m_LineColor, 1.0f,
                  lineBrush.GetAddressOf());
              if (lineBrush) {
                context->DrawGeometry(lineGeom.Get(), lineBrush.Get(),
                                      m_LineWidth);
              }
            }
          }
...
bool AreaGraphElement::HitTest(int x, int y) {
  ...
  for (int i = 0; i < numPoints; ++i) {
    ...
    int capacity = (m_MaxPoints > numPoints) ? m_MaxPoints : numPoints;
    float dx = (capacity > 1)
                   ? (width - 1.0f) / static_cast<float>(capacity - 1)
                   : 0.0f;

    float px = m_GraphStartLeft
                   ? (left + static_cast<float>(numPoints - 1 - i) * dx)
                   : (right - static_cast<float>(numPoints - 1 - i) * dx);
    float py = !m_Flip ? (bottom - norm * (height - 1.0f))
                       : (top + norm * (height - 1.0f));
    points.push_back(D2D1::Point2F(px, py));
  }

  const D2D1_POINT_2F p = D2D1::Point2F(targetX, targetY);
  const bool fillVisible =
      (m_FillAlpha > 0) || (m_FillGradient.type != GRADIENT_NONE);
