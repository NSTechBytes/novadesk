...
HCURSOR CursorManager::LoadCustomCursorFile(const std::wstring &fullPath) {
  ...
  HCURSOR cursor = reinterpret_cast<HCURSOR>(LoadImageW(
      nullptr, fullPath.c_str(), IMAGE_CURSOR, 0, 0, LR_LOADFROMFILE));
  if (cursor) {
...
HCURSOR CursorManager::GetCursorForElement(Element *element) {
  ...
  if (!dir.empty() && !name.empty()) {
    ...
    if (std::filesystem::exists(candidate)) {
      if (HCURSOR c = LoadCustomCursorFile(candidate))
        return c;
    }
    ...
    for (const auto &ext : exts) {
      ...
      if (std::filesystem::exists(withExt)) {
        if (HCURSOR c = LoadCustomCursorFile(withExt))
          return c;
      }
    }
  }
