...
InputBoxElement::InputBoxElement(const std::wstring &id, int x, int y,
...
InputBoxElement::CreateTextLayout(ID2D1DeviceContext *context,
                                  const std::wstring &text, float layoutW,
                                  float layoutH) const {
  (void)context;
  ...
  pFormat->SetWordWrapping(m_Multiline ? DWRITE_WORD_WRAPPING_WRAP
                                       : DWRITE_WORD_WRAPPING_NO_WRAP);

  Microsoft::WRL::ComPtr<IDWriteTextLayout> pLayout;
  hr = Direct2D::GetWriteFactory()->CreateTextLayout(
      text.c_str(), (UINT32)text.length(), pFormat.Get(), layoutW, layoutH,
...
float InputBoxElement::CaretIndexToX(UINT32 index) const {
  ...
  std::wstring text =
      m_Password ? std::wstring(m_Text.size(), L'\x2022') : m_Text;
  if (index > text.size())
    index = (UINT32)text.size();

  auto layout = const_cast<InputBoxElement *>(this)->CreateTextLayout(
      nullptr, text, w, h);
  if (!layout)
    return content.left;
...
void InputBoxElement::CaretIndexToXY(UINT32 index, float &outX, float &outY,
                                     float &outH) const {
  D2D1_RECT_F content = GetContentRect();
  float w = content.right - content.left;
  float h = content.bottom - content.top;
...
void InputBoxElement::EnsureCaretVisible() {
  ...
  if (m_Multiline) {
    std::wstring text =
        m_Password ? std::wstring(m_Text.size(), L'\x2022') : m_Text;
    auto layout = const_cast<InputBoxElement *>(this)->CreateTextLayout(
        nullptr, text, contentW, contentH);
    if (!layout) {
      m_ScrollOffset = 0.0f;
      return;
    }
    ...
    // A small margin (in DIPs) to ensure the caret is not clipped at the right
    // edge.
    constexpr float kCaretMargin = 2.0f;

    // If the text fits entirely (including the caret margin), no scrolling
    // needed.
    ...
    if (caretScreenX < 0.0f) {
      ...
      // Caret scrolled off the right edge: snap so caret is at the right edge
      // minus margin.
...
void InputBoxElement::NormalizeSelection(UINT32 &outStart,
                                         UINT32 &outEnd) const {
...
InputBoxElement::HandleCharResult InputBoxElement::HandleChar(wchar_t ch) {
  ...
  if (ch == 3 /*Ctrl+C*/ || ch == 22 /*Ctrl+V*/ || ch == 1 /*Ctrl+A*/ ||
      ch == 24 /*Ctrl+X*/)
    return HandleCharResult::Ignored;
...
bool InputBoxElement::HandleKeyDown(WPARAM vk, bool shift, bool control) {
  ...
  case VK_RIGHT: {
    UINT32 newPos =
        (m_CaretPos < m_Text.size()) ? m_CaretPos + 1 : (UINT32)m_Text.size();
    if (shift) {
      if (!HasSelection())
        m_SelectionAnchor = m_CaretPos;
      m_CaretPos = newPos;
  ...
  case VK_DOWN: {
    if (m_Multiline) {
      ...
      if (layout) {
        ...
        float targetY = (vk == VK_UP) ? (caretY - content.top - caretH / 2.0f)
                                      : (caretY - content.top + caretH * 1.5f);
        float targetX = caretX - content.left;

        BOOL isTrailing = FALSE;
        BOOL isInside = FALSE;
        DWRITE_HIT_TEST_METRICS metrics{};
        layout->HitTestPoint(targetX, targetY, &isTrailing, &isInside,
                             &metrics);
...
void InputBoxElement::HandleMouseUp() { m_IsDragging = false; }
...
void InputBoxElement::Render(ID2D1DeviceContext *context) {
  ...
  // Background (uses m_SolidColor / m_CornerRadius set via
  // SetSolidColor/SetCornerRadius)
  ...
  if ((m_HasFillColor && m_FillAlpha > 0) ||
    ...
    D2D1_RECT_F fillRect =
        D2D1::RectF((float)m_X, (float)m_Y, (float)(m_X + GetWidth()),
                    (float)(m_Y + GetHeight()));

    Microsoft::WRL::ComPtr<ID2D1Brush> fillBrush;
    if (Direct2D::CreateBrushFromGradientOrColor(
            context, fillRect, &m_FillGradient, m_FillColor,
            m_FillAlpha / 255.0f, fillBrush.GetAddressOf()) &&
        fillBrush) {
      if (m_BorderRadius > 0.0f) {
        D2D1_ROUNDED_RECT roundedRect =
            D2D1::RoundedRect(fillRect, m_BorderRadius, m_BorderRadius);
  ...
  if (m_Focused &&
      (m_HasBorderFocusColor || m_BorderFocusGradient.type != GRADIENT_NONE)) {
  ...
  if (m_BorderWidth > 0.0f &&
      (activeAlpha > 0 || activeGradient->type != GRADIENT_NONE)) {
    D2D1_RECT_F borderRectF =
        D2D1::RectF((float)m_X, (float)m_Y, (float)(m_X + GetWidth()),
                    (float)(m_Y + GetHeight()));

    Microsoft::WRL::ComPtr<ID2D1Brush> borderBrush;
    if (Direct2D::CreateBrushFromGradientOrColor(
            context, borderRectF, activeGradient, activeColor,
            activeAlpha / 255.0f, borderBrush.GetAddressOf()) &&
        borderBrush) {
      if (m_BorderRadius > 0.0f) {
        D2D1_ROUNDED_RECT borderRect =
            D2D1::RoundedRect(borderRectF, m_BorderRadius, m_BorderRadius);
        context->DrawRoundedRectangle(borderRect, borderBrush.Get(),
                                      m_BorderWidth, nullptr);
      } else {
        context->DrawRectangle(borderRectF, borderBrush.Get(), m_BorderWidth,
                               nullptr);
      }
    }
  }
  ...
  if (Direct2D::GetFactory()) {
    Direct2D::GetFactory()->CreateRectangleGeometry(content,
                                                    clipGeom.GetAddressOf());
  }
  ...
  const std::wstring &renderText =
      m_Password ? (m_Text.empty() ? m_Placeholder : maskedText) : drawText;
    ...
    if (layout) {
      ...
      if (m_Focused && HasSelection() && !showingPlaceholder) {
        ...
        if (s < e && e <= (UINT32)renderText.size()) {
          ...
          layout->HitTestTextRange(s, e - s, 0.0f, 0.0f, nullptr, 0,
                                   &actualCount);
          if (actualCount > 0) {
            std::vector<DWRITE_HIT_TEST_METRICS> metrics(actualCount);
            layout->HitTestTextRange(s, e - s, 0.0f, 0.0f, metrics.data(),
                                     actualCount, &actualCount);

            Microsoft::WRL::ComPtr<ID2D1Brush> selBrush;
            if (Direct2D::CreateBrushFromGradientOrColor(
                    context, content, &m_SelectionGradient, m_SelectionColor,
                    m_SelectionAlpha / 255.0f, selBrush.GetAddressOf()) &&
                selBrush) {
              for (UINT32 i = 0; i < actualCount; ++i) {
                D2D1_RECT_F r = D2D1::RectF(
                    content.left + metrics[i].left -
                        (m_Multiline ? 0.0f : m_ScrollOffset),
                    content.top + metrics[i].top -
                        (m_Multiline ? m_ScrollOffset : 0.0f),
      ...
      const GradientInfo *textGrad =
          showingPlaceholder ? &m_PlaceholderGradient : &m_FontGradient;

      Microsoft::WRL::ComPtr<ID2D1Brush> textBrush;
      if (Direct2D::CreateBrushFromGradientOrColor(context, content, textGrad,
                                                   color, alpha / 255.0f,
        ...
        context->DrawTextLayout(
            D2D1::Point2F(content.left - (m_Multiline ? 0.0f : m_ScrollOffset),
                          content.top - (m_Multiline ? m_ScrollOffset : 0.0f)),
  ...
  if (m_Focused && m_CaretVisible) {
    ...
    if (Direct2D::CreateBrushFromGradientOrColor(
            context, caretRect, &m_CaretGradient, m_CaretColor,
            m_CaretAlpha / 255.0f, caretBrush.GetAddressOf()) &&
        caretBrush) {
...
void InputBoxElement::SaveUndoState() {
  if (!m_UndoStack.empty()) {
    ...
    if (top.text == m_Text && top.caretPos == m_CaretPos &&
