...
void LineElement::SetLineColors(const std::vector<COLORREF> &colors,
...
bool LineElement::MapPoint(int dataIndex, int pointIndex, int totalPoints,
                           int capacityPoints, float minValue, float maxValue,
                           D2D1_POINT_2F &outPoint) {
  if (dataIndex < 0 || dataIndex >= (int)m_DataSets.size() ||
      totalPoints <= 0 || pointIndex < 0 || pointIndex >= totalPoints) {
    return false;
  }
  ...
  float normalized =
      (valueRange <= 0.000001f) ? 0.5f : ((value - minValue) / valueRange);
  normalized = std::max(0.0f, std::min(1.0f, normalized));

  if (!m_GraphHorizontalOrientation) {
    const float dx = (capacityPoints > 1)
                         ? ((w - 1.0f) / (float)(capacityPoints - 1))
                         : 0.0f;
    const int axisIndex = m_GraphStartLeft
                              ? (totalPoints - 1 - pointIndex)
                              : ((capacityPoints - totalPoints) + pointIndex);
    float x = left + (float)axisIndex * dx;
    float y = !m_Flip ? (top + (h - 1.0f) - normalized * (h - 1.0f))
                      : (top + normalized * (h - 1.0f));
    outPoint = D2D1::Point2F(x, y);
    return true;
  }

  const float dy =
      (capacityPoints > 1) ? ((h - 1.0f) / (float)(capacityPoints - 1)) : 0.0f;
  const int axisIndex = !m_Flip ? ((capacityPoints - totalPoints) + pointIndex)
                                : (totalPoints - 1 - pointIndex);
  float y = top + (float)axisIndex * dy;
  float x = m_GraphStartLeft ? (left + normalized * (w - 1.0f))
                             : (left + (w - 1.0f) - normalized * (w - 1.0f));
  outPoint = D2D1::Point2F(x, y);
  return true;
}

float LineElement::DistancePointToSegment(const D2D1_POINT_2F &p,
...
bool LineElement::HitTest(int x, int y) {
  ...
  for (int i = 0; i < m_LineCount && i < (int)m_DataSets.size(); ++i) {
    ...
    const int capacityPoints =
        (m_MaxPoints > totalPoints) ? m_MaxPoints : totalPoints;
    ...
    if (!m_GraphHorizontalOrientation) {
      const float baseY =
          !m_Flip ? ((float)m_Y + (float)GetHeight() - 1.0f) : (float)m_Y;
      base = D2D1::Point2F(a.x, baseY);
    } else {
      const float baseX = m_GraphStartLeft
                              ? (float)m_X
                              : ((float)m_X + (float)GetWidth() - 1.0f);
...
void LineElement::Render(ID2D1DeviceContext *context) {
  ...
  if (factory) {
    ...
    if (SUCCEEDED(factory->CreateStrokeStyle(strokeProps1, nullptr, 0,
                                             strokeStyle1.GetAddressOf()))) {
      strokeStyle = strokeStyle1;
    } else
#endif
    {
      D2D1_STROKE_STYLE_PROPERTIES strokeProps = D2D1::StrokeStyleProperties();
      factory->CreateStrokeStyle(strokeProps, nullptr, 0,
  ...
  if (m_HorizontalLines) {
    ...
    const D2D1_RECT_F elementRect = D2D1::RectF(
        (float)m_X, (float)m_Y, (float)m_X + width, (float)m_Y + height);
    Direct2D::CreateBrushFromGradientOrColor(
        context, elementRect, &m_HorizontalLineGradient, m_HorizontalLineColor,
        m_HorizontalLineAlpha / 255.0f, markerBrush.GetAddressOf());
    if (markerBrush) {
      const float left = (float)m_X;
      ...
      for (int i = 1; i <= markerCount; ++i) {
        ...
        context->DrawLine(D2D1::Point2F(left, y), D2D1::Point2F(right, y),
                          markerBrush.Get(), 1.0f, strokeStyle.Get());
      }
    }
  }
  ...
  for (int i = 0; i < m_LineCount && i < (int)m_DataSets.size(); ++i) {
    ...
    if (!m_GraphHorizontalOrientation) {
      ...
      const float baseX =
          m_GraphStartLeft ? (float)m_X : ((float)m_X + width - 1.0f);
    ...
    Direct2D::CreateBrushFromGradientOrColor(
        context, elementRect, lineGradient, m_LineColors[(size_t)i],
        m_LineAlphas[(size_t)i] / 255.0f, brush.GetAddressOf());

    if (brush) {
      context->DrawGeometry(geometry.Get(), brush.Get(), strokeWidth,
                            strokeStyle.Get());
    }
  }
