...
void FlexLayoutEngine::ApplyLayout(Element *container,
                                   const FlexLayoutConfig &config) {
  if (!container)
    return;
  ...
  // Logging::Log(LogLevel::Debug, L"[FLEX_LAYOUT] ApplyLayout on '%s': bounds
  // W=%d H=%d, padding L=%d T=%d R=%d B=%d, inner W=%d H=%d",
  //     container->GetId().c_str(), bounds.Width, bounds.Height,
  //     config.paddingLeft, config.paddingTop, config.paddingRight,
  //     config.paddingBottom, innerW, innerH);
  ...
  const bool isReverse =
      (flexDir == L"rowreverse" || flexDir == L"columnreverse");
  ...
  for (Element *child : orderedItems) {
    ...
    if (isHorizontal) {
      ...
      if (align == L"center") {
        ...
        // CSS Spec: For flex items, both "stretch" and "normal" behave
        // identically They stretch items to fill the cross axis
        crossPos = 0;
        if (innerH > 0) {
          childH = innerH;
      ...
      child->SetPosition(config.paddingLeft + cursor,
                         config.paddingTop + (crossPos < 0 ? 0 : crossPos));
      ...
      child->SetPosition(config.paddingLeft + (crossPos < 0 ? 0 : crossPos),
                         config.paddingTop + cursor);
