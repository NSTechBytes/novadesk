...
bool ShowInputBoxContextMenu(Widget &widget, InputBoxElement *inputElem,
  ...
  if (prevFocused && prevFocused != inputElem) {
    if (prevFocused->m_OnBlurCallbackId != -1)
      JSEngine::CallEventCallback(prevFocused->m_OnBlurCallbackId, &widget,
                                  nullptr);
  ...
  if (!inputElem->IsFocused()) {
    ...
    if (inputElem->m_OnFocusCallbackId != -1)
      JSEngine::CallEventCallback(inputElem->m_OnFocusCallbackId, &widget,
                                  nullptr);
  }
  ...
  bool canPaste =
      novadesk::shared::system::ClipboardGetText(clipText) && !clipText.empty();

  std::vector<MenuItem> menuItems;
  menuItems.push_back(
      {L"&Undo", CMD_UNDO, false, false, !inputElem->CanUndo()});
  menuItems.push_back(
      {L"&Redo", CMD_REDO, false, false, !inputElem->CanRedo()});
  ...
  menuItems.push_back(
      {L"Cu&t", CMD_CUT, false, false, !(hasSelection && !isPassword)});
  menuItems.push_back(
      {L"&Copy", CMD_COPY, false, false, !(hasSelection && !isPassword)});
  ...
  menuItems.push_back({L"Select &All", CMD_SELECTALL, false, false,
                       inputElem->GetText().empty()});
