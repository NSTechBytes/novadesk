...
WidgetDropTarget::WidgetDropTarget(Widget *widget) : m_Widget(widget) {}

void WidgetDropTarget::ExtractFiles(IDataObject *pDataObj,
...
Element *WidgetDropTarget::HitTestDropTarget(int screenX, int screenY,
                                             int &outLocalX, int &outLocalY) {
  outLocalX = 0;
  outLocalY = 0;
  ...
  for (auto it = elements.rbegin(); it != elements.rend(); ++it) {
    ...
    if (elem->IsContainer()) {
      ...
      if (WidgetLayoutHelper::HitTestContainerChildrenDetailed(
              *m_Widget, elem, x, y, WM_MOUSEMOVE, 0, hitChild, actionElem,
              mouseActionElem, tooltipElem)) {
...
STDMETHODIMP WidgetDropTarget::DragEnter(IDataObject *pDataObj,
  ...
  if (target && m_AcceptsDrop) {
    *pdwEffect &= (DROPEFFECT_COPY | DROPEFFECT_MOVE | DROPEFFECT_LINK);
    ...
    if (target->m_OnDragEnterCallbackId != -1) {
      ...
      data.effect = (*pdwEffect & DROPEFFECT_COPY)
                        ? "copy"
                        : ((*pdwEffect & DROPEFFECT_MOVE) ? "move" : "link");
      JSEngine::CallDropCallback(target->m_OnDragEnterCallbackId, m_Widget,
                                 &data);
    }
  } else {
...
STDMETHODIMP WidgetDropTarget::DragOver(DWORD grfKeyState, POINTL pt,
                                        DWORD *pdwEffect) {
  if (!pdwEffect)
    return E_INVALIDARG;
  ...
  if (target != m_CurrentHoverElement) {
    ...
    if (m_CurrentHoverElement &&
        m_CurrentHoverElement->m_OnDragLeaveCallbackId != -1) {
      JSEngine::DropEventData data;
      data.files = m_CachedFiles;
      data.screenX = pt.x;
      data.screenY = pt.y;
      ...
      data.clientX = clientPt.x;
      data.clientY = clientPt.y;
      JSEngine::CallDropCallback(m_CurrentHoverElement->m_OnDragLeaveCallbackId,
                                 m_Widget, &data);
    }
    ...
    *pdwEffect &= (DROPEFFECT_COPY | DROPEFFECT_MOVE | DROPEFFECT_LINK);
    ...
    if (target->m_OnDragOverCallbackId != -1) {
      ...
      data.effect = (*pdwEffect & DROPEFFECT_COPY)
                        ? "copy"
                        : ((*pdwEffect & DROPEFFECT_MOVE) ? "move" : "link");
      JSEngine::CallDropCallback(target->m_OnDragOverCallbackId, m_Widget,
                                 &data);
    }
  } else {
    *pdwEffect = DROPEFFECT_NONE;
...
STDMETHODIMP WidgetDropTarget::Drop(IDataObject *pDataObj, DWORD grfKeyState,
