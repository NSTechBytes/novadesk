...
namespace
{
    ...
    Widget::WindowAnimationTarget SampleWindowAnimationTarget(const Widget::WindowAnimationTarget &from, const Widget::WindowAnimationTarget &to, float p)
    {
        ...
        if (to.hasWidth) { out.hasWidth = true; out.width = lerp(from.width, to.width); }
        if (to.hasHeight) { out.hasHeight = true; out.height = lerp(from.height, to.height); }
        if (to.hasOpacity) { out.hasOpacity = true; out.opacity = lerp(from.opacity, to.opacity); }
    ...
    void MergeWindowAnimationTarget(Widget::WindowAnimationTarget &base, const Widget::WindowAnimationTarget &patch)
    {
        if (patch.hasX) { base.hasX = true; base.x = patch.x; }
        if (patch.hasY) { base.hasY = true; base.y = patch.y; }
        if (patch.hasWidth) { base.hasWidth = true; base.width = patch.width; }
        if (patch.hasHeight) { base.hasHeight = true; base.height = patch.height; }
        if (patch.hasOpacity) { base.hasOpacity = true; base.opacity = patch.opacity; }
    ...
    Widget::WindowAnimationTarget SampleWindowKeyframeTrack(
        const std::vector<float> &offsets,
        const std::vector<std::wstring> &easings,
        const std::vector<Widget::WindowAnimationTarget> &stops,
        float t,
        const std::wstring &defaultEasing)
    {
        if (offsets.empty() || stops.empty())
...
Widget::WindowAnimationTarget WidgetAnimationHelper::CaptureWindowAnimationState(const Widget &widget)
{
    ...
    std::vector<Widget::WindowAnimationTarget> &resolved)
{
    ...
    for (const Widget::WindowAnimationKeyframe &kf : keyframes)
    {
        ...
        if (unionMask.hasWidth) { stop.hasWidth = true; stop.width = carry.width; }
        if (unionMask.hasHeight) { stop.hasHeight = true; stop.height = carry.height; }
        if (unionMask.hasOpacity) { stop.hasOpacity = true; stop.opacity = carry.opacity; }
...
void WidgetAnimationHelper::ApplyWindowAnimationTarget(Widget &widget, const Widget::WindowAnimationTarget &target)
{
    ...
    if (target.hasX) { x = static_cast<int>(std::lround(target.x)); posChanged = true; }
    if (target.hasY) { y = static_cast<int>(std::lround(target.y)); posChanged = true; }
    if (target.hasWidth) { w = static_cast<int>(std::lround(target.width)); posChanged = true; }
    if (target.hasHeight) { h = static_cast<int>(std::lround(target.height)); posChanged = true; }
    ...
    if (posChanged)
    {
        ...
        if (target.hasWidth && w > 0) widget.m_Options.m_WDefined = true;
        if (target.hasHeight && h > 0) widget.m_Options.m_HDefined = true;

        if (widget.m_hWnd)
        {
            SetWindowPos(widget.m_hWnd, NULL, x, y, w, h, SWP_NOZORDER | SWP_NOACTIVATE);
        }
    }

    if (target.hasOpacity)
    {
        BYTE op = static_cast<BYTE>(std::clamp(static_cast<long>(std::lround(target.opacity * 255.0f)), 0L, 255L));
        widget.m_Options.windowOpacity = op;
    }

    if (target.hasBackgroundColor)
    {
        BYTE r = static_cast<BYTE>(std::clamp(static_cast<long>(std::lround(target.bgColorR)), 0L, 255L));
        BYTE g = static_cast<BYTE>(std::clamp(static_cast<long>(std::lround(target.bgColorG)), 0L, 255L));
        BYTE b = static_cast<BYTE>(std::clamp(static_cast<long>(std::lround(target.bgColorB)), 0L, 255L));
        widget.m_Options.color = RGB(r, g, b);
        widget.m_Options.bgAlpha = static_cast<BYTE>(std::clamp(static_cast<long>(std::lround(target.bgAlpha)), 0L, 255L));
    }
}
    ...
    int iterationCount)
{
    ...
    ResolveKeyframeStops(element, keyframes, anim.keyframeOffsets, anim.keyframeEasings, anim.resolvedStops);
    ...
    int iterationCount)
{
    ...
    ResolveWindowKeyframeStops(widget, keyframes, anim.keyframeOffsets, anim.keyframeEasings, anim.resolvedStops);
    if (anim.resolvedStops.size() < 2)
        return;
...
void WidgetAnimationHelper::StepAnimations(Widget &widget)
{
    ...
    for (auto it = widget.m_WindowAnimations.begin(); it != widget.m_WindowAnimations.end();)
    {
        const DWORD elapsed = now - it->startTick;
        float t = static_cast<float>(elapsed) / static_cast<float>(it->durationMs);
        if (t < 0.0f) t = 0.0f;
        ...
        if (it->useKeyframes)
            sampled = SampleWindowKeyframeTrack(it->keyframeOffsets, it->keyframeEasings, it->resolvedStops, t, it->easing);
        else
        {
            const float p = AnimationEasing::Evaluate(t, it->easing);
    ...
    if (widget.m_Animations.empty() && widget.m_WindowAnimations.empty() && widget.m_hWnd)
        KillTimer(widget.m_hWnd, kTimerId);
}
