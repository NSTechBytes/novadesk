...
constexpr int W = 320, H = 346, SV_WIDTH = 320, SV_HEIGHT = 190, HUEY = 212,
              EDITY = 253, MODEY = 303, MODEBOTTOM = 338;
constexpr int INPUT_HEIGHT = 30, RGB_INPUT_WIDTH = 72, HEX_INPUT_WIDTH = 234;
constexpr int MAGNIFIER_SIZE = 124, MAGNIFIER_BORDER = 0,
...
COLORREF HsvToColor(float hue, float saturation, float value) {
  ...
  const float secondary =
      chroma * (1.0f - std::fabs(std::fmod(h, 2.0f) - 1.0f));
  ...
  return RGB(static_cast<BYTE>((red + match) * 255),
             static_cast<BYTE>((green + match) * 255),
             static_cast<BYTE>((blue + match) * 255));
}
...
bool TryParseHexColor(const wchar_t *text, COLORREF &color) {
  ...
  color = RGB(values[0] * 16 + values[1], values[2] * 16 + values[3],
...
void DrawSmoothEllipse(ID2D1RenderTarget *target, float x, float y, float width,
                       float height, COLORREF fill, COLORREF outline,
...
void DrawEyedropperSvg(ID2D1RenderTarget *target, int left, int top, int size) {
  ...
  static const char svg[] =
      "<svg viewBox=\"0 0 16 16\"><path fill=\"#444\" d=\"M15 "
      "1c-1.8-1.8-3.7-0.7-4.6 0.1-0.4 0.4-0.7 0.9-0.7 1.5v0c0 1.1-1.1 1.8-2.1 "
      "1.5l-0.1-0.1-0.7 0.8 0.7 0.7-6 6-0.8 2.3-0.7 0.7 1.5 1.5 0.8-0.8 "
      "2.3-0.8 6-6 0.7 0.7 0.7-0.6-0.1-0.2c-0.3-1 0.4-2.1 1.5-2.1v0c0.6 0 "
      "1.1-0.2 1.4-0.6 0.9-0.9 2-2.8 0.2-4.6zM3.9 13.6l-2 0.7-0.2 0.1 0.1-0.2 "
      "0.7-2 5.8-5.8 1.5 1.5-5.9 5.7z\"/></svg>";
  ...
  for (NSVGshape *shape = image->shapes; shape; shape = shape->next) {
    for (NSVGpath *path = shape->paths; path; path = path->next) {
      ...
      for (int i = 0; i < path->npts - 1; i += 3) {
        ...
        sink->AddBezier(D2D1::BezierSegment(D2D1::Point2F(control1X, control1Y),
                                            D2D1::Point2F(control2X, control2Y),
                                            D2D1::Point2F(endX, endY)));
        currentX = endX;
        currentY = endY;
      }
      sink->EndFigure(path->closed ? D2D1_FIGURE_END_CLOSED
  ...
  if (Direct2D::CreateSolidBrush(target, RGB(68, 68, 68), 1.0f,
                                 iconBrush.GetAddressOf()))
    target->FillGeometry(iconPath.Get(), iconBrush.Get());
}
} // namespace

ColorPickerPopup::ColorPickerPopup(Widget *w, ColorPickerElement *p)
    : m_Widget(w), m_Picker(p) {
  SetRGB(p->GetColor(), false);
}
...
void ColorPickerPopup::Show() {
  ...
  m_hWnd = CreateWindowExW(
      WS_EX_TOOLWINDOW | WS_EX_TOPMOST, L"NovadeskColorPickerPopup", L"",
      WS_POPUP | WS_BORDER | WS_CLIPCHILDREN, x, y, W, H, m_Widget->GetWindow(),
  ...
  if (m_hWnd) {
    ...
    if (m_Picker && m_Picker->m_OnOpenCallbackId != -1) {
      ...
      JSEngine::CallEventCallbackWithText(m_Picker->m_OnOpenCallbackId,
                                          m_Widget, s);
    }
  } else {
...
void ColorPickerPopup::Close() {
  if (m_hWnd && !m_Canceled && m_Picker &&
    ...
    COLORREF c = HSV();
    swprintf_s(s, L"#%02X%02X%02X", GetRValue(c), GetGValue(c), GetBValue(c));
    JSEngine::CallEventCallbackWithText(m_Picker->m_OnCloseCallbackId, m_Widget,
                                        s);
  }
  FlushWidgetRedraw();
...
void ColorPickerPopup::StartEyedropper() {
  ...
  if (m_Picker && m_Picker->m_OnEyedropperOpenCallbackId != -1) {
    JSEngine::CallEventCallback(m_Picker->m_OnEyedropperOpenCallbackId,
                                m_Widget);
  }
}
...
void ColorPickerPopup::ShowEyedropperMagnifier(POINT screenPosition) {
  ...
  if (!m_Magnifier) {
    m_Magnifier = CreateWindowExW(
        WS_EX_TOOLWINDOW | WS_EX_TOPMOST | WS_EX_NOACTIVATE | WS_EX_TRANSPARENT,
        L"NovadeskColorPickerMagnifier", L"", WS_POPUP, 0, 0, MAGNIFIER_SIZE,
        MAGNIFIER_SIZE, m_hWnd, nullptr, GetModuleHandleW(nullptr), this);
  }
  if (!m_Magnifier)
    return;
  ...
  if (!m_MagnifierFrameDc) {
    ...
    if (screenDc) {
      ...
      m_MagnifierFrameBitmap =
          CreateCompatibleBitmap(screenDc, MAGNIFIER_SIZE, MAGNIFIER_SIZE);
      if (m_MagnifierFrameDc && m_MagnifierFrameBitmap)
        m_MagnifierFrameOldBitmap =
            SelectObject(m_MagnifierFrameDc, m_MagnifierFrameBitmap);
      else {
  ...
  GetMonitorInfoW(MonitorFromPoint(screenPosition, MONITOR_DEFAULTTONEAREST),
                  &monitorInfo);
  ...
  SetWindowPos(m_Magnifier, HWND_TOPMOST, left, top, MAGNIFIER_SIZE,
               MAGNIFIER_SIZE, positionFlags);
}
...
void ColorPickerPopup::UpdateEyedropperSample(POINT screenPosition) {
  ...
  if (screenDc) {
    if (m_MagnifierFrameDc) {
      ...
      FillRect(m_MagnifierFrameDc, &frame,
               static_cast<HBRUSH>(GetStockObject(BLACK_BRUSH)));
      const int previousStretchMode =
          SetStretchBltMode(m_MagnifierFrameDc, COLORONCOLOR);
      StretchBlt(m_MagnifierFrameDc, MAGNIFIER_BORDER, MAGNIFIER_BORDER,
                 MAGNIFIER_SIZE - MAGNIFIER_BORDER * 2,
                 MAGNIFIER_SIZE - MAGNIFIER_BORDER * 2, screenDc,
                 screenPosition.x - MAGNIFIER_SOURCE_SIZE / 2,
                 screenPosition.y - MAGNIFIER_SOURCE_SIZE / 2,
                 MAGNIFIER_SOURCE_SIZE, MAGNIFIER_SOURCE_SIZE,
                 SRCCOPY | CAPTUREBLT);
      ...
      Rectangle(m_MagnifierFrameDc, center - 4, center - 4, center + 5,
                center + 5);
    }
    ReleaseDC(nullptr, screenDc);
  }
...
void ColorPickerPopup::ApplyEyedropperSelection(POINT screenPosition) {
  ...
  // If the cursor is directly over the popup's own SV gradient, map
  // mathematically
  if (m_hWnd) {
    POINT localPt = screenPosition;
    ScreenToClient(m_hWnd, &localPt);
    if (localPt.x >= 0 && localPt.x < SV_WIDTH && localPt.y >= 0 &&
        localPt.y < SV_HEIGHT) {
      const float s = Clamp(localPt.x / static_cast<float>(SV_WIDTH - 1));
      const float v =
          Clamp(1.0f - localPt.y / static_cast<float>(SV_HEIGHT - 1));
      pickedColor = HsvToColor(m_H, s, v);
      SetHSV(m_H, s, v, true);
    } else if (localPt.x >= 120 && localPt.x < 300 && localPt.y >= HUEY &&
               localPt.y < HUEY + 20) {
  ...
  if (pickedColor != CLR_INVALID && m_Picker &&
      m_Picker->m_OnEyedropperPickCallbackId != -1) {
    wchar_t s[8];
    swprintf_s(s, L"#%02X%02X%02X", GetRValue(pickedColor),
               GetGValue(pickedColor), GetBValue(pickedColor));
    JSEngine::CallEventCallbackWithText(m_Picker->m_OnEyedropperPickCallbackId,
                                        m_Widget, s);
  }
}
...
void ColorPickerPopup::PaintEyedropperMagnifier(HDC dc) {
  ...
  if (m_MagnifierFrameDc)
    BitBlt(dc, 0, 0, client.right, client.bottom, m_MagnifierFrameDc, 0, 0,
           SRCCOPY);
  else
    FillRect(dc, &client, static_cast<HBRUSH>(GetStockObject(BLACK_BRUSH)));
}

COLORREF ColorPickerPopup::HSV() const { return HsvToColor(m_H, m_S, m_V); }

void ColorPickerPopup::SetRGB(COLORREF c, bool n) {
  float r = GetRValue(c) / 255.f, g = GetGValue(c) / 255.f,
        b = GetBValue(c) / 255.f, mx = (std::max)(r, (std::max)(g, b)),
        mn = (std::min)(r, (std::min)(g, b)), d = mx - mn;
  m_V = mx;
  m_S = mx ? d / mx : 0;
  if (d >= 0.015f) {
    if (mx == r)
...
void ColorPickerPopup::Paint(HDC targetDc) {
  ...
  const COLORREF bgColor =
      m_Picker ? m_Picker->m_PopupBackground : RGB(255, 255, 255);
  const COLORREF accentColor =
      m_Picker ? m_Picker->m_PopupAccentColor : RGB(0, 0, 0);
  ...
  StretchDIBits(dc, 0, 0, SV_WIDTH, SV_HEIGHT, 0, 0, SV_WIDTH, SV_HEIGHT,
                m_SaturationValuePixels.data(), &bitmapInfo, DIB_RGB_COLORS,
                SRCCOPY);
  ...
  DrawSmoothEllipse(vectorTarget.Get(), hueSelectorX - 10.0f,
                    hueSelectorY - 10.0f, 20.0f, 20.0f, RGB(255, 255, 255),
                    RGB(0, 0, 0));
  DrawSmoothEllipse(vectorTarget.Get(), hueSelectorX - 8.0f,
                    hueSelectorY - 8.0f, 16.0f, 16.0f,
                    HsvToColor(m_H, 1.0f, 1.0f), RGB(255, 255, 255));
  // Current-color swatch circle
  COLORREF c = HSV();
  RECT sw{58, 201, 102, 245};
  DrawSmoothEllipse(vectorTarget.Get(), static_cast<float>(sw.left),
                    static_cast<float>(sw.top),
                    static_cast<float>(sw.right - sw.left),
                    static_cast<float>(sw.bottom - sw.top), c, c, 0.0f);
  // SV selector ring
  const float saturationValueX = m_S * (SV_WIDTH - 1.0f);
  const float saturationValueY = (1.0f - m_V) * (SV_HEIGHT - 1.0f);
  DrawSmoothEllipse(vectorTarget.Get(), saturationValueX - 8.0f,
                    saturationValueY - 8.0f, 16.0f, 16.0f, RGB(255, 255, 255),
                    RGB(0, 0, 0));
  // Eyedropper icon
  if (showEyedropper)
  ...
  if (showFormat) {
    ...
    const COLORREF hoverAccent =
        RGB(static_cast<BYTE>(GetRValue(accentColor) * 0.7f),
...
void ColorPickerPopup::EnsureSaturationValueBitmap() {
  if (m_SaturationValuePixels.size() ==
          static_cast<size_t>(SV_WIDTH * SV_HEIGHT) &&
      std::fabs(m_SaturationValueHue - m_H) < 0.0001f)
    return;
  ...
  for (int y = 0; y < SV_HEIGHT; ++y) {
    ...
    for (int x = 0; x < SV_WIDTH; ++x)
      m_SaturationValuePixels[y * SV_WIDTH + x] = ColorRefToDibPixel(
          HsvToColor(m_H, x / static_cast<float>(SV_WIDTH - 1), value));
  }
  m_SaturationValueHue = m_H;
}

LRESULT CALLBACK ColorPickerPopup::OutsideClickMouseHook(int code,
                                                         WPARAM message,
                                                         LPARAM data) {
  if (code == HC_ACTION && message == WM_LBUTTONDOWN && g_OutsideClickPopup &&
      !g_OutsideClickPopup->m_eye) {
    ...
    for (HWND window = clickedWindow; window; window = GetParent(window)) {
      if (window == g_OutsideClickPopup->m_hWnd ||
          window == g_OutsideClickPopup->m_Magnifier) {
        isInsidePicker = true;
        break;
      }
    }
...
LRESULT CALLBACK ColorPickerPopup::MagnifierWndProc(HWND h, UINT m, WPARAM w,
                                                    LPARAM l) {
  auto *popup =
      reinterpret_cast<ColorPickerPopup *>(GetWindowLongPtrW(h, GWLP_USERDATA));
  if (m == WM_NCCREATE) {
    popup = static_cast<ColorPickerPopup *>(
...
LRESULT ColorPickerPopup::Handle(UINT m, WPARAM w, LPARAM l) {
  ...
  if (m == WM_CREATE) {
    ...
    const DWORD rgbInputStyle =
        WS_CHILD | WS_VISIBLE | WS_BORDER | ES_NUMBER | ES_CENTER;
    const DWORD hexInputStyle =
        WS_CHILD | WS_BORDER | ES_AUTOHSCROLL | ES_CENTER;
    m_R = CreateWindowW(L"EDIT", L"", rgbInputStyle, 33, EDITY, RGB_INPUT_WIDTH,
                        INPUT_HEIGHT, m_hWnd, (HMENU)1, 0, 0);
    m_G = CreateWindowW(L"EDIT", L"", rgbInputStyle, 115, EDITY,
                        RGB_INPUT_WIDTH, INPUT_HEIGHT, m_hWnd, (HMENU)2, 0, 0);
    m_B = CreateWindowW(L"EDIT", L"", rgbInputStyle, 197, EDITY,
                        RGB_INPUT_WIDTH, INPUT_HEIGHT, m_hWnd, (HMENU)3, 0, 0);
    m_Hex =
        CreateWindowW(L"EDIT", L"", hexInputStyle, 33, EDITY, HEX_INPUT_WIDTH,
                      INPUT_HEIGHT, m_hWnd, (HMENU)4, 0, 0);
    if (m_Font) {
    ...
    SendMessageW(m_R, EM_SETMARGINS, EC_LEFTMARGIN | EC_RIGHTMARGIN,
                 MAKELPARAM(0, 0));
    SendMessageW(m_G, EM_SETMARGINS, EC_LEFTMARGIN | EC_RIGHTMARGIN,
                 MAKELPARAM(0, 0));
    SendMessageW(m_B, EM_SETMARGINS, EC_LEFTMARGIN | EC_RIGHTMARGIN,
                 MAKELPARAM(0, 0));
    SendMessageW(m_Hex, EM_SETMARGINS, EC_LEFTMARGIN | EC_RIGHTMARGIN,
                 MAKELPARAM(0, 0));
  ...
  if (m == WM_LBUTTONDOWN) {
    ...
    if (x >= 32 && x <= 308 && y >= MODEY && y <= MODEBOTTOM) {
      ...
      m_dragSV = true;
      SetCapture(m_hWnd);
      SetHSV(m_H, x / static_cast<float>(SV_WIDTH - 1),
             1 - y / static_cast<float>(SV_HEIGHT - 1));
    } else if (x >= 120 && x < 300 && y >= HUEY && y < HUEY + 20) {
      m_dragHue = true;
      SetCapture(m_hWnd);
      SetHSV((x - 120) / 179.0f, m_S, m_V);
    } else if (x < 45 && y >= 200 && y < 245 &&
               (!m_Picker || m_Picker->m_ShowEyedropper)) {
      m_eye = true;
  ...
  if (m == WM_MOUSEMOVE) {
    ...
    const bool formatHover =
        x >= 32 && x <= 308 && y >= MODEY && y <= MODEBOTTOM;
    ...
    if (m_eye) {
      ...
      if (m_dragSV)
        SetHSV(m_H, p.x / static_cast<float>(SV_WIDTH - 1),
               1 - p.y / static_cast<float>(SV_HEIGHT - 1));
      else
  ...
  if (m == WM_SETCURSOR) {
    ...
    if (LOWORD(l) == HTCLIENT) {
      ...
      if (cursor.x >= 32 && cursor.x <= 308 && cursor.y >= MODEY &&
          cursor.y <= MODEBOTTOM) {
  ...
  if (m == WM_KEYDOWN && w == VK_ESCAPE) {
    ...
    if (m_Picker && m_Picker->m_OnCancelCallbackId != -1) {
      wchar_t s[8];
      swprintf_s(s, L"#%02X%02X%02X", GetRValue(m_OriginalColor),
                 GetGValue(m_OriginalColor), GetBValue(m_OriginalColor));
      JSEngine::CallEventCallbackWithText(m_Picker->m_OnCancelCallbackId,
                                          m_Widget, s);
    }
