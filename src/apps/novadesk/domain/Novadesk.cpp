...
static RECT GetTrayIconRect(int trayId) {
  ...
  // Dynamically load Shell_NotifyIconGetRect for compatibility with MinGW and
  // older systems
  typedef HRESULT(WINAPI * PFN_Shell_NotifyIconGetRect)(
      const NOTIFYICONIDENTIFIER *, RECT *);
  ...
  if (!pfnChecked) {
    ...
    if (hShell32) {
      pfnGetRect = (PFN_Shell_NotifyIconGetRect)GetProcAddress(
          hShell32, "Shell_NotifyIconGetRect");
    }
    pfnChecked = true;
  }
...
static LRESULT CALLBACK TrayMouseHookProc(int nCode, WPARAM wParam,
                                          LPARAM lParam) {
...
static void ShowTrayMenuInternal(int trayId, const std::vector<MenuItem> *menu,
                                 const POINT *position);

static bool SendIpcCommand(HWND hWnd, const std::wstring &command,
...
int APIENTRY wWinMain(_In_ HINSTANCE hInstance,
  ...
  // Enable DPI awareness with runtime fallback for older SDK
  // headers/toolchains.
  ...
  if (lockAlreadyHeld) {
    ...
    if (argv && argc > 1) {
      ...
      if (hExisting) {
        ...
        if (listScripts) {
          ...
          if (!tempList.empty()) {
            handledCommand =
                SendIpcCommand(hExisting, L"list", tempList) || handledCommand;
            ...
            handledCommand =
                SendIpcCommand(hExisting, L"list", L"") || handledCommand;
          }
        }
        if (!refreshPath.empty()) {
          handledCommand = SendIpcCommand(hExisting, L"refresh", refreshPath) ||
                           handledCommand;
        }
        if (refreshAll) {
          handledCommand =
        ...
        for (const auto &p : loadPaths) {
          handledCommand =
              SendIpcCommand(hExisting, L"load", p) || handledCommand;
        }
        if (setHardwareAcceleration.has_value()) {
          handledCommand =
              SendIpcCommand(hExisting,
                             *setHardwareAcceleration
                                 ? L"set-hardware-acceleration-on"
                                 : L"set-hardware-acceleration-off",
                             L"") ||
              handledCommand;
        }
        if (setDebugging.has_value()) {
          handledCommand = SendIpcCommand(hExisting,
                                          *setDebugging ? L"set-debugging-on"
                                                        : L"set-debugging-off",
                                          L"") ||
                           handledCommand;
        }
        ...
        if (setSaveLogToFile.has_value()) {
          handledCommand =
              SendIpcCommand(hExisting,
                             *setSaveLogToFile ? L"set-save-log-to-file-on"
                                               : L"set-save-log-to-file-off",
                             L"") ||
              handledCommand;
        }
      }
    ...
    if (!handledCommand && !onlyInternalLockArg) {
      ...
      MessageBoxW(nullptr, message.c_str(), appTitle.c_str(),
  ...
  wcex.lpfnWndProc = [](HWND hWnd, UINT message, WPARAM wParam,
  ...
  g_trayMouseHook = SetWindowsHookEx(WH_MOUSE_LL, TrayMouseHookProc,
                                     GetModuleHandle(nullptr), 0);
  if (!g_trayMouseHook) {
    Logging::Log(LogLevel::Warn,
                 L"Failed to install tray mouse hook (Error: %d)",
                 GetLastError());
  }
  ...
  if (setHardwareAcceleration.has_value()) {
    Settings::SetGlobalBool("useHardwareAcceleration",
                            *setHardwareAcceleration);
...
int APIENTRY WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR,
                     int nCmdShow) {
  // Forward an empty command line; wWinMain now reads argv directly via
  // GetCommandLineW().
...
void TrayDestroy(int trayId) { RemoveTrayIcon(trayId); }
...
void InitTrayIcon(int trayId) {
  ...
  const std::wstring tip =
      state->toolTip.empty() ? std::wstring(szTitle) : state->toolTip;
  wcsncpy_s(state->nid.szTip, _countof(state->nid.szTip), tip.c_str(),
            _TRUNCATE);
...
void TraySetImage(int trayId, const std::wstring &path) {
  ...
  bool owned = false;
  if (!path.empty()) {
    newIcon = (HICON)LoadImageW(nullptr, path.c_str(), IMAGE_ICON, 0, 0,
                                LR_LOADFROMFILE | LR_DEFAULTSIZE);
    owned = (newIcon != nullptr);
    if (!newIcon) {
      Logging::Log(LogLevel::Warn, L"Tray setImage failed to load: %s",
                   path.c_str());
      return;
    }
  } else {
...
void TraySetToolTip(int trayId, const std::wstring &toolTip) {
  ...
  wcsncpy_s(state->nid.szTip, _countof(state->nid.szTip), toolTip.c_str(),
            _TRUNCATE);
  Shell_NotifyIconW(NIM_MODIFY, &state->nid);
}
