...
void WidgetLayoutHelper::SetLayoutConfig(Widget &widget, const std::wstring &id,
...
bool WidgetLayoutHelper::TryGetLayoutConfig(const Widget &widget,
...
bool WidgetLayoutHelper::IsLayoutContainer(const Widget &widget,
                                           const std::wstring &id) {
  if (id.empty())
    return false;
...
void WidgetLayoutHelper::ApplyLayoutForContainer(const Widget &widget,
                                                 Element *container) {
  if (!container)
    return;
...
void WidgetLayoutHelper::UpdateContainerForElement(
    Widget &widget, Element *element, const std::wstring &newContainerId) {
  if (!element)
    return;
  ...
  if (element->GetId() == newContainerId) {
    Logging::Log(LogLevel::Error, L"Container cannot self-reference: %s",
                 newContainerId.c_str());
    return;
  }
  ...
  if (!newContainer) {
    Logging::Log(LogLevel::Error, L"Invalid container: %s",
                 newContainerId.c_str());
    return;
  }
  ...
  if (element->IsContainer()) {
    Logging::Log(LogLevel::Error, L"Container cannot be contained: %s",
                 element->GetId().c_str());
    return;
  }
  ...
  if (newContainer->IsContained() &&
      !IsLayoutContainer(widget, newContainer->GetId())) {
    Logging::Log(LogLevel::Error, L"Nested containers are not allowed: %s",
                 newContainerId.c_str());
    return;
  }
  ...
  if (WouldCreateContainerCycle(element, newContainer)) {
    Logging::Log(LogLevel::Error, L"Container cycle detected for: %s",
                 newContainerId.c_str());
    return;
  }
...
bool WidgetLayoutHelper::WouldCreateContainerCycle(const Element *element,
...
void WidgetLayoutHelper::RenderContainerChildren(const Widget &widget,
                                                 Element *container) {
  ...
  D2D1_RECT_F clipRect = D2D1::RectF((float)bounds.X, (float)bounds.Y,
  ...
  if (!isLayoutContainer && bounds.Width > 0 && bounds.Height > 0) {
    ...
    if (SUCCEEDED(hr) && maskTarget) {
      ...
      maskTarget->SetTransform(
          D2D1::Matrix3x2F::Translation(-(FLOAT)bounds.X, -(FLOAT)bounds.Y));

      if (ShapeElement *shapeContainer =
              dynamic_cast<ShapeElement *>(container)) {
        ...
        if (factory && shapeContainer->CreateGeometry(factory, geom)) {
          ...
          if (Direct2D::CreateSolidBrush(maskTarget.Get(), RGB(255, 255, 255),
                                         1.0f, &brush)) {
      ...
      if (maskBitmap) {
        ...
        HRESULT brushResult =
            context->CreateBitmapBrush(maskBitmap.Get(), &brush1);
  ...
  if (hasOpacityMask) {
    context->PushLayer(D2D1::LayerParameters(clipRect, nullptr,
  ...
  // Children have container-relative coordinates, so we translate by (bounds.X
  // - scrollX, bounds.Y - scrollY)
  ...
  D2D1_MATRIX_3X2_F translate = D2D1::Matrix3x2F::Translation(
      (float)bounds.X - scrollX, (float)bounds.Y - scrollY);
  context->SetTransform(translate * originalTransform);

  // Render all children of this container (elements stored in widget's flat
  // list)
  for (Element *child : container->GetContainerItems()) {
    ...
    } else if (child->GetType() == ELEMENT_LAYOUT_BOX) {
      child->Render(context);
    }
    if (child->IsContainer()) {
  ...
  // Restore transform before drawing scrollbars (in container-local space, not
  // scrolled)
  context->SetTransform(originalTransform);

  // Draw scrollbars using dedicated ScrollbarRenderer
  ScrollbarRenderer::DrawScrollbars(widget, container, context, bounds);

  context->PopLayer();
}

bool WidgetLayoutHelper::HitTestContainerChildren(Element *container, int x,
                                                  int y, Element *&outElement) {
  // Note: This overload exists for backwards compatibility
  // In practice, we need the Widget reference, so we pass through detailed
  // version The detailed version will handle the container hit testing logic
  Element *outActionElement = nullptr;
  Element *outMouseActionElement = nullptr;
  Element *outToolTipElement = nullptr;

  // For the simple version, we create a minimal hit test without
  // Widget-specific features This is safe because the basic hit testing doesn't
...
bool WidgetLayoutHelper::HitTestContainerChildrenDetailed(
    const Widget &widget, Element *container, int x, int y, UINT message,
    WPARAM wParam, Element *&outHitElement, Element *&outActionElement,
    Element *&outMouseActionElement, Element *&outToolTipElement) {
  if (!container || !container->IsContainer() || !container->IsVisible())
    return false;
  ...
  for (auto it = items.rbegin(); it != items.rend(); ++it) {
    ...
    if (child->IsContainer()) {
      if (HitTestContainerChildrenDetailed(
              widget, child, x, y, message, wParam, outHitElement,
              outActionElement, outMouseActionElement, outToolTipElement)) {
        return true;
      }
      continue;
    }
