...
void ApplyToolbarIcon(HWND hWnd, const WidgetOptions &options,
  ...
  if (!options.toolbarIcon.empty()) {
    icon = reinterpret_cast<HICON>(
        LoadImageW(nullptr, options.toolbarIcon.c_str(), IMAGE_ICON, 0, 0,
    ...
    if (icon) {
      ...
      Logging::Log(LogLevel::Warn,
                   L"Widget '%s': failed to load toolbarIcon '%s'",
                   options.id.c_str(), options.toolbarIcon.c_str());
    }
  }
...
void ApplyToolbarTitle(HWND hWnd, const WidgetOptions &options) {
  ...
  const std::wstring title =
      options.toolbarTitle.empty() ? options.id : options.toolbarTitle;
