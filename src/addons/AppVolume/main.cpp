...
std::string WideToUtf8(const std::wstring &s) {
  ...
  WideCharToMultiByte(CP_UTF8, 0, s.c_str(), -1, out.data(), len, nullptr,
                      nullptr);
  return out;
}
...
std::wstring ResolveIconPath(const std::wstring &filePath) {
  ...
  if (FileExistsNonEmpty(outPath) ||
      ExtractFileIconToIco(filePath, outPath, 64)) {
  ...
  do {
    ...
    for (int i = 0; i < count; ++i) {
      ...
      if (byPid) {
        ...
        std::wstring curName =
            ToLowerCopy(FileNameFromPath(GetProcessPathByPid(curPid)));
...
bool AppVolumeListSessions(std::vector<AppVolumeSessionInfo> &sessions) {
  ...
  do {
    ...
    for (int i = 0; i < count; ++i) {
      ...
      if (SUCCEEDED(control2->QueryInterface(IID_IAudioMeterInformation_Local,
                                             (void **)&meter)) &&
          meter) {
...
bool AppVolumeGetByPid(uint32_t pid, float &outVolume, bool &outMuted,
                       float &outPeak) {
...
bool AppVolumeGetByProcessName(const std::wstring &processName,
                               float &outVolume, bool &outMuted,
                               float &outPeak) {
  std::vector<AppVolumeSessionInfo> sessions;
  if (!AppVolumeListSessions(sessions))
    return false;
...
bool AppVolumeSetVolumeByPid(uint32_t pid, float volume01) {
  ...
  if (volume01 > 1.0f)
    volume01 = 1.0f;
  return SetForMatchingSessions(static_cast<DWORD>(pid), L"", true, &volume01,
                                nullptr);
}

bool AppVolumeSetVolumeByProcessName(const std::wstring &processName,
                                     float volume01) {
  if (volume01 < 0.0f)
    volume01 = 0.0f;
  if (volume01 > 1.0f)
...
bool AppVolumeSetMuteByPid(uint32_t pid, bool mute) {
  return SetForMatchingSessions(static_cast<DWORD>(pid), L"", true, nullptr,
                                &mute);
}
...
int JsAppVolumeGetByProcessName(novadesk_context ctx) {
  ...
  bool muted = false;
  if (!nameC ||
      !AppVolumeGetByProcessName(Utf8ToWide(nameC), volume, muted, peak)) {
    g_Host->PushNull(ctx);
    return 1;
  }
  g_Host->PushObject(ctx);
...
int JsAppVolumeSetVolumeByPid(novadesk_context ctx) {
  if (!g_Host->IsNumber(ctx, 0) || !g_Host->IsNumber(ctx, 1)) {
    g_Host->ThrowError(
        ctx, "appVolume.setVolumeByPid(pid, volume01) requires pid and volume");
    return 0;
  }
  int pid = static_cast<int>(g_Host->GetNumber(ctx, 0));
  float volume = static_cast<float>(g_Host->GetNumber(ctx, 1));
  g_Host->PushBool(
      ctx, AppVolumeSetVolumeByPid(static_cast<uint32_t>(pid), volume) ? 1 : 0);
  return 1;
}

int JsAppVolumeSetVolumeByProcessName(novadesk_context ctx) {
  if (!g_Host->IsString(ctx, 0) || !g_Host->IsNumber(ctx, 1)) {
    g_Host->ThrowError(ctx, "appVolume.setVolumeByProcessName(name, volume01) "
                            "requires name and volume");
    return 0;
  }
  const char *nameC = g_Host->GetString(ctx, 0);
  float volume = static_cast<float>(g_Host->GetNumber(ctx, 1));
  g_Host->PushBool(
      ctx, (nameC && AppVolumeSetVolumeByProcessName(Utf8ToWide(nameC), volume))
               ? 1
               : 0);
  return 1;
}
...
int JsAppVolumeSetMuteByPid(novadesk_context ctx) {
  if (!g_Host->IsNumber(ctx, 0) || g_Host->GetTop(ctx) < 2) {
    g_Host->ThrowError(
        ctx, "appVolume.setMuteByPid(pid, mute) requires pid and mute");
    return 0;
  }
  int pid = static_cast<int>(g_Host->GetNumber(ctx, 0));
  ...
  g_Host->PushBool(
      ctx,
      AppVolumeSetMuteByPid(static_cast<uint32_t>(pid), mute != 0) ? 1 : 0);
  return 1;
}
...
int JsAppVolumeSetMuteByProcessName(novadesk_context ctx) {
  if (!g_Host->IsString(ctx, 0) || g_Host->GetTop(ctx) < 2) {
    g_Host->ThrowError(
        ctx,
        "appVolume.setMuteByProcessName(name, mute) requires name and mute");
    return 0;
  }
  const char *nameC = g_Host->GetString(ctx, 0);
  int mute = g_Host->GetBool(ctx, 1);
  g_Host->PushBool(ctx, (nameC && AppVolumeSetMuteByProcessName(
                                      Utf8ToWide(nameC), mute != 0))
                            ? 1
                            : 0);
  return 1;
}
...
NOVADESK_ADDON_INIT(ctx, hMsgWnd, host) {
  ...
  addon.RegisterFunction("setVolumeByProcessName",
                         JsAppVolumeSetVolumeByProcessName, 2);
  addon.RegisterFunction("setMuteByPid", JsAppVolumeSetMuteByPid, 2);
  addon.RegisterFunction("setMuteByProcessName",
                         JsAppVolumeSetMuteByProcessName, 2);
}
