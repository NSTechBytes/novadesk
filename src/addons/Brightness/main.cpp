...
public:
  explicit BrightnessControl(int displayIndex = 0)
  ...
  bool GetBrightness(BrightnessInfo &outInfo) {
    ...
    const bool ok = (m_method == Method::LaptopIoctl)
                        ? GetLaptopBrightness(outInfo)
                        : false;
    if (!ok) {
      outInfo.supported = false;
    }
...
private:
  enum class Method { Unknown, LaptopIoctl, None };
...
#define IOCTL_VIDEO_QUERY_SUPPORTED_BRIGHTNESS                                 \
  CTL_CODE(FILE_DEVICE_VIDEO, 0x125, METHOD_BUFFERED, FILE_ANY_ACCESS)
    ...
    if (!DeviceIoControl(m_lcdDevice, IOCTL_VIDEO_QUERY_SUPPORTED_BRIGHTNESS,
                         nullptr, 0, levels, sizeof(levels), &bytesReturned,
                         nullptr) ||
        bytesReturned < 2) {
    ...
    m_laptopLevels.erase(
        std::unique(m_laptopLevels.begin(), m_laptopLevels.end()),
        m_laptopLevels.end());
  ...
  BYTE FindClosestLaptopLevel(BYTE raw) const {
    ...
    for (BYTE level : m_laptopLevels) {
      const int diff =
          std::abs(static_cast<int>(level) - static_cast<int>(raw));
      if (diff < bestDiff) {
        best = level;
        bestDiff = diff;
      }
...
#define IOCTL_VIDEO_QUERY_DISPLAY_BRIGHTNESS                                   \
  CTL_CODE(FILE_DEVICE_VIDEO, 0x126, METHOD_BUFFERED, FILE_ANY_ACCESS)
    ...
    if (!DeviceIoControl(m_lcdDevice, IOCTL_VIDEO_QUERY_DISPLAY_BRIGHTNESS,
                         nullptr, 0, values, sizeof(values), &bytesReturned,
                         nullptr) ||
        bytesReturned < 3) {
      return false;
    }
    ...
    if (maxRaw > minRaw) {
      outInfo.percent = static_cast<int>(
          ...
          static_cast<double>(maxRaw - minRaw));
    } else {
      outInfo.percent = 0;
    }
...
#define IOCTL_VIDEO_SET_DISPLAY_BRIGHTNESS                                     \
  CTL_CODE(FILE_DEVICE_VIDEO, 0x127, METHOD_BUFFERED, FILE_ANY_ACCESS)
    ...
    if (maxRaw > minRaw) {
      targetRaw = static_cast<BYTE>(
          minRaw + static_cast<BYTE>((static_cast<double>(maxRaw - minRaw) *
    ...
    return DeviceIoControl(m_lcdDevice, IOCTL_VIDEO_SET_DISPLAY_BRIGHTNESS,
                           setValues, sizeof(setValues), nullptr, 0,
...
bool TryReadPropInt(novadesk_context ctx, const char *name, int &outValue,
                    bool &outFound) {
  outFound = false;
  ...
  if (top > 0 && !g_Host->IsNull(ctx, 0)) {
    if (g_Host->IsObject(ctx, 0)) {
      ...
      if (!TryReadPropInt(ctx, "percent", percent, foundPercent)) {
        g_Host->ThrowError(ctx,
                           "setValue({ percent }) requires numeric percent");
        return 0;
      }
      if (!foundPercent) {
        g_Host->ThrowError(ctx,
                           "setValue({ percent[, display] }) requires percent");
        return 0;
      }
