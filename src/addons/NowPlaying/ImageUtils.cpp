...
hstring SaveCover(IRandomAccessStreamReference image) {
  try {
    ...
    auto cover_file =
        temp_folder
            .CreateFileAsync(L"cover.png",
                             CreationCollisionOption::ReplaceExisting)
            .get();

    cover_stream
        .ReadAsync(cover_buffer, cover_buffer.Capacity(),
...
bool CoverHasTransparentBorder(hstring original) {
  ...
  if (cover.LockBits(&r, ImageLockModeRead, PixelFormat32bppARGB, &data) !=
      Ok) {
    return false;
  }
...
hstring CropCover(hstring original) {
  try {
    ...
    auto cropped_file =
        temp_folder
            .CreateFileAsync(L"cover_cropped.png",
                             CreationCollisionOption::ReplaceExisting)
            .get();
