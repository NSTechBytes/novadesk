...
void MediaController::WorkerThread() {
  ...
  while (true) {
    std::queue<MediaActionItem> pending;
    {
      ...
      m_actionCv.wait_until(lock, nextRefresh,
                            [this] { return m_stop || !m_actions.empty(); });
...
void MediaController::UpdateData() {
  ...
  // Fallback: If no current session, try to find ANY playing session from the
  // list
  if (!session) {
    try {
  ...
  if (session) {
    try {
      ...
      if (pbInfo) {
        ...
        if (shuffle)
          newStats.shuffle = shuffle.Value();
      ...
      if (timeline) {
        ...
        newStats.duration = static_cast<int>(
        ...
        if (newStats.state == 1 && newStats.duration > 0) {
          if (!m_hasLocalPos) {
            ...
            double elapsed =
                std::chrono::duration<double>(nowSteady - m_localPosTime)
                    .count();
            m_localPosTime = nowSteady;
            if (elapsed > 0 && elapsed < 5.0)
              m_localPosSec += elapsed;
...
void MediaController::UpdateCover(
  ...
  if (thumb) {
    ...
    if (!path.empty()) {
      ...
      bool isSpotify = (std::wstring_view(playerAppId).find(L"Spotify") !=
