...
bool IsModifierVk(UINT vk) {
  return vk == VK_CONTROL || vk == VK_LCONTROL || vk == VK_RCONTROL ||
         vk == VK_MENU || vk == VK_LMENU || vk == VK_RMENU || vk == VK_SHIFT ||
...
void PostHotkeyEvent(int id, bool down) {
  ...
  PostMessageW(g_MessageWindow, WM_USER + 101,
               reinterpret_cast<WPARAM>(&DispatchHotkey),
               reinterpret_cast<LPARAM>(evt));
}
...
bool ReadHandler(novadesk_context ctx, int argIndex, void *&outDown,
                 void *&outUp) {
  outDown = nullptr;
  outUp = nullptr;
...
int JsHotkeyRegister(novadesk_context ctx) {
  ...
  if (top < 2 || !g_Host->IsString(ctx, 0)) {
    g_Host->ThrowError(
        ctx,
        "hotkey.register(hotkey, handler) requires hotkey string and handler");
    return 0;
  }
  ...
  if (!ReadHandler(ctx, 1, downFn, upFn)) {
    g_Host->ThrowError(ctx,
                       "hotkey.register handler must be function or object");
    return 0;
  }
...
NOVADESK_ADDON_UNLOAD() { ClearHotkeys(); }
