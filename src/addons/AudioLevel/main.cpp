...
public:
  ...
  bool GetStats(AudioLevelStats &outStats, const AudioLevelConfig &config) {
    ...
    outStats.peak[0] =
        Clamp01(m_peak[0] * static_cast<float>(m_config.peakGain));
    outStats.peak[1] =
        Clamp01(m_peak[1] * static_cast<float>(m_config.peakGain));
...
private:
  ...
  bool SameConfig(const AudioLevelConfig &c) const {
    return m_config.port == c.port && m_config.deviceId == c.deviceId &&
           m_config.fftSize == c.fftSize &&
           m_config.fftOverlap == c.fftOverlap && m_config.bands == c.bands &&
           m_config.freqMin == c.freqMin && m_config.freqMax == c.freqMax &&
           ...
           m_config.fftDecay == c.fftDecay && m_config.rmsGain == c.rmsGain &&
  ...
  bool EnsureInitialized(const AudioLevelConfig &config) {
    ...
    if (!m_config.deviceId.empty())
      ...
      hr = m_enumerator->GetDefaultAudioEndpoint(dataFlow, eMultimedia,
                                                 &m_device);
  ...
  std::vector<float> BuildBands(int bands) const {
    ...
    const double freqMax =
        (m_config.freqMax <= freqMin) ? 20000.0 : m_config.freqMax;
    ...
    for (int i = 0; i < bands; ++i) {
      ...
      const double f2 =
          freqMin * std::pow(freqMax / freqMin, static_cast<double>(i + 1) /
                                                    static_cast<double>(bands));
      int idx1 = static_cast<int>(f1 * 2.0 * outSize /
                                  static_cast<double>(m_sampleRate));
      int idx2 = static_cast<int>(f2 * 2.0 * outSize /
                                  static_cast<double>(m_sampleRate));
      if (idx1 < 0)
        idx1 = 0;
      if (idx2 >= outSize)
  ...
  void PollCapture() {
    ...
    while (SUCCEEDED(hr) && packetLength > 0) {
      ...
      if (!silent && data && frames > 0) {
        ...
        if (isFloat) {
          ...
          for (UINT32 i = 0; i < frames; ++i) {
            const float l =
                static_cast<float>(in[i * m_pwfx->nChannels + 0]) / 32768.0f;
            const float r =
                (ch > 1) ? static_cast<float>(in[i * m_pwfx->nChannels + 1]) /
                               32768.0f
                         : l;
            sumSq[0] += l * l;
            sumSq[1] += r * r;
    ...
    if (hadAudio && frameCount > 0) {
      ...
      m_rms[1] = Clamp01(rmsNow1 +
                         m_kRMS[(rmsNow1 < m_rms[1])] * (m_rms[1] - rmsNow1));
      m_peak[0] = Clamp01(peak[0] + m_kPeak[(peak[0] < m_peak[0])] *
                                        (m_peak[0] - peak[0]));
