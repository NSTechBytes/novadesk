 ...
 *
 * This header provides an engine-agnostic interface for extending Novadesk with
 * native C++ DLLs. It includes a raw C-compatible Host API and a C++ helper
 * layer for safe registration.
 */
 ...
 *
 * Addons interact with the JavaScript engine exclusively through these
...
typedef void (*NovadeskAddonInitFn)(novadesk_context ctx, HWND hMsgWnd,
...
class JsFunction {
public:
  JsFunction(novadesk_context ctx, const NovadeskHostAPI *host, int idx)
...
/**
 * @class Dispatcher
 * @brief Mechanism for safe communication from background threads to the main
 * thread.
 */
class Dispatcher {
public:
...
/**
 * @class Addon
 * @brief The main C++ helper for creating addons and registering
 * properties/functions.
 */
class Addon {
public:
  /// Creates a primary addon object for the current context.
  Addon(novadesk_context ctx, const NovadeskHostAPI *host)
      : m_ctx(ctx), m_host(host) {
    m_host->PushObject(m_ctx);
  }

  /// Creates a nested object and populates it using the provided lambda.
  template <typename F> void RegisterObject(const char *name, F populateFunc) {
  ...
  void RegisterFunction(const char *name, int (*func)(novadesk_context ctx),
                        int nargs = 0) {
  ...
  void RegisterArray(const char *name, const std::vector<double> &values) {
    m_host->RegisterArrayNumber(m_ctx, name, values.data(),
                                (size_t)values.size());
  }
  ...
  bool GetProperty(int objIndex, const char *name) {
    return m_host->GetProperty(m_ctx, objIndex, name) != 0;
  }
...
private:
  Addon(novadesk_context ctx, const NovadeskHostAPI *host, bool)
      : m_ctx(ctx), m_host(host) {}
  novadesk_context m_ctx;
  const NovadeskHostAPI *m_host;
};
